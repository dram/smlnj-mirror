 ...
 * (e.g. function Var for mcexp constructor Var) *)
...
(* Letm : V.var list * SV.svar list * mcexp -> mcexp *)
(* "let (v1, ..., vn) = (sv1, ..., svn) in rhsexp" *)
(* Letm is no longer needed because we use MCUtil.substVars to directly substitute
 * the svars for the vars from the pattern.
fun Letm (vars: V.var list, svars: SV.svar list, body) =
    let fun wrapLets (nil,nil) = body
	  | wrapLets (v::restv, sv::restsv) =
	    mkLetVar (v, VARexp(ref(SV.svarToVar sv), nil),
    ...
    (case cases  (* distinguish vector switch special case by key = V _ *)
       of nil => bug "Switch: empty cases"
	    ...
	    (* "let val len = Vector.length svar in <<switch over int values of len>>"
             * where "len" is a fresh internal variable (generated in Translate.mkExp0) *)
...
fun Sfun (pvars, body) =
	...
	val rule = RULE(AS.VARpat argvar, wrappedBody)
     in AS.FNexp([rule], pvarsTy, T.UNDEFty) (* result type won't be needed *)
    end
