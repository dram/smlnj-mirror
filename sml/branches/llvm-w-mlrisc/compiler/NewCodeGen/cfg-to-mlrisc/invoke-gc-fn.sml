...
functor InvokeGCFn (
    ...
    structure Regs : CPSREGS where T.Region = CFGRegions
    structure TS : MLTREE_STREAM where T = Regs.T
    structure CtlFlowGraph : CONTROL_FLOW_GRAPH where P = TS.S.P
    ...
    type stream = (TS.T.stm, TS.T.mlrisc list, CtlFlowGraph.cfg) TS.stream
  ...
  (* generate the actual GC invocation code *)
    ...
    structure Cells = Regs.C
    structure CtlFlowGraph = CtlFlowGraph
    structure TS = TS
    ...
    type stream = (T.stm, T.mlrisc list, CtlFlowGraph.cfg) TS.stream
    ...
    *)
     val calleesaves = List.take(Regs.miscregs, MS.numCalleeSaves)

   (*
    * registers that are the roots of gc.
    *)
     val gcParamRegs =
	    Regs.stdlink vfp :: Regs.stdclos vfp ::
	    Regs.stdcont vfp :: Regs.stdarg vfp ::
	    calleesaves
    ...
    *)
     val gcCall = let
	   ...
	   val def = case Regs.exhausted of NONE => use | SOME cc => T.CCR cc::use
	   val call = T.CALL{
		   funct = T.LOAD(pty,
		     T.ADD(addrTy, Regs.frameptr vfp, LI MS.startgcOffset),
		     R.stack),
   ...
   *)
    fun set bindings = let
	  val theVfp = Regs.vfp
	  val theFp = (case Regs.frameptr false
   ...
   *)
    fun checkLimit (emit, maxAlloc) = let
	  ...
	  in
	    if maxAlloc < skidPad
	      then (case Regs.exhausted
		 of SOME cc => gotoGC cc
		  | NONE => gotoGC normalTestLimit
		(* end case *))
	      else let
		val shiftedAllocPtr = T.ADD(addrTy,Regs.allocptr,LI(maxAlloc-skidPad))
		val shiftedTestLimit = T.CMP(pty, gcCmp, shiftedAllocPtr, Regs.limitptr vfp)
		in
		  case Regs.exhausted
   ...
   *)
    fun computeBasePtr(emit,defineLabel,annotation) = let
	  ...
	  val baseExp =
	        T.ADD(addrTy, Regs.gcLink vfp, T.LABEXP(T.SUB(addrTy, baseOffset, T.LABEL returnLab)))
	  in
	    defineLabel returnLab;
	    annotation(ZERO_FREQ);
	    emit (case Regs.baseptr vfp
  ...
  (* Partition the root set into boxed, integer, and float types *)
    fun split ([], [], boxed, int, float) = {boxed=boxed, int=int, float=float}
      | split (T.GPR r::rl, CFG.NUMt _::tl, b, i, f) = split(rl, tl, b, r::i, f)
      | split (T.GPR r::rl, CFG.FLTt _::tl, b, i, f) = error "split: T.GPR"
      | split (T.GPR r::rl, _::tl, b, i, f) = split(rl, tl, r::b, i, f)
      | split (T.FPR r::rl, CFG.FLTt _::tl, b, i, f) = split(rl, tl, b, i, r::f)
      | split _ = error "split"
...
(* REAL32: FIXME *)
	 ...
	 *)
		 ...
		 of ([], []) => map bind boxed
			...
			case float
			 of [] => ()
			  | _  => if align
			      then emit(T.MV(addrTy, allocptrR, T.ORB(addrTy, Regs.allocptr, LI 4)))
			      else ()
			(* end case *);
	 ...
	 *)
	  fun prolog (hp, unusedRoots, [], rds, rss) = let
		...
		(* update the heap pointer if we have done any allocation *)
		  if hp > 0
		    then emit(T.MV(addrTy, allocptrR, T.ADD(addrTy, Regs.allocptr, LI hp)))
		    else ();
	...
	(* Make a record and put it in reg *)
	  and makeRecord (hp, {boxed, words, reg, fields, ...}) = let
		fun disp n = T.ADD(addrTy, Regs.allocptr, LI n)
   ...
   *)
    fun emitCallGC {
	  ...
	  fun setToMLTree {regs, mem} =
		...
		map (fn i => T.LOAD(ity, T.ADD(addrTy, Regs.frameptr vfp, LI(i)), R.memory)) mem
    ...
    *)
    fun invokeGC (stream, externalEntry) gcInfo = let
	   ...
	   *)
	    ...
	    exitBlock (case Regs.exhausted of NONE    => liveout | SOME cc => T.CCR cc::liveout)
	  end
