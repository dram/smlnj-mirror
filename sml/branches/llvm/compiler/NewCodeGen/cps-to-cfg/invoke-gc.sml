 ...
 *)
   ...
   *)
    fun assignRoots (isCont, params) = let
	  ...
	  val (rawObj, nBoxed) = (case (raw32, raw64)
		 of ([], []) => (Unit, nBoxed)
		  | _ => (Raw(List.rev raw32, List.rev raw64), nBoxed+1)
		(* end case *))
	  val nExcess = nBoxed - numGCRoots
	  val (boxed, excessObj) = if (nExcess > 0)
		then let
   ...
   *)
	...
	(* first we construct the code to unpack the live data from the new roots *)
	  ...
	  and unpackRecord (rExp, flds, k) = let
		val tpl = LV.mkLvar()
		fun select ix = C.SELECT{idx = ix, arg = var tpl}
		fun unpackFlds (ix, Boxed jx :: flds) = (
		      Array.update (results, jx, select ix);
		...
		in
		  C.LET(rExp, {name=tpl, ty=C.PTRt}, unpackFlds (0, flds))
		end
	...
	(* pack the roots from the live data *)
	  ...
	  and packRecord (flds, k) = let
		fun packFlds ([], n, args) = let
		  ...
		  | packFlds (Boxed ix :: flds, n, args) =
		      packFlds (flds, n+1, Vector.sub(live, ix) :: args)
	  ...
	  and packRaw (raw32, raw64, k) = raise Fail "unimplemented"
	  val code = pack (roots, [])
	  in
	    code
	  end
