...
functor CPStoCFGFn (MS : MACH_SPEC) : sig
  ...
  (* CFG integer constants *)
    ...
    val sNUM = num true
    val uNUM = num false
    fun sSzNum sz iv = CFG.NUM{iv = iv, signed = true, sz = sz}
    fun w2NUM iv = uNUM(Word.toLargeInt iv)
    fun mlInt' n = sNUM(n+n+1)
    fun mlInt n = mlInt'(IntInf.fromInt n)

  (* some useful constants *)
    val one = sNUM 1
    fun allOnes sz = uNUM(ConstArith.bNot(sz, 0)) (* sz-wide 1's *)

  (* convert CPS types to CFG types *)
    fun cvtTy cpsTy = (case cpsTy
	   of CPS.NUMt{sz, tag=true} => CFG.NUMt ity
...
(* QUESTION: should we track CFG types instead of CPS types? *)
    ...
    val getDescriptor = TP.LOAD_RAW{offset= ~ws, kind=TP.INT, sz=ity}

  (* get length field as tagged integer *)
    fun getObjLength obj =
	  pureOp (TP.ORB, ity, [
	      pureOp (TP.RSHIFTL, ity, [
		  CFG.PURE(getDescriptor, [obj]),
    ...
    fun gen (info) = let
	  ...
	  fun genV (VAR x) = binding x
	    ...
	    | genV (NUM{ty={sz, ...}, ival}) =
		CFG.NUM{iv = ival, signed = ??, sz = sz}
	    | genV (REAL _) = raise Fail "unexepected REAL value"
	    | genV (STRING _) = raise Fail "unexepected STRING value"
	    | genV VOID = raise Fail "unexepected VOID value"
...
??
		  | FIX _ => raise Fail "unexpected FIX"
		  | SWITCH(v, _, cases) => CFG.SWITCH(genV v, List.map genE cases)
		  | BRANCH(test, vs, _, k1, k2) =>
??
		  | SETTER(oper, vs, k) => genSetter (oper, List.map genV vs, k)
		  | LOOKER(oper, vs, x, ty, k) =>
		      genCont (genLooker (oper, List.map genV vs), x, ty, k)
...
??
		  | ARITH(P.IARITH{oper, sz}, vs, x, ty, k) => if isTaggedInt sz
			...
			fun continue (CFG.Var _) = bindVarIn (x, k)
			  | continue exp = genCont(exp, x, ity, k)
			in
			  genTagged (oper, sz, x, continue)
			end
		      else let
			fun arith (oper, vs) =
				...
				bindVarIn (x, k))
			in
			  case (oper, List.map genV args)
			   of (P.IADD, args) => arith (TP.IADD, args)
			    | (P.ISUB, args) => arith (TP.ISUB, args)
			    | (P.IMUL, args) => arith (TP.IMUL, args)
			    | (P.IDIV, args) => arith (TP.IDIV, args)
			    | (P.IMOD, args) => arith (TP.IMOD, args)
			    | (P.IQUOT, args) => arith (TP.IQUOT, args)
			    | (P.IREM, args) => arith (TP.IREM, args)
			    | (P.INEG, [a]) => arith (TP.ISUB, [sSzNum sz 0, a])
			  (* end case *)
			end
...
??
		(* end case *))
	  and allocRecord (desc, fields, x, k) = let
		...
		fun getField (v, p) = getPath (genV v, p)
		in
		  CFG.ALLOC(record desc, List.map getField fields, x, k())
		end
	  and genSetter (oper, args, k) = (case (oper, args)
		 of (P.NUMUPDATE{kind}, _) =>
??
...
??
		(* end case *))
	  and genLooker (oper, args) = (case (oper, args)
		 of (P.DEREF, _) => CFG.LOOKER(TP.DEREF, args)
		  | (P.SUBSCRIPT, [arr, ix]) =>
		      CFG.LOOKER(TP.SUBSCRIPT, [
			  CFG.SELECT(genV arr, 0), untagSigned ix
			])
		  ...
		  | (P.NUMSUBSCRIPT{kind=FLOAT sz}, [arr, ix]) =>
		      genRawSubscript (TP.FLT, sz, v1, v2)
		  | (P.GETSPECIAL, [obj]) => getObjLength obj
		  | (P.GETHDLR, _) => CFG.LOOKER(TP.GET_HDLR, args)
		  | (P.GETVAR, _)=> CFG.LOOKER(TP.GET_VAR, args)
		  | (P.RAWLOAD{kind}, [adr]) =>
??
		  | (P.RAWLOAD{kind}, [adr, ix]) =>
??
		(* end case *))
	  and genPure (p, vs) = (case (p, vs)
		 of (P.PURE_ARITH{oper, kind=P.INT sz}, _) =>
		      if isTaggedInt sz
			then genPureTagged (oper, true, sz, vs)
			else (case (oper, List.map genV vs)
			   of (P.NEG, [a]) => pureOp (TP.SUB, sz, [zero, comp v])
		  ...
		  | (P.PURE_ARITH{oper, kind=P.UINT sz}, _) =>
			...
			then genPureTagged (oper, false, sz, vs)
			   ...
			   of (P.NEG, [a]) => pureOp (TP.SUB, sz, [zero, comp v])
			    | (P.NOTB, [a]) => pureOp (TP.XORB, sz, [comp v, allOnes sz])
			    | (P.ADD, vs) => pureOp (TP.ADD, sz, vs)
			    | (P.SUB, vs) => pureOp (TP.SUB, sz, vs)
		  ...
		  | (P.PURE_ARITH{oper, kind=P.FLOAT sz}, _) => (
		      case (rator, List.map comp vs)
		       of (ADD, args) => pureOp (TP.FADD, sz, args)
			| (SUB, args) => pureOp (TP.FSUB, sz, args)
			| (MUL, args) => pureOp (TP.FMUL, sz, args)
			| (FDIV, args) => pureOp (TP.FDIV, sz, args)
			| (FABS, args) => pureOp (TP.FABS, sz, args)
			| (FSQRT, args) => pureOp (TP.FSQRT, sz, args)
		  ...
		  | (P.PURE_NUMSUBSCRIPT{kind=P.FLOAT sz}, [v1, v2]) =>
		      genRawSubscript (TP.FLT, sz, v1, v2)
		  | (P.LENGTH, [v]) => CFG.SELECT(1, genV v)
		  | (P.OBJLENGTH, [v]) => (* get length field from object header *)
		      getObjLength (genV v)
		  | (P.COPY{from, to}, [v]) =>
		      if (from = to)
			then genV v
...
??
		  ...
		  | (P.SUBSCRIPTV, [v1, v2]) =>
			  ...
			  CFG.SELECT(0, genV v1),
			  untagSigned v2
			])
		  | (P.GETTAG, [v]) =>
		      toMLWord (pureOp (TP.ANDB, ity, [
			  CFG.PURE(getDescriptor, [genV v]),
			  uNUM(D.powTagWidth-1)
			]))
		  | (P.CAST, [v]) => genV v
		  ...
		  | (P.UNWRAP(P.INT sz), [v]) =>
		      CFG.PURE(TP.LOAD_RAW{sz=sz, kind=TP.INT, offset=0}, [genV v])
		  | (P.UNWRAP(P.FLOAT sz), [v]) =>
		      CFG.PURE(TP.LOAD_RAW{sz=sz, kind=TP.FLT, offset=0}, [genV v])
		  | (P.GETSEQDATA, [v]) => CFG.SELECT(0, genV v)
	...
	(* subscript from packed numeric vector *)
	  and genRawSubscript (kind, sz, vec, idx) =
		CFG.PURE(TP.PURE_RAW_SUBSCRIPT{kind=kind, sz=sz}, [
		    CFG.SELECT(genV vec, 0), untagSigned idx
		  ])
	  ...
	  and untagSigned (NUM{ival, ...}) = sNUM ival
	    | untagSigned v = pureOp (TP.RSHIFT, ity, [genV v, one])
	  and untagUnsigned (NUM{ival, ...}) = uNUM ival
