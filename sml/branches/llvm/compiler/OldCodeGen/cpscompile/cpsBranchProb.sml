...
signature CPS_BRANCH_PROB =
  sig
    ...
    val branchProb : CPS.function list list -> (CPS.lvar -> Probability.prob option)

  end

structure CpsBranchProb : CPS_BRANCH_PROB =
  struct
 ...
 (* condensed CPS flow graph *)
    ...
    fun computeProbs clusters = let
	  val infoTbl : info LV.Tbl.hash_table = LV.Tbl.mkTable(32, InfoTbl)
	  val insertInfo = LV.Tbl.insert infoTbl
	  val findInfo = LV.Tbl.find infoTbl

	  val brProbTbl : Prob.prob LV.Tbl.hash_table = LV.Tbl.mkTable(32, CpsProbTbl)
	 ...
	 *)
	  ...
	  fun assign (SWITCH cs) = List.app assign cs
		...
		fun combine(f, trueProb) = (case (f(), trueProb)
		       of (NONE, NONE) => NONE
			| (NONE, p as SOME _) => p
			| (p as SOME _, NONE) => p
		...
		in
		   case List.foldl combine NONE [ph, oh, rh, raiseExn, boundsCheck]
		    of NONE => ()
		     | SOME prob => LV.Tbl.insert brProbTbl (x, prob)
		   (*esac*);
		   assign(c1);
		   assign(c2)
		end
	    | assign _ = ()
	  fun doCluster cluster = List.app assign (List.map buildInfo cluster)
	  in
	    List.app doCluster clusters;
	    LV.Tbl.find brProbTbl
	  end (* computeProbs *)

    fun branchProb clusters = if !disableF
	  then (fn _ => NONE)
	  else computeProbs clusters
