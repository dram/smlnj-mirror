...
val patsUsed : int list ref = ref []  (* updateable ordered list of rule nos, ruleset ref *)
val maybeUsed : int list ref = ref [] (* updateable ordered list of rule nos, ruleset ref *)
val results : (lvar * lvar list) list ref = ref []

(* taglist assumed in ascending order, tag is inserted to maintain
 * ordering. insertion is "destructive", updates the taglist ref *)
fun insert(taglist: int list ref, tag : int) =
    let fun insertTag [] = [tag]
	  | insertTag (taglist as (t::more)) =
		if tag = t then taglist
		else if tag < t then tag :: taglist
		else t :: insertTag more
    in taglist := insertTag (!taglist)
    end

(* unusedRules : rule list -> rule list 
 * unusedRules rules = R.difference(rules, !patsUsed) *)
fun unusedRules rules =
    let fun find([],[],_) = []
	  | find([], _::r, i) = i :: find([],r,i+1)
	  | find(rules as rule::rules', _::r, i) =
		if rule = i
		then find(rules,r,i+1)
		else i :: find(rules',r,i+1)
	  | find _ = ErrorMsg.impossible "unusedRules in mc"
    in  find(!patsUsed,rules,0)
    end
	
(* redundant : rulest -> ruleset *)
fun redundant rules =  (* R.difference(rules, !patsUsed) *)
    if length rules = length (!patsUsed) then []
    else unusedRules rules

fun areNotRedundant () =
    case !maybeUsed
      of [] => ()
       | (tag::_) => (insert(patsUsed,tag); maybeUsed := [])
...
fun convert(bindings,tag) =
	...
	val args = order free
    in  insert(patsUsed,tag); APP(VAR f,RECORD(map VAR args))
    end
...
fun convertDefault(bindings,tag) =
	...
	val args = order free
    in  insert(maybeUsed,tag); APP(VAR f,RECORD(map VAR args))
    end

(* freevars: pat -> lvar list
   -- result is sorted list *)
fun freevars pat =
    case pat
      of VARpat(VALvar{access=LVAR v,...}) => [v]
...
fun andSwitch x =
let
(* andS : (pat list * lexp) list -> (<constructor> * (pat list * lexp)) list * (pat list * lexp) list *)
fun andS [] = ([],[])
