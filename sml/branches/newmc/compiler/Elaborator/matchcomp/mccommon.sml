...
structure MCCommon =  (* abbreviated MC *)
struct
...
(* andor: The "full" andor tree, with node Ids, types, and variants, and defaults.
 *  These are generated by the 2nd pass tree construction from the protoAndor tree.
 *  [OR]defaults rulesets are computed based on subtracting case rules (directs) from
 *  inherited live rule sets (active), without reference to variable bindings. *)
datatype andor
...
fun conEq (DATAcon (d1, _), DATAcon (d2, _)) = TU.eqDatacon (d1, d2)
  | conEq (INTcon {ival = ival1, ...}, INTcon {ival = ival2, ...}) = (ival1 = ival2)   (* types assumed compatible *)
...
(* getType : andor -> Types.ty *)
fun getType (AND{typ, ...}) = typ
  | getType (OR{typ, ...}) = typ
  | getType (VAR{typ, ...}) = typ
