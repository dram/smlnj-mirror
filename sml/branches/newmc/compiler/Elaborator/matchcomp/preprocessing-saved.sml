...
fun orExpand (AS.ORpat(pat1,pat2)) =
      (orExpand pat1)@(orExpand pat2)
  | orExpand (pat as AS.RECORDpat{fields,...}) =
     map (MU.mkRECORDpat pat) (foldr allConses [nil] (map (orExpand o #2) fields))
  | orExpand (AS.VECTORpat(pats,t)) =
      map (fn p => AS.VECTORpat(p,t)) (foldr allConses [nil] (map orExpand pats))
  | orExpand (AS.APPpat(k,t,pat)) =
      map (fn pat => AS.APPpat(k,t,pat)) (orExpand pat)
  | orExpand (AS.CONSTRAINTpat(pat,_)) =
      orExpand pat
  | orExpand (AS.LAYEREDpat(AS.CONSTRAINTpat(lpat, _), bpat)) =
      orExpand (AS.LAYEREDpat(lpat, bpat))
  | orExpand (AS.LAYEREDpat(lpat, bpat)) =
      map (fn pat => AS.LAYEREDpat(lpat,pat)) (orExpand bpat)
  | orExpand pat = [pat]
...
let fun processRule ((pat, rhs), (expandedRuleno, originalRuleno, ruleTable, pats, paths, rhsBinders)) =
	 let val pat = AbsynUtil.stripPatMarks pat
	     ...
	     (* abstractRHS : V.var list * exp -> exp *)
             (* Assumption: the pre-translated rhs may contain occurrences of the pattern pvar(s) *)
	     fun abstractRHS ([], rhs) = FNexp ([RULE(unitPat, rhs), BT.unitTy, rhsTy)  (* no pvars *)
	       | abstractRHS ([var], rhs) =
		   FNexp ([RULE (var, rhs) , V.varToType var, rhsTy)
