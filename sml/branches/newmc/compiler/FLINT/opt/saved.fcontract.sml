...
signature FCONTRACT =
  sig
    ...
    (* needs Collect to be setup properly (where? how?) *)
    val contract : options -> FLINT.prog -> FLINT.prog
    val contractCount : int ref
  end
...
structure FContract :> FCONTRACT =
  struct
    ...
    fun sayDebug msg = if !debugging then (Control_Print.say msg; Control_Print.flush()) else ()
    fun bug msg = ErrorMsg.impossible ("FContract: "^msg)
    fun buglexp (msg,le) = (say "\n"; PF.printLexp le; bug msg)
    fun bugval (msg,v) = (say "\n"; PF.printSval v; bug msg)
...
(* fun sayexn e = app say (map (fn s => s^" <- ") (SMLofNJ.exnHistory e)) *)
  ...
  (* a map that tracks the bindings of the lvars *)
	...
	(case sval2lty sv
	  of SOME lty => SOME(LT.lt_select(lty, i, "fcontract#238"))
	   | _ => NONE)
      | sval2lty _ = NONE
   ...
   *)
	...
	(contractCount := !contractCount + 1;
	 sayDebug (">>> FContract.contract "^Int.toString (!contractCount)^"\n"))
    fun endContract () =
	(sayDebug ("<<< FContract.contract "^Int.toString (!contractCount)^"\n"))
    ...
    fun contract {etaSplit,tfnInline} (fdec as (_,f,_,_)) = let
	  ...
	  fun lookup m lv = (case M.find(m,lv)
		 of NONE => (
		      ...
		      say (!PF.lvarToStringRef lv);
		      raise Lookup)
		  | SOME x => (sayDebug ("LOOK " ^ !PF.lvarToStringRef lv ^ "\n"); x)
		(*esac*))
	  ...
	  fun addbind (m,lv,sv) =
	      (sayDebug ("BIND "^ !PF.lvarToStringRef lv ^ "\n"); M.insert(m, lv, sv))
	 ...
	 *)
		    ...
		    (* F.APP inlining (if any) *)
			  ...
			  of Fun(g,body,args,{inline,...},actuals) =>
				 ...
				 fun simpleinline () =
				     ...
				     (click_simpleinline();
				      (* say(concat["fcApp.simpleinline ", PrintFlint.toStringValue f, " -> ", C.LVarString g, "\n"]); *)
		    ...
		    (* F.TAPP inlining (if any) *)
		      ...
		      fun specialize (g,tfk,args,body,tycs) = let
			    ...
			    in
				  ...
				  F.LET(_,nprog,F.RET _) => (PF.printLexp nprog; nprog)
				| _ => bug "specialize"
			    end
	  ...
	  in
	    (* C.collect fdec; -- why not? *)
	    case fcexp S.empty M.empty (F.FIX([fdec], F.RET[F.VAR f])) #2
	      of F.FIX([fdec], F.RET[F.VAR f]) => (endContract (); fdec)
