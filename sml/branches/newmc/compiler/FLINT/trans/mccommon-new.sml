...
fun constantEq (DATApcon (d1, _), DATApcon (d2, _)) = conEq(d1, d2)
      (* ignoring type instantiation tyvars, which are guaranteed to be equivalent at
       * the common pattern node. *)
  ...
  | constantEq (VLENpcon (n, _), VLENpcon (n',_)) = n = n'
      (* ignoring element type which is guaranteed to be the same when we are comparing them
       * at a given pattern node. *)
  | constantEq _ = false
