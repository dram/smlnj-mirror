(* FLINT/trans/andorsimple.sml *)
(* revised "old" match compiler *)

(* build a simple "proto"-AndOr tree (type simpleAndor), by layering the pattern infor for 
 * sequence of patterns (match left-hand-sides) to build up andorSimple nodes. *)

structure AndorSimple =
struct
...
(* makeAndor : pat list -> MCT.simpleAndor *)
...
let (* genAndor : pat * ruleno -> andor *)
    ...
    and mergeAndor (VARpat var, andor, rule) = addVar (var, rule, andor)
      ...
      | mergeAndor (CONpat _, ANDs _, _) =
	  bug "mergeAndor - merging constant constructor into record"
      | mergeAndor (APPpat _, ANDs _, _) =
