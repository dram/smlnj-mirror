...
(* flattenCases : andorCase list * path * ruleset * DA.consig -> decision   *)
...
fun generate (dt, matchRep, rootVar, (toTyc, toLty), giis) =
  let val (subtree, envout) = pass1(dt, [(0, [ROOTPATH])], matchRep)
      ...
      (* genswitch : PL.lexp * DA.consig * (con * lexp) list * lexp option -> PL.lexp *)
...
(* doMatchCompile : (pat * exp) list * (lexp -> lexp?) * lvar * toTcLt * errTy * giisTy
