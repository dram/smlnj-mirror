...
fun pass1 (RHS n, _, rlhs) = (RHS n, lhsToPathSet(n, rlhs))
  | pass1 (CHOICE {path, sign, cases, default = NONE}, inps, rlhs) =
      let val (cases', outps') =
	      pass1cases (cases, unionPS(inps, subPaths path), NONE, path, rlhs)
       in (CHOICE {path = path, sign = sign, cases = cases', default = NONE}, outps')
      end
  | pass1 (CHOICE {path, sign, cases, default = SOME defaultDT}, inps, rlhs) =
      let val newps = unionPS (inps, subPaths path)
	  val (defaultDT0, defaultOutps) = pass1 (defaultDT, newps, rlhs)
	  val (cases', outps') = pass1cases (cases, newps, SOME defaultOutps, path, rlhs)
  ...
  | pass1 (BIND _, _, _) = bug "pass1: BIND"

(* pass1cases : (con * dectree) list * pathSet * pathSet option * path * ramifiedLHS
 *              -> (con * dectree) list * pathSet *)
and pass1cases ((con, dectree) :: rest, inps, outpsOp, path, rlhs) =
      let val (dectree0, outps0) = pass1(dectree, inps, rlhs)
	  ...
	  val outps1 =
		...
		| NONE => otherBindings
	  val (rest', outps2) = pass1cases (rest, inps, SOME outps1, path, rlhs)
	  val bindPathSet = unionPS (mustBindHere, differencePS (otherBindings, outps2))
	  val dectree1 = wrapBindings (bindPathSet, dectree0)
       in ((con, dectree1) :: rest', outps2)
      end
  | pass1cases (nil, inps, outpsOp, path, rlhs) =
      (case outpsOp
...
fun generate (dectree, rlhs, rootVar, (toTyc, toLty), giis) =
  let 
      ...
      fun genpath (PI n :: path : path, env : pathLvarEnv): PL.lexp =
	    ...
	    in PL.APP(PL.PRIM(PO.SUBSCRIPTV, lt_sub, [tycc]),
		 PL.RECORD [ PL.VAR (lookupPath(path, env)),
		             PL.INT {ival = IntInf.fromInt n, ty = Target.defaultIntSz} ])
            end
        | genpath (VLEN t :: path, env) =
            let val tc = toTyc t
                val lt_len = LT.ltc_poly([LT.tkc_mono],
                                 [LT.ltc_parrow(LT.ltc_tv 0, LT.ltc_int)])
                val argtc = LT.tcc_vector tc
             in PL.APP (PL.PRIM(PO.LENGTH, lt_len, [argtc]),
			PL.VAR(lookupPath(path, env)))
            end
        | genpath (nil, env) = PL.VAR(lookupPath(nil, env))
	...
	| genswitch (sv, _, cases as ((PL.INTcon{ty=0, ...}, _) :: _), default) =
      ...
      val (dectree', pathset) = pass1 (dectree, [(0, [nil])], rlhs)

   in case wrapBindings(pathset, dectree')
        of BIND (nil, dectree'') =>
             pass2 (dectree'', [(nil, rootVar)], matchRep)
         | _ => pass2 (dectree', [], matchRep)
  end (* fun generate *)
