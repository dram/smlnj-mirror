...
fun collectAndorStats (andor: andor) : unit =
	...
	and scanSubcase CONST = ()
	  | scanSubcase (DCON andor) = scanTree andor
	  | scanSubcase (VEC andors) = app scanTree andors
    ...
    in scanTree dectree;
       dectreeStats :=
         SOME {rulesUsed = !rules,
	       numFAIL = !countFAIL,
	       numRHS = !countRHS,
	       numCHOICE = !countCHOICE,
	       numTotal = !countCHOICE + !countRHS + !countFAIL,
...
fun reportStats () =
    case (!andorStats, !dectreeStats)
	  ...
	  SOME {rulesUsed, numCHOICE, numFAIL, numRHS, ...}) =>
	 if numCHOICE > caseThreshold then
	     (saynl "andor stats:";
	      ...
	      saynl "dectree stats:";
	      says ["  #CHOICE = ", Int.toString numCHOICE];
