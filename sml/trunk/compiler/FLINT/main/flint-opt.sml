...
structure FLINTOpt : sig
   ...
   * The "split" phase has been eliminated  *)
    ...
    structure PF = PPFlint  (* renamed to PrintFlint in newmc *)
...
*)
	...
	let val outS = TextIO.openAppend fileName  (* appends to file if called repeatedly *)
	    ...
	    PPFlint.printFundec prog  (* PPF.ppProg prog *)
    ...
    fun optimize (prog, sourceName) =
	  ...
	  * fk: flintkind      flint "variant" of f
	  ...
	  fun runphase (phase: string, pdata as (f, fk, l): phaseData) : phaseData =
		 ...
		 of (("fcontract" | "lcontract"), FK_DEBRUIJN) =>
		  ...
		  | ("wrap", FK_NAMED) =>
			  ...
			  val f_wrapped_deb = names2deb f_wrapped (* covert to FK_DEBRUIJN for type check *)
		       in if !CTRL.wrdebugging
			  then (dumpProg (sourceName ^ "-named.wrap", f_wrapped);
		  ...
		  | _ => (case phase
			    of "id" => ()  (* does nothing *)
			     | "wellformed" => wff(f,l)
			     | "recover" =>
			         let val getLty = #getLty (recover(f, fk = FK_REIFY))
