...
fun ppAndor ppstrm =
    let fun ppNode ppstrm (AND{path,live,defaults,children,...}) =
	    (PP.openHOVBox ppstrm (PP.Abs 0);
	     ...
	     PP.break ppstrm {nsp=1,offset=0};
	     ppRuleset ppstrm live;
	     PP.break ppstrm {nsp=1,offset=0};
	     ppRuleset ppstrm defaults;
	     PP.closeBox ppstrm;
	     ppAndChildren ppstrm children;
	     PP.closeBox ppstrm)
	  | ppNode ppstrm (OR{path,live,defaults,variants,...}) = 
	    (PP.openHOVBox ppstrm (PP.Abs 0);
             PP.openHBox ppstrm;
	     ppPath ppstrm path;
	     PP.break ppstrm {nsp=1,offset=0};
	  ...
	  | ppNode ppstrm (VARS{vars,live,defaults,...}) =
	    (PP.openHBox ppstrm;
	     ...
	     ppRuleset ppstrm defaults;
	     PP.closeBox ppstrm)
	  | ppNode ppstrm (LEAF{live,defaults,...}) =
	    (PP.openHBox ppstrm;
...
val ppDecTree =
    let fun ppDec ppstrm (D.CHOICE{node,choices,default}) =
            (PP.openHBox ppstrm;
	     PP.string ppstrm "DEC";
	     PP.break ppstrm {nsp=1,offset=0};
	     ppPath ppstrm (getPath node);
	     ppChoices ppstrm choices;
	     PP.closeBox ppstrm)
	  | ppDec ppstrm (D.DLEAF r) =
	    (PP.openHBox ppstrm;
	     PP.string ppstrm "DLEAF";
	     PP.break ppstrm {nsp=1,offset=0};
	     PU.ppi ppstrm r;
	     PP.closeBox ppstrm)
	  | ppDec ppstrm D.RAISEMATCH =
	    (PP.openHBox ppstrm;
	     PP.string ppstrm "MATCH";
	     PP.closeBox ppstrm)
	and ppChoices ppstrm decvariants =
...
val ppCode =
    let fun ppc ppstrm (C.Letr(v,vars,body)) =
	    (PP.openVBox ppstrm (PP.Abs 2);
             PP.openHBox ppstrm;
	     ...
	     ppc ppstrm body;
	     PP.closeBox ppstrm)
	  | ppc ppstrm (C.Case(v,cases,default)) =
	    (PP.openVBox ppstrm (PP.Rel 2);
	     ...
	     ppLvar ppstrm v;
	     PP.closeBox ppstrm;
	     ppcases ppstrm cases;
	     PP.closeBox ppstrm)
	  | ppc ppstrm (C.Var lvar) =
	     PU.ppi ppstrm lvar
	  | ppc ppstrm (C.RHS r) = 
	    (PP.openHBox ppstrm;
