...
functor MLRiscGen (
  ...
  (* lift selected CPS datatypes *)
    ...
    datatype accesspath = datatype CPS.accesspath
...
(* REAL32: FIXME *)
      | ctyToAnn (CPS.FLTt n) = raise Fail(concat[
  ...
  (* convert CPS type to gc type *)
    fun ctyToGCty (CPS.FLTt 64) = REAL64
      | ctyToGCty (CPS.FLTt n) = raise Fail(concat[
...
(* QUESTION: do we care about the redundancy between Target.mlValueSz and MS.wordBitWidth? *)
   ...
   *)
    fun codegen args = let
	  val { funcs : CPS.function list, limits:CPS.lvar -> (int*int), source } = args
...
(* REAL32: FIXME *)
	 ...
	 *)
	  exception TypTbl
	  val typTbl  : CPS.cty Tbl.hash_table = Tbl.mkTable(32, TypTbl)
	...
	fun genCluster cluster = let
	    ...
	    *)
	     ...
	     *)
	      val gcTest = if C.signedGCTest
		    then M.CMP(pty, M.GT, C.allocptr, C.limitptr vfp)
		    else M.CMP(pty, M.GTU, C.allocptr, C.limitptr vfp)
...
(* NOTE: by default, this function is a no-op, because Control.CG.memDisambiguate is false *)
	     ...
	     *)
	      fun resolveHpOffset (M.CONST(absoluteHpOffset)) = let
		    ...
		    in
		      emit(M.MV(pty, tmpR, M.ADD(addrTy, C.allocptr, LI' offset)));
		      M.REG(pty, tmpR)
		    end
		| resolveHpOffset e = e
	     ...
	     *)
	      fun resolveHpOffset' (M.CONST(absoluteHpOffset)) = let
		    val offset = absoluteHpOffset - !advancedHP
		    in
		      markPTR(M.ADD(addrTy, C.allocptr, LI' offset))
		    end
		| resolveHpOffset' e = e
	     ...
	     *)
	      fun allocRecord (markComp, mem, desc, fields, hp) = let
		    fun getField (v, e, CPS.OFFp n) = indexEA(e, n)
...
(* REAL32: FIXME *)
		...
		| tag (true, e) = tagSigned e
	      ...
	      fun untagUnsigned (CPS.NUM{ty={tag=true, ...}, ival}) = LI ival
		| untagUnsigned (CPS.NUM _) = error "untagUnsigned: boxed int"
		| untagUnsigned v = M.SRL(ity, regbind v, one)

	      fun untagSigned (CPS.NUM{ty={tag=true, ...}, ival}) = LI ival
	     ...
	     *)
	      fun callSetup (formals, actuals) = let
		    fun isTreeified (CPS.VAR r) = (treeify r = TREEIFIED)
		      | isTreeified _ = false
	     ...
	     *)
	      fun genCPSFunction (lab, kind, f, params, formals, tys, e) = let
		    ...
		    fun init e = (case e
			   of CPS.RECORD(k,vl,x,e) => (
				case k
				 of (CPS.RK_FCONT | CPS.RK_RAW64BLOCK) => hasFloats := true
				  | _ => ()
				(* end case *);
				addRecValues vl; add(x, CPSUtil.BOGt);
				init e)
			    | CPS.SELECT(_,v,x,t,e) => (addValue v; add(x,t); init e)
			    | CPS.OFFSET(_,v,x,e) => (addValue v; add(x, CPSUtil.BOGt); init e)
			    | CPS.SWITCH(v,_,el) => (needBasePtr := true; addValue v; app init el)
			    | CPS.SETTER(_,vl,e) => (addValues vl; init e)
			    | CPS.LOOKER(looker,vl,x,t,e) => (
				 addValues vl;
				...
				*)
				 ...
				 (* end case *);
				 add(x,t); init e)
			    | CPS.ARITH(_,vl,x,t,e) => (addValues vl; add(x,t); init e)
			    | CPS.RCC(_,_,_,vl,wl,e) => (addValues vl; app add wl; init e)
			    | CPS.PURE(p,vl,x,t,e) => (
				case p
			    ...
			    | CPS.BRANCH(_,vl,_,e1,e2) => (addValues vl; init e1; init e2)
			    | CPS.APP(v,vl) => (addValue v; addValues vl)
			...
			| _ => let (* Standard function *)
				...
				then let
				  ...
				  in
				    emit(assign(C.baseptr(vfp), baseval))
				  end
				else ();
	      ...
	      and internalApp(f, args, hp) = (case lookupGenTbl f
		     of Frag.KNOWNFUN(ref(Frag.GEN formals)) => (
			  ...
			  in
			    ...
			    updtHeapPtr hp;
			    callSetup (formals, args);
			    genCPSFunction (lab, CPS.KNOWN, f, vl, formals, tl, e)
			  end
			  ...
			  in
			    ...
			    callSetup (formals, args);
			    testLimit hp;
			    genCPSFunction (lab, CPS.KNOWN_CHECK, f, vl, formals, tl, e)
			  end
...
(* REAL32: FIXME *)
	     ...
	     *)
		(** RECORD **)
	      and gen (CPS.RECORD(CPS.RK_VECTOR, vl, w, e), hp) = mkVector(vl, w, e, hp)
		| gen (CPS.RECORD(CPS.RK_FCONT, vl, w, e), hp) = mkFblock(vl, w, e, hp)
		| gen (CPS.RECORD(CPS.RK_RAW64BLOCK, vl, w, e), hp) = mkFblock(vl, w, e, hp)
		| gen (CPS.RECORD(CPS.RK_RAWBLOCK, vl, w, e), hp) = mkIntBlock(vl, w, e, hp)
		| gen (CPS.RECORD(_, vl, w, e), hp) = mkRecord(vl, w, e, hp)

		(*** SELECT ***)
		| gen (CPS.SELECT(i, NUM{ty={tag=true, ...}, ival}, x, t, e), hp) =
		    funnySelect(IntInf.fromInt i, ival, x, t, e, hp)
		| gen (CPS.SELECT(i, v, x, FLTt 64, e), hp) = f64select(i, v, x, e, hp) (* REAL32: *)
		| gen (CPS.SELECT(i, v, x, t, e), hp) = select(i, v, x, t, e, hp)

		(*** OFFSET ***)
		| gen (CPS.OFFSET(i, v, x, e), hp) =
		    defBoxed(x, scaleWord(regbind v, cpsInt i), e, hp)

		(*** APP ***)
		| gen (CPS.APP(NUM{ty={tag=true, ...}, ...}, args), hp) = updtHeapPtr hp
		| gen (CPS.APP(VAR f, args), hp) = externalApp(f, args, hp)
		| gen (CPS.APP(LABEL f, args), hp) = internalApp(f, args, hp)

		(*** SWITCH ***)
		| gen (CPS.SWITCH(NUM _, _, _), hp) = error "SWITCH on constant"
		| gen (CPS.SWITCH(v, _, l), hp) = let
		    val lab = newLabel ()
		...
		(*** PURE ***)
(* REAL32: FIXME *)
		| gen (CPS.PURE(P.INT_TO_REAL{from, to=64}, [v], x, _, e), hp) =
...
(* REAL32: FIXME *)
		| gen (CPS.PURE(P.PURE_ARITH{oper, kind=P.FLOAT 64}, [v], x, _, e), hp) = let
		    val r = fregbind v
		    in
		      case oper
		...
		| gen (CPS.PURE(P.PURE_ARITH{oper, kind=P.FLOAT 64}, [v,w], x, _, e), hp) = let
		    val v = fregbind v
		    val w = fregbind w
		    in
		      case oper
		...
		| gen (CPS.PURE(P.PURE_ARITH{oper=P.ORB, kind}, [v,w], x, _, e), hp) =
		    defWithKind(kind, x, M.ORB(ity, regbind v, regbind w), e, hp)
		| gen (CPS.PURE(P.PURE_ARITH{oper=P.ANDB, kind}, [v,w], x, _, e), hp) =
		    defWithKind(kind, x, M.ANDB(ity, regbind v, regbind w), e, hp)
		| gen (CPS.PURE(P.PURE_ARITH{oper, kind}, [v,w], x, ty, e), hp) = (case kind
		    ...
		    (* end case *))
		| gen (CPS.PURE(P.PURE_ARITH{oper=P.NOTB, kind}, [v], x, _, e), hp) = let
		    val sz = (case kind
			   of P.UINT sz => sz
		...
		| gen (CPS.PURE(P.PURE_ARITH{oper=P.NEG, kind}, [v], x, _, e), hp) = let
		    val sz = (case kind
			   of P.UINT sz => sz
		...
		| gen (CPS.PURE(P.COPY_INF _, _, _, _, _), hp) =
		    error "gen:PURE:COPY_INF"
		| gen (CPS.PURE(P.EXTEND{from=8, to}, [v], x, _ ,e), hp) = let
		...
		| gen (CPS.PURE(P.EXTEND{from, to}, [v], x, _ ,e), hp) =
		    if (from = to)
		      then copyM(from, x, v, e, hp)
		    else if (from = Target.defaultIntSz) andalso (to = ity)
		      then defINT (x, M.SRA(ity, regbind v, one), e, hp)
		      else error "gen:PURE:EXTEND"
		| gen (CPS.PURE(P.EXTEND_INF _, _, _, _, _), hp) =
		    error "gen:PURE:EXTEND_INF"
		| gen (CPS.PURE(P.TRUNC{from, to}, [v], x, _, e), hp) =
		    if (from = to)
		      then copyM(from, x, v, e, hp)
		    else if (to = 8)
		...
		| gen (CPS.PURE(P.TRUNC_INF _, _, _, _, _), hp) =
		    error "gen:PURE:TRUNC_INF"
		| gen (CPS.PURE(P.OBJLENGTH, [v], x, _, e), hp) =
		    defTAGINT(x, orTag(getObjLength v), e, hp)
		| gen (CPS.PURE(P.LENGTH, [v], x, t, e), hp) = select(1, v, x, t, e, hp)
		| gen (CPS.PURE(P.SUBSCRIPTV, [v, ix as NUM{ty={tag=true, ...}, ...}], x, t, e), hp) = let
		  (* get data pointer *)
		...
		| gen (CPS.PURE(P.SUBSCRIPTV, [v, w], x, _, e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		    val mem' = arrayRegion mem
 ...
 *)
		| gen (CPS.PURE(P.PURE_NUMSUBSCRIPT{kind=(P.UINT 8|P.INT 8)}, [v,i], x, _, e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		    val mem' = arrayRegion mem
		...
		| gen (CPS.PURE(P.GETTAG, [v], x, _, e), hp) =
		    defTAGINT(x,
		      tagUnsigned(M.ANDB(ity, getObjDescriptor v, LI(D.powTagWidth-1))),
		      e, hp)
		| gen (CPS.PURE(P.MKSPECIAL, [i, v], x, _, e), hp) = let
		    val desc = (case i
		...
		| gen (CPS.PURE(P.MAKEREF, [v], x, _, e), hp) = let
		...
		| gen (CPS.PURE(P.BOX, [u], w, _, e), hp) = copy(PTR, w, u, e, hp)
		| gen (CPS.PURE(P.UNBOX, [u], w, _, e), hp) = copy(INT, w, u, e, hp)
		| gen (CPS.PURE(P.WRAP kind, [u], w, _, e), hp) = (case kind
...
(* REAL32: FIXME *)
		    ...
		    (* end case *))
		| gen (CPS.PURE(P.UNWRAP kind, [u], w, _, e), hp) = (case kind
...
(* REAL32: FIXME *)
		...
		| gen (CPS.PURE(P.CAST,[u],w,_,e), hp) = copy(PTR, w, u, e, hp)
		| gen (CPS.PURE(P.GETCON,[u],w,t,e), hp) = select(0,u,w,t,e,hp)
		| gen (CPS.PURE(P.GETEXN,[u],w,t,e), hp) = select(0,u,w,t,e,hp)
		| gen (CPS.PURE(P.GETSEQDATA, [u], x, t, e), hp) = select(0,u,x,t,e,hp)
		| gen (CPS.PURE(P.RECSUBSCRIPT, [v, NUM{ty={tag=true, ...}, ival}], x, t, e), hp) =
		    select(IntInf.toInt ival, v, x, t, e, hp)
		| gen (CPS.PURE(P.RECSUBSCRIPT, [v, w], x, _, e), hp) = let
		  (* no indirection! *)
		    val mem = arrayRegion(getRegion v)
		    in
		      defTAGINT(x, M.LOAD(ity, scaleWord(regbind v, w), mem), e, hp)
		    end
		| gen (CPS.PURE(P.RAW64SUBSCRIPT, [v, i], x, _, e), hp) = let
		    val mem = arrayRegion(getRegion v)
		    in
		      treeifyDefF64(x, M.FLOAD(fty,scale8(regbind v, i), mem), e, hp)
		    end
		| gen (CPS.PURE(P.NEWARRAY0, [_], x, t, e), hp) = let
		...
		| gen (CPS.PURE(P.RAWRECORD NONE, [NUM{ty={tag=true, ...}, ival}], x, _, e), hp) =
		    (* allocate space for CPS spilling *)
		    treeifyAlloc(x, hp, e, hp + IntInf.toInt ival * ws) (* no tag! *)
		| gen (CPS.PURE(P.RAWRECORD(SOME rk), [NUM{ty={tag=true, ...}, ival}], x, _, e), hp) = let
		  (* allocate an uninitialized record with a tag *)
		    val (tag, fp) = (case rk (* tagged version *)
			   of (CPS.RK_FCONT | CPS.RK_RAW64BLOCK) => (D.tag_raw64, true)
			    | CPS.RK_RAWBLOCK => (D.tag_raw, false)
			    | CPS.RK_VECTOR => error "rawrecord VECTOR unsupported"
...
(* 64BIT: REAL32: FIXME *)
		...
		(*** ARITH ***)
		| gen (CPS.ARITH(P.IARITH{sz=sz, oper=P.INEG}, [v], x, _, e), hp) = (
		    updtHeapPtr hp;
		    if (sz <= Target.defaultIntSz)
		      then defTAGINT(x, M.SUBT(ity, two, regbind v), e, 0)
		      else defINT(x, M.SUBT(ity, zero, regbind v), e, 0))
		| gen (CPS.ARITH(P.IARITH{sz=sz, oper}, [v, w], x, _, e), hp) = (
		    updtHeapPtr hp;
		    if (sz <= Target.defaultIntSz)
		      then (case oper
			...
			(* end case *)))
		| gen (CPS.ARITH(P.TESTU{from, to}, [v], x, _, e), hp) =
		...
		| gen (CPS.ARITH(P.TEST{from, to}, [v], x, _, e), hp) =
		    if (from = to)
		      then copyM(from, x, v, e, hp)
		    else if (from = ity) andalso (to = Target.defaultIntSz)
		...
		| gen (CPS.ARITH(P.TEST_INF _, _, _, _, _), hp) =
		    error "gen:ARITH:TEST_INF"

		(*** LOOKER ***)
		| gen (CPS.LOOKER(P.DEREF, [v], x, _, e), hp) = let
		    val mem = arrayRegion(getRegion v)
		    in
		      defBoxed (x, M.LOAD(ity, regbind v, mem), e, hp)
		    end
		| gen (CPS.LOOKER(P.SUBSCRIPT, [v,w], x, _, e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		    val mem' = arrayRegion mem
 ...
 * sign extending the result.
 *)
		| gen (CPS.LOOKER(P.NUMSUBSCRIPT{kind=(P.UINT 8|P.INT 8)}, [v, i], x, _, e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		    val mem' = arrayRegion mem
...
(* REAL32: FIXME *)
		| gen (CPS.LOOKER(P.NUMSUBSCRIPT{kind=P.FLOAT 64}, [v,i], x, _, e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		    val mem' = arrayRegion mem
		...
		| gen (CPS.LOOKER(P.GETHDLR,[],x,_,e), hp) = defBoxed(x, C.exnptr(vfp), e, hp)
		| gen (CPS.LOOKER(P.GETVAR, [], x, _, e), hp) = defBoxed(x, C.varptr(vfp), e, hp)
		| gen (CPS.LOOKER(P.GETSPECIAL, [v], x, _, e), hp) =
		  (* special tag is in length field; we assume it is unsigned *)
		    defTAGINT(x, orTag(getObjLength v), e, hp)
		| gen (CPS.LOOKER(P.RAWLOAD{ kind }, [i], x, _, e), hp) =
		    rawload (kind, regbind i, x, e, hp)
		| gen (CPS.LOOKER(P.RAWLOAD{ kind }, [i,j], x, _, e), hp) =
		    rawload (kind, M.ADD(addrTy,regbind i, regbind j), x, e, hp)

		(*** SETTER ***)
		| gen (CPS.SETTER(P.RAWUPDATE(FLTt 64),[v,i,w],e),hp) = (
(* REAL32: FIXME *)
		    emit(M.FSTORE(fty, scale8(regbind' v, i), fregbind w,R.memory));
		    gen(e, hp))
		| gen (CPS.SETTER(P.RAWUPDATE _, [v,i,w], e), hp) = (
		    emit(M.STORE(ity, scaleWord(regbind' v, i), regbind' w, R.memory));
		    gen(e, hp))
		| gen (CPS.SETTER(P.ASSIGN, [a as VAR arr, v], e), hp) = let
		    val ea = regbind a
		...
		| gen (CPS.SETTER(P.UNBOXEDASSIGN, [a, v], e), hp) = let
		    val mem = arrayRegion(getRegion a)
		    in
		      emit(M.STORE(ity, regbind a, regbind v, mem));
		      gen(e, hp)
		    end
		| gen (CPS.SETTER(P.UPDATE, [v,i,w], e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		...
		| gen (CPS.SETTER(P.UNBOXEDUPDATE, [v, i, w], e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		    val mem' = arrayRegion mem
 ...
 * sign extending the result.
 *)
		| gen (CPS.SETTER(P.NUMUPDATE{kind=(P.UINT 8|P.INT 8)}, [s,i,v], e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		...
		| gen (CPS.SETTER(P.NUMUPDATE{kind=P.FLOAT 64},[v,i,w],e), hp) = let
		  (* get data pointer *)
		    val mem  = dataptrRegion v
		    val a    = markPTR(M.LOAD(ity, regbind v, mem))
		    val mem' = arrayRegion mem
		...
		| gen (CPS.SETTER(P.SETSPECIAL, [v, i], e), hp) = let
		...
		| gen (CPS.SETTER(P.SETHDLR,[x],e), hp) =
		    (emit(assign(C.exnptr(vfp), regbind x)); gen(e, hp))
		| gen (CPS.SETTER(P.SETVAR,[x],e), hp) =
		    (emit(assign(C.varptr(vfp), regbind x)); gen(e, hp))
		| gen (CPS.SETTER (P.RAWSTORE { kind }, [i, x], e), hp) = (
		    rawstore (kind, regbind i, x); gen (e, hp))
		| gen (CPS.SETTER (P.RAWSTORE { kind }, [i, j, x], e), hp) = (
		    rawstore (kind, M.ADD(addrTy, regbind i, regbind j), x);
		    gen (e, hp))

		| gen (CPS.RCC(arg as (_, _, _, _, wtl, e)), hp) = let
...
(* this case should never arise, since CPS contraction handles it? *)
		| gen (CPS.BRANCH(P.CMP{oper, kind}, [NUM v, NUM k], _, e, d), hp) =
raise Fail "unexpected constant branch"
		| gen (CPS.BRANCH(P.CMP{oper, kind=P.INT _}, vw, p, e, d), hp) =
		    branch(p, signedCmp oper, vw, e, d, hp)
		| gen (CPS.BRANCH(P.CMP{oper, kind=P.UINT _}, vw, p, e, d), hp) =
		    branch(p, unsignedCmp oper, vw, e, d, hp)
		| gen (CPS.BRANCH(P.FCMP{oper, size}, [v,w], p, d, e), hp) = let
		    val trueLab = newLabel ()
...
(* REAL32: FIXME *)
		| gen (CPS.BRANCH(P.FSGN 64, [v], p, d, e), hp) = let
		    val trueLab = newLabel ()
		    val r = fregbind v
		    val r' = newReg INT
		...
		| gen (CPS.BRANCH(P.BOXED, [x], p, a, b), hp) = branchOnBoxed(p, x, a, b, hp)
		| gen (CPS.BRANCH(P.UNBOXED, [x], p, a, b), hp) = branchOnBoxed(p, x, b, a, hp)
		| gen (CPS.BRANCH(P.PEQL, vw, p, e, d), hp) = branch(p, M.EQ, vw, e, d, hp)
		| gen (CPS.BRANCH(P.PNEQ, vw, p, e, d), hp) = branch(p, M.NE, vw, e, d, hp)
		| gen (CPS.BRANCH(P.STRNEQ, [NUM{ty={tag=true, ...}, ival},v,w], p, d, e), hp) =
		    branchStreq(ival, v, w, e, d, hp)
		| gen (CPS.BRANCH(P.STREQL, [NUM{ty={tag=true, ...}, ival},v,w],p,d,e), hp) =
