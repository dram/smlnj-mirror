...
(* QUESTION: should this constant be 1024*wordSz? *)
    ...
    *)
   fun pack(emit, gcroots, boxed, int, float) =
   let (*
	...
	* available for communicating to the collector.
        ...
        *)
       val (gcroots, boxed) =
           case (gcroots, int, float, boxed) of
             ([], [], [], []) => ([], []) (* it's okay *)
   ...
   let val {boxed, int, float} = split(regfmls, regtys, [], [], [])
   in  emitCallGC{stream=stream, known=true,
                  boxed=boxed, int=int, float=float, ret=ret}
   end
    ...
    *)
   fun rootSetToString{boxed, int, float} =
   ...
   let val {known, optimized, boxed, int, float, regfmls, ret, lab} =
           case gcInfo of
             GCINFO info => info
   ...
   in  if externalEntry then entryLabel (!lab) else defineLabel (!lab);
       ...
       if optimized then
            (annotation(#create MLRiscAnnotations.GCSAFEPOINT
               (if !debug then
                 rootSetToString{boxed=boxed, int=int, float=float}
                else ""
               ));
             emit ret
            )
   ...
   let (* GC code can be shared if the calling convention is the same
       ...
       fun longJumps(MODULE{addrs=ref [],...}) = ()
         | longJumps(MODULE{info=GCINFO{lab,boxed,int,float,...}, addrs}) =
           let val regRoots  = map T.GPR (int @ boxed)
