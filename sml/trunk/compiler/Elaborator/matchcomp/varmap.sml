...
fun makemap (VARS{svar,vars,asvars,...}, map) =
    let val map1 = foldl (fn ((var, rule),map) => M.insert(map,rule,(var,svar)::M.lookup(map,rule)))
			 map vars
	val map2 = foldl (fn ((var, rule),map) => M.insert(map,rule,(var,svar)::M.lookup(map,rule)))
			 map1 asvars
    in map2
    end
...
val lookup : rulemap * ruleno -> (V.var * SV.svar) list = M.lookup
