...
#define CHK_RETURN(sts)	{		\
    if (sts == -1) raise_syserror(MLState, 0);	\
 ...
 */
static void back_up_kont (MLState)
     MLState_ptr MLState;
{
 ...
 * errno.
 */
void raise_syserror (MLState, alt_msg)
    MLState_ptr MLState;
    char	    *alt_msg;
{
    ...
    if (alt_msg != 0) {
	s = ML_alloc_string (MLState,alt_msg);
	errno = -1;
    }
    else if ((0 <= errno) && (errno < sys_nerr))
	s = ML_alloc_string (MLState,sys_errlist[errno]);
    else {
	char		buf[32];
	sprintf(buf, "<unknown error %d>", errno);
	s = ML_alloc_string (MLState,buf);
    }

    REC_ALLOC2 (arg, INT_CtoML(errno), s);
    REC_ALLOC2 (exn, arg, PTR_CtoML(syserror_id0+1));
    MLState->fault_exn = exn;

    raise_ml_exn (MLState);
}
    ...
    ML_val_t	    arg;
{
...
#if (MAX_PROCS > 1)
    if (code == 1) /* exit */ {
      mp_shutdown(MLState,INT_MLtoC(ML_hd(p)));
    }
#endif
	...
	switch (i) {
	  ...
	  default:
	    raise_syserror (MLState,"ml_syscall: too many args");
	    return;
	}
    else {
      /* a SIGSYS occurred (because of a bad syscall) */
	sigsetmask (0);  /* re-enable signals */
	raise_syserror (MLState, "bad syscall");
	return;
    }
 ...
 */
void ml_connect_unix (MLState,arg)
    MLState_ptr MLState;
    ML_val_t	    arg;
{
 ...
 */
void ml_connect_inet (MLState,arg)
    MLState_ptr MLState;
    ML_val_t	    arg;
{
    ...
    raise_syserror(MLState,0);
#else
    raise_syserror(MLState,"unimplemented");
#endif
 ...
 */
void ml_wait_for_in (MLState,arg)
    MLState_ptr MLState;
    ML_val_t	    arg;
{
    ...
    if (MLState->inSigHandler
    || ((! _setjmp (MLState->SysCallEnv)) && 
	(((MLState->ioWaitFlag = 1), (MLState->NumPendingSigs == 0))))) {
#ifdef RISCos
 ...
 */
void ml_wait_for_out (MLState,arg)
    MLState_ptr MLState;
    ML_val_t	    arg;
{
    int		    fd = INT_MLtoC(arg), sts;
 ...
 */
static int write_multiple (MLState, fd, iov, iovcnt, nbytes)
    MLState_ptr MLState;
    int		    fd;
    struct iovec    *iov;
    int		    iovcnt, nbytes;
{
	...
	while (skip > 0) {
		...
		if ((nbytes -= (iov->iov_len - skip)) == 0)
		    return iovcnt;
		else if (MLState->NumPendingSigs > 0)
		    return ((iovcnt - i) + 1);
	    }
	    i--;  iov++;
	}
    ...
    ML_val_t	    arg;
{
	...
	if ((++i == WRITEVEC_SZ) || (p == ML_nil)) {
	    if ((n = write_multiple(MLState,fd, vec, i, nbytes)) < 0)
		return; /* error */
	    else if (n < i) {
 ...
 */
static void ml_getdirent (MLState, arg)
    MLState_ptr     MLState;
    ML_val_t	    arg;
{
	...
	if ((nbytes = READDIR(fd, buf, DIRBLKSIZ)) == -1) {
		...
		if (dp->d_name[0] != 0) {
		    s = ML_alloc_string (MLState,dp->d_name);
		    l = ML_cons (s, l);
		}
	    } /* end of for */
	}
    ...
    ML_val_t	    arg;
{
	...
	ML_val_t	path;
	buf[n] = '\0';
	path = ML_alloc_string(MLState,buf);
	RETURN (path);
    }
    ...
    ML_val_t	    arg;
{
    ...
    for (;  p != ML_nil;  p = ML_tl(p)) {
	  ...
	  default:
	    raise_syserror (MLState,"unknown access mode");
	    return;
	} /* end of switch */
    } /* end of for */
 ...
 */
static int stat_file (MLState, f, buf)
    MLState_ptr     MLState;
    ML_val_t	    f;
    struct stat	    *buf;
{
...
/* ml_getwd : unit -> string
 * Return the current working directory
 */
void ml_getwd (MLState, arg)
     MLState_ptr MLState;
     ML_val_t    arg;
{
  char           buf[MAXPATHLEN+1], *getwd();
    ...
    ML_val_t	    f;
{
	...
	switch (buf.st_mode & S_IFMT) {
	  ...
	  default:
	    raise_syserror(MLState,"unknown file type");
	    return;
	}
	RETURN(typ);
    }
 ...
 */
static ML_val_t fd_set2list (MLState, fds, width)
    MLState_ptr     MLState;
    register fd_set *fds;
    register int    width;
{
    ...
    ML_val_t	    arg;
{
	...
	if (sts == 0)
	    ...
	    rfdl = fd_set2list (MLState, rfds, width);
	    wfdl = fd_set2list (MLState, wfds, width);
	    efdl = fd_set2list (MLState, efds, width);
	}
 ...
 */
void ml_argv (MLState)
    MLState_ptr     MLState;
{
    extern char **global_argv;

    RETURN (make_str_list (MLState,global_argv));
 ...
 */
void ml_environ (MLState)
    MLState_ptr     MLState;
{
    extern char **environ;

    RETURN (make_str_list (MLState,environ));
    ...
    MLState_ptr     MLState;
{
	...
	name = ML_alloc_string(MLState,buf);
	RETURN (name);
    }
    else
	raise_syserror (MLState,0);
 ...
 */
void ml_blast_out (MLState, arg)
    MLState_ptr     MLState;
    ML_val_t	    arg;
{
    blast_fd	    = REC_SELINT(arg, 0);
    MLState->ml_arg = REC_SEL(arg, 1);
    callgc0 (MLState, CAUSE_BLAST, 0, CONT_ARGS_MASK);

} /* end of ml_blast_out */

/* blast_write:
 */
void blast_write (MLState, start, end, ptr)
    MLState_ptr MLState;
    int		start, end, ptr;
{
    int		hdr[3];
    ...
    ML_val_t	    arg;
{
  ...
  /* shed the unecessary stuff */
    callgc0(MLState, CAUSE_EXPORT, 0, CONT_ARGS_MASK);
    callgc0(MLState, CAUSE_EXPORT, 0, CONT_ARGS_MASK);
...
#endif
    isExported = 1;
    Exporters_State = MLState;
    i=export (fd);
    isExported = 0;
    Exporters_State = (MLState_ptr)0;
    if (i) raise_syserror (MLState, 0);
 ...
 * on most machines.
 */
void ml_flush_icache (MLState, s)
    MLState_ptr     MLState;
    ML_val_t		s;
{
    ...
    int		    level;
{
    ...
    switch (INT_MLtoC(level)) {
      case 0: callgc0 (MLState, CAUSE_MINOR, 0, CONT_ARGS_MASK); break;
      default: callgc0 (MLState, CAUSE_MAJOR, 0, CONT_ARGS_MASK); break;
    }
...
/* ml_masked : unit -> bool
 * Tell whether signals are masked or not
 */
void ml_masked (MLState, arg)
    MLState_ptr     MLState;
    ML_val_t	    arg;
{
    if (MLState->maskSignals == 0) {
      RETURN(ML_false);
    } else {
      RETURN(ML_true);
    }
 ...
 */
void ml_getstorelist (MLState, arg)
    MLState_ptr     MLState;
    ML_val_t	    arg;
{
    int		   i;
    ...
    callgc0 (MLState, CAUSE_STORE, 0, CONT_ARGS_MASK);
...
void ml_geticount (MLState)
    MLState_ptr     MLState;
{   int x;
    ML_val_t		res;
#ifdef ICOUNT
    x=(int)(MLState->ml_icount);
    MLState->ml_icount=0;
#else
    x=0;
#endif
    REC_ALLOC2(res,INT_CtoML(icountM), INT_CtoML(x));
    icountM=0;
    RETURN(res);
...
/*                                  "xxxxxxxxxxxxxxxx" MAX NAME LENGTH (16) */
	...
	FUNCTION (ml_masked,	    "masked"),
	FUNCTION (ml_geticount,     "geticount"),
	FUNCTION (ml_getwd,         "getwd"),
