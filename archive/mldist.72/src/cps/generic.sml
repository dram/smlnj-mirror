...
(************* old versions of standard formals 69 ***********************
...
let 
    ...
    val targetinglevel = if (k=0) then 0 else 1       (* temporarily used *)
    val calleesaveregs = fromto(3,k+2)
    fun standardformals args = 
       let val t = length(args)
        in if t = (k+3) then ([GPR 0,GPR 1,GPR 2]@calleesaveregs)
           else if t = (k+1) then ((GPR 1)::calleesaveregs)
                else if ((t = 2) andalso (k = 0)) then [GPR 2,GPR 1]
 ...
 *)
    ...
    val ss : int ref = ref(targetinglevel)  (* global var *)
    ...
    and getroot(APP(VAR f,wl)) = (f,mix(wl,standardformals wl))
      | getroot(APP(LABEL f,wl)) = 
         (case know f 
            of KNOWN((vl,e),ref(SOME fmls)) => (f,mix(wl@(map VAR vl),fmls@fmls))
	     | KNOWN((vl,e),ref NONE) => 
    ...
    and root(RECORD(_,_,e)) = root e
      | root(SELECT(_,_,_,e)) = root e
      | root(OFFSET(_,_,_,e)) = root e
      | root(SWITCH(_,el)) = fold union2 (map root el) (0,nil)
      | root(PRIMOP(_,_,_,el)) = fold union2 (map root el) (0,nil)
	...
	let  val fp_ops = [P.fadd, P.fdiv, P.feql, P.fge, P.fgt, 
	    ...
	    fun fp_use(PRIMOP(P.updatef,[_,_,VAR x'],_,_)) = x'=x
	      | fp_use(PRIMOP(p,vl,_,_)) = 
		is_fp_op p andalso exists (fn VAR x' => x=x'| _ => false) vl
	    fun f (cexp,level) =
	        case cexp
		  of RECORD(_,_,ce) => f(ce,level+1)
		   ...
		   | PRIMOP(_,_,_,cl) => if fp_use cexp then SOME level
	...
	in if v=f then get(default,standardformals([GPR 0,GPR 0,GPR 0]@calleesaveregs))
	   else find tinfo
	end
	  ...
	  alloc(x,any, fn x' => tempreg(x', fn x'' =>
       ...
       | PRIMOP(P.getvar,[],[x],[
           e0 as PRIMOP(P.unboxedupdate, [VAR x',i,v], [], [e])]) =>
            if varptr_indexable andalso
	         x=x' andalso not (SortedList.member (cexp_freevars e) x)
               then (storeindexl(gpregbind v, varptr, gpregbind i);
		     gen e)
               else alloc(x,any, fn x' => (move(varptr,x'); gen e0))
       | PRIMOP(P.getvar,[],[x],[
       ...
       | PRIMOP(P.setvar, [x],[],[e]) => (move(gpregbind x, varptr); gen e)
       | PRIMOP(P.uselvar, [x],[],[e]) => gen e
       | PRIMOP(P.deflvar, [],[x],[e]) => alloc(x,any, fn x' => gen e)
