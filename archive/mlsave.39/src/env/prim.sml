...
struct
   ...
   open Access Basics BasicTypes Env
   val namespaces = 7  (* also defined in Env *)

   val symbol = Symbol.symbol
   fun number(s: string) = Symbol.number(Symbol.symbol s)
...
(* primTypes structure *)
   ...
   fun entercon(s: string, c: datacon) =
       IntStrMap.add typesTable
         (namespaces*(number s)+0,s,CONbind c) (* varIndex(symbol s) *)

   fun entertyc(s : string, t : tycon ref) =
       IntStrMap.add typesTable 
         (namespaces*(number s)+1,s,TYCbind t) (* tycIndex(symbol s) *)
   ...
   val primTypes = (
	...
	entercon("::", consDcon);
	IntStrMap.add typesTable
	  (namespaces*(number "::")+6,  (* fixIndex(number "::") *)
	...
	mkSTR([symbol "PrimTypes"], typesTable, emptyStrenv,Stamp.global))
...
(* inLine structure *)
   ...
   fun enter( s : string, p : primop ) =
       let val name = symbol s
        in IntStrMap.add inLineTable
              (namespaces*(Symbol.number name)+0,   (* varIndex *)
   ...
   val inLine =
       (app enter primopNames;
        mkSTR([symbol "InLine"], inLineTable, emptyStrenv, Stamp.global))
