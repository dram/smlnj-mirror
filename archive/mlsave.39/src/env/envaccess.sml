...
val bogusStrStamp = Stamp.newStr(Stamp.global)

local val b = STRstr{stamp=bogusStrStamp, sign=0, table=newTable(), env=emptyStrenv,
...
val bogusTyc = mkDEFtyc([bogusID],TYFUN{arity=0,body=ERRORty},YES,Stamp.global)
...
fun lookTYC' look (id:symbol) =
    case look(tycKey(id))
      of (TYCbind(tycref as ref(INDtyc i)), {strenv={t,s},path}) =>
       ...
       | _ => impossible "EnvAccess.lookTYC' 2"
...
fun lookArTYCinSig (depth: int) (id: symbol, arity: int) =
    (case look(tycKey id)
      of (TYCbind(tycref as ref(INDtyc i)), {strenv={t,s},path=h::r}) =>
	   if h >= 0
	   then let val tyc = t sub i
		 ...
		 ref(RELtyc(if depth+h = 0 then [i]
			    else addzeros(depth+h,r@[i]))))
...
fun lookPathArTYCinSig (depth: int) (spath as first::rest, arity) : tycon ref =
	...
	val leadStr = lookSTR0 first
     ...
     in case leadStr
	  of (STRvar{binding=INDstr i,...},{path=h::r,strenv={s,...}}) =>
...
fun iterFct (gen : unit -> binder * info)
    ...
    let val newtable = newTable()
	val strComps = array(maxStrComps,NULLstr)
	and tycComps = array(maxTypeComps,NULLtyc)
	and strCount = ref 1
	and tycCount = ref 0
	fun fill (count) =
	     (case gen()
		of ((i,n,STRbind(STRvar{name,access,binding})),
		    ...
		    {path,strenv}) =>
		     (IntStrMap.add newtable
		       (i,n,VARbind(case access
				     of INLINE _ => 
					  var  (* ground type, primitive tycons *)
				      | _ => let val typ =
						     typeInContext(!vtype,strenv)
					      in VALvar{access=SLOT count,
							name=name,
							vtype=ref typ}
					     end));
		      VALtrans(extendPath(access,path))::fill(count+1))
		 ...
		 | (binding,_) => (IntStrMap.add newtable binding; fill(count)))
	    handle BinSort.Finished => []
     in (fill(0) handle Bind => (print "Bind in iterFct.fill\n"; raise Bind), newtable,
...
(* relative: int list * int list -> int list 
   relative(x,y)
     x is current structure path
     y is tycon access path from root
     result is relative access path to tycon from current structure
*)

fun backout([],m) = m
  | backout(_::r,m) = backout(r,0::m)

fun relative((i:int)::l,y as j::m) =
    if i = j
    then relative(l,m)
    else backout(l,0::y)
  | relative([],y) = y
...
(* abstraction of functor body structure; relativizing bound type constructors *)

fun abstractBody(str as STRstr{stamp,sign,table,env as {s,t},kind},tycTotal: int)
      : Structure =
    let val stampmap = array(tycTotal, []:int list)
	val tenvsize0 = Array.length t
	val tenvsize = ref(tenvsize0)
	val extraTycons = ref([]: tycon list)
	val FCTstr(STRstr{env=penv,...}) = s sub 0  (* penv = parameter strenv *)
	fun initStampMap() =
	    let fun esearch({s,t},path) =
		    let fun tsearch i =
			    let val stamp = tycStamp(t sub i)
			     in if Stamp.bound stamp andalso
				   null(stampmap sub stamp)
				then update(stampmap,stamp,rev(i::path))
				else ();
				tsearch(i+1)
			    end
			fun ssearch i =
			    (case s sub i
			      of STRstr{env,stamp,...} =>
				   if Stamp.bound stamp
				   then esearch(env, i::path)
				   else ()
			       | _ => ErrorMsg.impossible "abstractBody 1";
			     ssearch(i+1))
		     in tsearch 0
			handle Subscript =>
			  ssearch 1 (* substructures start at index 1 *)
			  handle Subscript => ()
		    end
	     in esearch(penv,[1,0]); esearch(env,[])
	    end

	fun abstractType(path,ty) =
	    let fun absty ty =
		    case ty
			  ...
			  (case tyc
			     of RELtyc _ => () (* already been done *)
			      | TYCON{stamp,kind,...} =>
				  if Stamp.fixed stamp then ()
				  else (if null(stampmap sub stamp) (* new bound tycon *)
					then (update(stampmap,stamp,[!tenvsize]);
					      inc tenvsize;
					      extraTycons := tyc :: !extraTycons;
					      case kind
					       of DEFtyc(TYFUN{body,...}) =>
						    abstractType([],body)
						| _ => ())
					else ();
					tycref :=
					  RELtyc(relative(path,stampmap sub stamp)));
			   app absty args)
		       | POLYty{tyfun=TYFUN{body,...},...} => absty body
		       | VARty _ => ()
		       | _ => impossible "abstractType"
	     in absty ty
	    end

	fun abstractBinding (p: path) ({s,t}: strenv) binding =
	    case binding
	     of VARbind(VALvar{vtype=ref typ,...}) => abstractType(p,typ)
	      | CONbind(DATACON{typ,...}) => abstractType(p,typ)
	      | TYCbind(ref(INDtyc i)) =>
		 (case t sub i
		   of TYCON{stamp,kind=DEFtyc(TYFUN{body,...}),...} =>
			if Stamp.bound stamp then abstractType(p,body) else ()
		    | _ => ())
	      | STRbind(STRvar{binding=INDstr i,...}) =>
		 let val str as STRstr{stamp,env,table,...} = s sub i
		  in if Stamp.bound stamp
		     then abstractStr(p@[i],env,table)
		     else ()
		 end
	      | _ => ()

	and abstractStr(p: path, env: strenv, table: symtable) =
	    let val absbind = abstractBinding p env
	     in IntStrMap.app (fn (_,_,b) => absbind b) table
	    end

     in initStampMap();
	abstractStr([],env,table);
	if !tenvsize = tenvsize0  (* no extra tycons found *)
	then str (* return modified argument *)
	else let val newtenv = array(!tenvsize,NULLtyc)
	         fun fillold i = (update(newtenv,i,t sub i); fillold(i+1))
		 fun fillnew [] = ()
		   | fillnew(tyc::rest) =
		       (dec tenvsize; update(newtenv,!tenvsize,tyc); fillnew rest)
	      in fillold 0 handle Subscript => fillnew(!extraTycons);
		 STRstr{stamp=stamp,table=table,sign=sign,env={s=s,t=newtenv},kind=kind}
		 (* return new structure with extended type environment *)
	     end
    end
