...
structure Basics : BASICS = struct
  ...
  (* building tycons, signatures, structures, and functors *)
    ...
    fun mkABStyc(path: symbol list, arity: int, eq: bool3, stamps: Stamp.state)
	 : tycon =
	TYCON{stamp = Stamp.newTyc(stamps), path = path, arity = arity,
	      eq = ref eq, kind = ABStyc}
    
    fun mkDEFtyc(path: symbol list, def as TYFUN{arity,...}: tyfun, eq: bool3,
	         stamps: Stamp.state)
        : tycon =
	TYCON{stamp = Stamp.newTyc(stamps), path = path, arity = arity, eq = ref eq,
	      kind = DEFtyc def}
    ...
    fun mkSTR(path, table, env, stamps: Stamp.state) = 
        STRstr{stamp = Stamp.newStr(stamps),
               sign = 0,  (* guaranteed not to agree with any valid sig stamp *)
	       table = table,
	       env = env,
	       kind = STRkind{path=path}}

 (* these definitions are redundant -- functions in Stamp should replace them *)

    type stampInfo = int * int
      (* first unused free (generative) type and structure stamp *)

    fun currentInfo() = Stamp.current(Stamp.global)

    fun restoreInfo(nextTypeStamp,nextStrStamp): unit =
	Stamp.set(Stamp.global, nextTypeStamp, nextStrStamp)
