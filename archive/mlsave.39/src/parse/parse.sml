    ...
    case !nextToken
     of FN => (advance(); FNexp(match()))
      | CASE => CASEexp((advance(); exp()),
			(checkToken(OF); match()))
      | WHILE => WHILEexp((advance(); exp()), (checkToken(DO); markexp exp()))
      | IF => IFexp((advance(); exp()), (checkToken(THEN); markexp exp()),
       	            (checkToken(ELSE); markexp exp()))
...
and exp1 () = leftAssoc(markexp exp2,ORELSE,ORELSEexp,(fn x=>x))

and exp2 () = leftAssoc(markexp exp3,ANDALSO,ANDALSOexp,(fn x=>x))
...
and aexp () =
	...
	| HASH => (advance(); selector())
	    ...
	    protect(protectScope,
		    (fn()=>(advance();
		     	   (LETexp(ldecs([],Stamp.global),
			           (checkToken(IN); SEQexp(exp_ps()))))
			   before checkToken(END))))
			      (* using Stamp.global here is a bug.  Potential
				 problems in functor bodies.  This shortcut
				 avoids the need to pass the stamps parameter
				 through all expression functions. [dbm] *)
        | FN =>   exp()
        | CASE => exp()
        | WHILE => exp()
        | IF => exp()
...
fun sigbody(depth: int, stamps: Stamp.state) : Structure = 
	...
	(* dumpSig used to implement include specs *)
	fun dumpSig(sgn as STRstr{kind=SIGkind{stampcounts,...},...}) =
	    let val tBase = !tCount
		and sBase = !sCount
		val STRstr{table,env={s=senv,t=tenv},...} =
		      SigMatch.newsig(stamps,sgn)

		fun adjust(CONty(ref(RELtyc(p)),args)) =
		      let val newp = case p
				       of [i] => [i+tBase]
					| i::r => (i+sBase)::r
					| _ => impossible "Parse.adjust"
		       in CONty(ref(RELtyc(newp)), map adjust args)
		      end
		...
		sCount := !sCount + Array.length senv - 1 (* adjust for parent *)
	    end
	...
	and strspec1() =
		...
		bindSTR(name,STRvar{name=name,access=LVAR(0),
				    binding= sNext(sgn(depth+1,stamps))})
	    end
	...
	val result =
	      STRstr{stamp=stamp,
		     ...
		     kind=SIGkind{share=shareSpec,
				  bindings=bindings,
				  stampcounts=Stamp.current(stamps)}}
     in ArrayExt.app((SigMatch.setParent result),senv,1);
	result
    end (* fun sigbody *)

and sgn(depth: int, stamps: Stamp.state) : Structure = 
    case !nextToken
      of ID s =>
	    let val name = s before advance()
		val SIGvar{binding,...} = lookSIG(name)
	     in SigMatch.newsig(stamps, binding)
	    end
       | Token.SIG => (advance(); sigbody(depth,stamps) before checkToken(END))
...
fun str(abs: bool, constraint: Structure option, path: spath, context: strContext,
	stamps: Stamp.state, parent: Structure) : strexp * Structure * thinning =
    case !nextToken
      of IDDOT _ =>
	   let val strVar as STRvar{binding,...} = qid(lookSTRinStr)
	    in case constraint
		 of NONE => (VARstr strVar, binding, NONE)
		  | SOME sgn =>
		      let val (str,thin) = SigMatch.match(abs,path,stamps,sgn,binding)
	   ...
	   (advance();
	    let val _ = openStr()
		val body = sdecs(path,context,stamps)
	     in (case constraint
		   of NONE =>
		       (case context
			  of FCTBODY =>
			      let val (thin,table,env) = buildFctTable ()
				  val bodystr = mkSTR(path,table,env,stamps)
			       in SigMatch.linkParents bodystr;
				  SigMatch.setParent parent bodystr;
			   ...
			   | REGULAR =>
			      let val (thin,table) = buildStrTable ()
				   ...
				   mkSTR(path,table,emptyStrenv,stamps),
				   NONE)
			      end)
		    | SOME sgn => 
		       let val (str,thin) =
				 SigMatch.realize(abs,path,stamps,
				 		  Stamp.newStr(stamps),sgn,parent)
			in closeStr();
	       ...
	       then let val fctVar as FCTvar{binding=fct,...} = lookFCT id
			val (argexp,argstr) =
			     ...
			     (if !nextToken = RPAREN
			      ...
			      then let val _ = openStr()
				       val body = sdecs([anonParamName],context,stamps)
					...
					mkSTR([anonParamName],table,
				       ...
				       val (strexp,str,_) =
					     str(false,NONE,[paramName],
					         context,stamps,NULLstr)
				    in (strexp,str)
				   end)
			     before checkToken(RPAREN)
			val (result,thin1) = 
			      SigMatch.applyFunctor(fct,argstr,path,stamps)
			val strexp = APPstr{oper=fctVar,
			  ...
			  of NONE => (strexp,result,NONE)
			   | SOME sgn =>
			       let val (thinned,thin2) =
				       SigMatch.match(abs,path,stamps,sgn,result)
			 ...
			 of NONE => (VARstr strVar, binding, NONE)
			  | SOME sgn =>
			     let val (str,thin) =
				     SigMatch.match(abs,path,stamps,sgn,binding)
			      in (VARstr strVar, str, thin)
			     end
		    end
	   end
...
and sdecs(args as (path: spath, context: strContext, stamps: Stamp.state)) : dec list =
    let fun sdec() : dec =
	    if at(STRUCTURE)
	      then STRdec(strb(false,path,context,stamps))
	    else if at(ABSTRACTION)
	      then ABSdec(strb(true,path,context,stamps))
...
and strb(abstract: bool, path: spath, context: strContext, stamps: Stamp.state) =
    let fun strb1() =
	let val name = ident()
	    val constraint = 
		  if abstract
	    ...
	    val _ = checkToken(EQUAL) 
	    val (strexp,str,thin) =
		  str(abstract,constraint,name::path,context,stamps,NULLstr)
...
and fctb1() =
	...
	val stamps = Stamp.new(0,0)
	val (pname,paccess,pstr,spreadParams) =
	     (checkToken(LPAREN);
	      (case !nextToken
		of RPAREN => (anonParamName,LVAR(namedLvar(anonParamName)),nullSig,
	         ...
	         | ID s => let val tenv = array(0, NULLtyc)
			       ...
			       val _ = openNew{path=[~1], strenv={t=tenv,s=senv}}
			       val name = ident()
			       val access = LVAR(namedLvar(name))
			       val _ = checkToken(COLON)
			       val pstr = sgn(2,stamps)
			    in update(senv,1,pstr);
			       bindSTR(name,STRvar{name=name,
					   	   access=access,
						   binding=INDstr(1)});
			       (name,access,pstr,false)
			   end
		 | tok => if firstSpec(tok)
			  then let val plvar = namedLvar(anonParamName)
				   val pstr as STRstr{env,table,...} =
				         sigbody(2,stamps)
				in openOld({path=[~1,1],strenv=env},table);
				   (anonParamName,LVAR(plvar),pstr,true)
			       end
			  else condemn ("expected functor parameter spec, found "
				       ^tokenName tok))
	      before checkToken(RPAREN))
	val STRstr{kind=SIGkind{stampcounts,...},...} = pstr
	val fctstr = FCTstr pstr
	val resSign = 
	    if !nextToken = COLON
	    then (advance();
		 let val res = sgn(2,Stamp.new(stampcounts))
		  in SigMatch.setParent fctstr res;
		     SOME res
		 end)
	    else NONE
	val _ = if spreadParams
		then let val STRstr{table,env,...} = pstr
        ...
        val _ = checkToken(EQUAL)
	val stamps = Stamp.new(stampcounts)
        val (strexp,body,thin) = str(false,resSign,[],FCTBODY,stamps,fctstr)
	val (tycTotal,_) = Stamp.current(stamps)
	val body = abstractBody(body,tycTotal)
	val paramvar = STRvar{name = pname, access = paccess, binding = pstr}
	val fctv = FCTvar{name=name, 
			  ...
			  binding=FUNCTOR{paramName=pname,
					  ...
					  tycCount=tycTotal, (* ??? *)
					  body=body}}
	val fb = FCTB{fctvar=fctv, param=paramvar, def=strexp, thin=thin,
		      constraint=resSign}
     in resetEnv(mEntry);
	(name,fctv,fb)
    end
...
fun tdec() =
    case !nextToken
      of SIGNATURE => (advance(); SIGdec(sigb()))
       ...
       | STRUCTURE =>
	  (debugmsg "tdec(Structure)"; advance();
           STRdec(strb(false,[],REGULAR,Stamp.global)))
       | ABSTRACTION => (advance(); ABSdec(strb(true,[],REGULAR,Stamp.global)))
       | Token.OPEN =>
	   let val strs = (advance(); qid_p())
	    in app openStructureVar strs;
	       OPENdec strs
...
fun interdec() =
	...
	(case !nextToken
	   of SIGNATURE => (advance(); SIGdec(sigb()))
	    ...
	    | STRUCTURE => (advance(); STRdec(strb(false,[],REGULAR,Stamp.global)))
	    | ABSTRACTION => (advance(); STRdec(strb(true,[],REGULAR,Stamp.global)))
            | IMPORT=>(advance(); raise Import(IMPORTdec(importdec())))
	    | EOF => raise Eof
	    | tok => let val dec =
			     if firstLdec(!nextToken)
			     then ldec([],Stamp.global)
