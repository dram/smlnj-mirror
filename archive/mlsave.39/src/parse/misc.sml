...
in
  ...
  fun getSymbol () = case !Lex.nextToken of
		     Token.ID s => (Lex.advance(); s)
                   | Token.ASTERISK => (Lex.advance(); ASTERISKsym)
		   | Token.EQUAL => (Lex.advance(); EQUALsym)
		   | Token.TYVAR s => (Lex.advance(); s)
		   | Token.IDDOT s => (Lex.advance(); s)
  ...
  (* structure context *)
  datatype strContext = REGULAR | FCTBODY
  ...
  val nullSig = 
      STRstr{stamp = 0,
	     ...
	     kind = SIGkind{share={s=[],t=[]},
			    ...
			    stampcounts=(0,1)}}
  val nullStr = 
      STRstr{stamp = Stamp.newStr(Stamp.global),
             sign = nullSigStamp,
	     table = newTable(),
	     env = emptyStrenv,
	     kind = STRkind{path=[]}}
