...
struct
 local open InLine Ref PreString PrimTypes in
  ...
  in
    fun mtime({filid, closed, ...}: instream): int =
      if !closed then
	raise Io "mtime: closed stream"
      else
	let val tim = mtim filid
	in  if InLine.ieql(tim, ~1) then raise Io "mtime" else tim
	end
  end
...
structure System : SYSTEM =
  struct
    ...
    open PreLim
    val version = "Standard ML of New Jersey, Version 0.39, 8 September 1989"
