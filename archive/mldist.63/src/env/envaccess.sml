...
fun lookTYCinTable(table,id) =
    let val TYCbind tyc = IntStrMap.map table (tycKey id)
     in tyc
    end
...
fun lookTYC' look (id:symbol) =
    case look(tycKey(id))
      of (TYCbind(INDtyc i), {strenv=REL{s,t},path}) =>
	    (t sub i  handle Subscript => impossible "EnvAccess.lookTYC' 1")
...
fun checkopen (env,v) =
  let fun check (i,s,_) =
	let val (binding,{path,strenv}) = lookEnv(env,(i,s))
