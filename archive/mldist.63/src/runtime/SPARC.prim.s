 ...
 *
 * %o0 = arith. temp
 * %o1,%g1-%g3,%l0-%l7,%i4 = misc. registers (contain only pointers or tagged ints)
 * %o5 = pointer temp.
 *
...
2:				      /* there are pending signals */
	FETCH	(_maskSignals, %o0)	/* check if signals are masked */
	tst	%o0
	bne	1b
	...
	bne	1b
	set	1,%o0			/* (delay slot) */
	STORE	(%o0, _handlerPending)	/* note that a handler trap is pending */
