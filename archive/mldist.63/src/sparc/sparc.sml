...
(*       fun emit_fneg args = C.emit (SparcInstr.I_fneg args) *)
	...
	fun mkLabExp (lab, n) = LABELexp{base= C.baseLab, dst= lab, offset= (n-4096)}
      ...
      fun setBaseAddr (contReg, lab, tmpR, addr) = C.emitSDI (
  ...
  (** Dedicated registers **)
    ...
    val miscregs = map (Direct o REG) [     (* %g1-%g3, %o1, %l0-%l7, %i4 *)
	  1, 2, 3, 9, 16, 17, 18, 19, 20, 21, 22, 23, 28
	  ]

    val dataptrR = REG 6                    (* %g6 *)
    val limitptrR = REG 4                   (* %g4 *)
    val limitptrRand = REGrand limitptrR
    val arithtempR = REG 8
    val arithtempRand = REGrand arithtempR
   ...
   *)
    fun beginStdFn (Direct closureReg, ImmedLab lab, addr) = let val tmpR = getTmpReg()
	  in
	    setBaseAddr (closureReg, lab, tmpR, addr);
	    freeTmpReg tmpR
	  end
