...
val anyreg = 3 (* the index of the first miscreg *)
...
local val okreg = array(length allregs, true)
 ...
 in exception Getscratch
    fun getscratch(preferred, prohibited) =
	    ...
	    (if okreg sub preferred then preferred 
	     else (find next handle Subscript => find2 0))
...
let 
    ...
    fun allocparams(args,formals) =
	   ...
	   fun f(already, VAR a :: ar, occupied::occupied', b::br) =
		...
		let val z = getscratch(anyreg, already@occupied)
		 in addbinding(b,z); 
...
(*		    align();   temporarily removed so 68020 will work. *)
	...
	alloc(w,any,  fn w' =>
	    (select(~1, regbind a, w');
	     ashr(immed(width_tags-1),w', w');
	     orb(immed 1, w', w');
	     gen e))
       | PRIMOP(P.slength, [a as VAR _], [w], [e]) =>
	  alloc(w,any, fn w' =>
	     (select(~1, regbind a, w');
	      ashr(immed(width_tags-1), w', w');
	      orb(immed 1, w', w');
