...
let 
  ...
  (* generate a new code label *)
    ...
    |   genlab(lab, cexp, SOME closureReg) =
        (root1 := root cexp; define lab; beginStdFn(closureReg, lab); gen cexp)
   ...
   root registers (for offset computations in "boot") *)
	 ...
	 in alloc(x,e,any, fn x' => 
		  (case (isimmed v', isimmed w') of
		     (SOME k,_) =>
          (ashr(immed 1,w',arithtemp); ashlt(arithtemp,immed(k-1), x'))
		   | (_,SOME k) => 
	  (addl3(immed(~1),v',arithtemp);
	   ashlt(immed(Bits.rshift(k,1)), arithtemp, x'))
		   | _ => 
	 ...
	 (fn ([v],[w],[e]) =>
	    alloc(w, e,any, fn w' =>
		(if !CGoptions.profile then profile(Profile.REFCELLS,2) else ();
		 record([(immed tag_suspension,OFFp 0),
			 (regbind v, OFFp 0)], w');
		 gen e)))
