 ...
 *
 *)
functor MDMC(Comp : MD_COMPILE) : MD_GEN_MODULE =
struct
   structure Comp = Comp
   ...
   let (* name of the functor and signature *)
       ...
       val widths = ListMergeSort.uniqueSort Int.compare (map #1 formats)
       ...
       val _ = app 
          (fn w =>
              if w < 8 orelse w > 32 orelse w mod 8 <> 0 then
                 error("instruction format must be 8, 16, 24, or 32 bits")
              else ()) widths
           ...
           let val DUMMYbind = FUNbind("dummy",[])
               ...
               let fun f 0 = []
                     ...
                     | f b = VAL("b"^itos b,ID "w")::
                                VAL("w",SLR(ID "w",WORDexp 0w8))::f(b - 8)
                   fun g 0 = []
       ...
       val cellFuns = 
           let fun mkEmitCell(CELLdecl{id, from, ...}) =
                   FUN'(emit id, IDpat "r", 
                      APP("itow", MINUS(APP("regmap", ID "r"),INTexp(!from))))
        ...
        *) 
       val datatypeFuns =   
           let fun WORD w = TYPEDexp(WORDexp w,WORD32ty)
               ...
               let fun missing() =
                   ...
                   fun loop(w, [], cs, found) = (w, rev cs, found)
                     | loop(w, (cb as CONSbind{id, ty, mc, ...})::cbs, 
                       ...
                       in loop(w, cbs, 
                               Env.consToClause 
                                 {prefix="I", pat=fn p=>p, cons=cb, exp=e}::cs,
                               found)
                       end
        ...
        *)
       val formatFuns = 
           let fun mkFormat(width, FORMATbind(formatName, fields, NONE)) =
                     mkDefinedFormat(width, formatName, fields)
                 | mkFormat(width, FORMATbind(formatName, fields, SOME e)) =
                   ...
                   in  if realWidth <> totalWidth then
                       ...
                       mkFormatFun(formatName, fields,   
                                   APP("eWord"^itos totalWidth,
                                       foldr PLUS (WORDexp constant) exps))
                   end
