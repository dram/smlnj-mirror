...
functor MDDelaySlots(Comp : MD_COMPILE) : MD_GEN_MODULE =
struct
   ...
   let (* Name of the functor and its signature *)
       ...
       val delaySlots = Comp.delayslots md

       fun mkFun(name,args,x,body,default) = 
           FUNdecl[FUNbind(name,
              [CLAUSE([RECORDpat(map (fn x => (x,IDpat x)) args, false)],
               LETexp([FUNdecl
                         [FUNbind(name,[CLAUSE([IDpat x],
                                        CASEexp(ID x,
                                            body @ [CLAUSE([WILDpat],default)]
                          ))])]],
                      [APPexp(ID name,ID x)]))
              ])]

       (* Size of the delay slot in bytes *)
       val delaySlotSize = VAL("delaySlotSize", INTexp(delaySlots))
           ...
           let fun mkPat cons = Env.consToPat {prefix="I",cons=cons}
               fun g [] = []
                 ...
                 | g((c as CONSbind{id,delayslot=(d1,d2),
                                    nop,nullified,...})::cbs) = 
                     CLAUSE([mkPat c],delaySlotEntry(flag nop, flag nullified,
       ...
       val delaySlotCandidate = 
           let fun g [] = []
                 ...
                 | g((c as CONSbind{delaycand=SOME e, ...})::cbs) = 
                     CLAUSE([Env.consToPat {prefix="I",cons=c}],e)::g cbs
