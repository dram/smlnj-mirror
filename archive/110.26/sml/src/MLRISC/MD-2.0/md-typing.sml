...
struct
   ...
   let val poly = ref false
       fun ty ==> (t as VARty(_,_,_,ref NONE)) = (poly := true; t)
         ...
         | ty ==> t = t
   in  #ty (Trans.rewrite{ty=ty,pat=Trans.noRewrite,decl=Trans.noRewrite,
                          exp=Trans.noRewrite}) t;
       !poly
   end
   ...
   val cellTy    = polyTy "cell"
   val operandTy = polyTy "operand"
   ...
   let val _   = TypeUtil.init()
       ...
       let val CELLdecl{bits=n, count, id, ...} = Comp.lookupCellKind md m
           fun log2 1 = 0
           ...
           val argTy = case count of  
                   ...
                   | NONE   => 
                      (case id of
                       ...
                       | _ => (error("$"^m^"["^e2s e^"] is illegal"); Env.var E)
                      )
            val _ = 
               if id = "MEM" then 
               (case region of 
                ...
                | NONE => error("region missing in memory reference "^e2s exp)
               ) else
               (case region of 
                  SOME _ => error("illegal region in "^e2s exp)
               | _ => ()
       ...
       and W E (IDexp id) = Env.lookupVal E id
         ...
         | W E (e as INTexp _)  = (e, intTy)
         | W E (e as WORDexp _) = (e, wordTy)
         | W E (e as BOOLexp _) = (e, boolTy)
         | W E (e as STRINGexp _) = (e, stringTy)
       ...
       and P E (IDpat id) = Pvar E id
         ...
         | P E (INTpat _) = (INTty, Env.empty)
         | P E (BOOLpat _) = (BOOLty, Env.empty)
         | P E (WORDpat _) = (WORD32ty, Env.empty)
         | P E (STRINGpat _) = (STRINGty, Env.empty)
       ...
       and D E (DATATYPEdecl(dbs,tbs)) = 
         ...
         | D E (d as OPENdecl ids) = 
           ...
           in  (d, E) end
         | D E (STRUCTUREdecl(id,args,sexp)) =
           let val (sexp, E') = SE E sexp
           in  (STRUCTUREdecl(id,args,sexp), Env.STRbind(id,args,E')) end
         ...
         | D _ d = (error("unknown semanatics declaration "^d2s d); 
                    (d, Env.empty))
