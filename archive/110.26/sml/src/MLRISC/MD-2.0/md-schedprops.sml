...
struct
   ...
   let val resources   = Comp.resources md
       ...
       val getCPU =
           FUNdecl[FUNbind("cpu", 
             map (fn CPUbind{name,aliases,...} => 
                 CLAUSE([ORpat(map STRINGpat (name::aliases))],ID name)) cpus
       ...
       val zeroRT = VAL("zeroRT",TUPLEexp(many(fn n => WORDexp 0w0)))
       ...
       let fun genRes{resource, count, word, bit} =   
               (resource, 
                  TUPLEexp(many(fn w => if w = word then 
                               WORDexp(Word32.<<(0w1,Word.fromInt bit)) else 
                               WORDexp 0w0)))
           ...
           val emptyRT = VAL("emptyRT"^cpu,
                           TUPLEexp(many(fn n => WORDexp(get(n,extra)))))
       ...
       let val check = checkResource("pipeline "^name)
           ...
           fun genClause(pat,PIPELINE pe) = 
               CLAUSE([resourcesPat,pat],LISTexp(trans pe,NONE))
       ...
       let fun genClause(pat,e) = CLAUSE([resourcesPat,pat],e)
