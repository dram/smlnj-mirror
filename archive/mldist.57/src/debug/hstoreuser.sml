...
struct
  local
    ...
    val (updatedAList: (object Array.array weak,int) plist ref,_,
	 hcreate: ((int * object) -> (object array))) = 
		  cast (!System.Control.Debug.interface 16)
  in
    open Array
    fun array(cnt,obj) = let val r = cast (hcreate(cnt,cast obj))
			 in (*print "hcreate "; print cnt; print "\n";*)
			    r
			 end
...
struct
  local
    open System.Unsafe Weak
    val (_,updatedRList: object Array.array weak list ref,_) =
		  cast (!System.Control.Debug.interface 16)
  in
    open Ref
