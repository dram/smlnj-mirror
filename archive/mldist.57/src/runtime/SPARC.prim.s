 ...
 *   08/02/90  revised for new run-time architecture
 *
 ...
 *
 * %o0,%o1 = arith. temps
 * %g1-%g3,%l0-%l7,%i4 = misc. registers (contain only pointers or tagged ints)
 * %o5 = pointer temp.
 *
...
_restoreregs:
	...
	set	1,%o0			/* note that we have entered ML code */
	STORE	(%o0, _inML)
	FETCH	(_NumPendingSigs, %o0)	/* check for pending signals */
	tst	%o0
	bne	2f
	nop
1:
...
2:				      /* there are pending signals */
	FETCH	(_inSigHandler, %o0)
	tst	%o0			/* check if we are currently handling a signal */
	bne	1b
	set	1,%o0			/* (delay slot) */
	STORE	(%o1, _handlerPending)	/* note that a handler trap is pending */
