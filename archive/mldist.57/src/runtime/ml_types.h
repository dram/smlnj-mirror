 ...
 */
#define OBJ_isBOXED(p)		(((int)(p) & 0x1) == 0)
...
#define REC_ALLOC1(r, a)	{					\
	register ML_val_t *__p = (ML_val_t *)(MLState->ml_allocptr);	\
	*__p++ = (ML_val_t)MAKE_DESC(1, tag_record);			\
	*__p++ = (a);							\
	(r) = PTR_CtoML(MLState->ml_allocptr + 4);			\
	MLState->ml_allocptr = (int)__p;				\
    }

#define REC_ALLOC2(r, a, b)	{					\
	register ML_val_t *__p = (ML_val_t *)(MLState->ml_allocptr);	\
...
#define MACHINEID(id) struct machineid machine_id = \
                   {MAKE_DESC(sizeof(id)-1, tag_string), id}

/* heap allocation macros */
#define ML_alloc_write(i, x)	(((int *)(MLState->ml_allocptr))[(i)] = (int)(x))
#define ML_alloc(n)		(			\
    (MLState->ml_allocptr += ((n)+1)<<2),		\
    PTR_CtoML(MLState->ml_allocptr - ((n)<<2)))
...
/* ML closures */
#define CODE_ADDR(c)		((int)REC_SELPTR(c, 0))
