...
fun abstract(sgn as STRstr{kind=SIGkind{stamps={strStamps=sigStrStamps,
	...
	fun abstractTyc(sigtyc,strtyc) =
	    case sigtyc
	      of GENtyc{kind=DATAtyc _,...} => strtyc
	       | GENtyc{kind=ABStyc NULLtyc,stamp,arity,eq,path} => 
			if tycFixed stamp then strtyc
			  else GENtyc{kind=ABStyc strtyc, 
	...
	fun checkDataconSign(name,DATACON{sign=s1,...}::_,
	    ...
	    case spec
		 ...
		 (let val sigTycon = tSig sub i
		      ...
		      fun doFixed() = if Sharing.equalTycon(sigTycon,strTycon)
				       then ()
				       else complain 
		       ...
		       of DEFtyc _ => doFixed()
		        | GENtyc{stamp=s,arity,kind,eq=ref eq,...} =>
				 ...
				 case (kind, strTycon)
				  ...
				  GENtyc{arity=a',kind=DATAtyc(ref dc'),...})=>
