...
fun printDec dec =
 let val dec = pruneDec dec
  in fn lookup => 
	...
	and printAbsTyc(GENtyc{path=name::_, arity, eq, kind=ABStyc _, ...}) =
	    (print(if (!eq=YES) then "eqtype" else "type"); 
	     printFormals arity; print " ";
	     printSym name; newline())

	and printDataTyc(GENtyc{path=name::_,arity,kind=DATAtyc(ref dcons),...}) =
	    (print "datatype ";
	     printFormals arity; print " ";
	     printSym name; newline();
		...
		fun printBinder(VARbind(VALvar{name=[n],typ,...})) = 
		  ...
		  | printBinder(TYCbind tyc) = 
			  ...
			  fun showit(arity, name) =
			      (if EqTypes.isEqTycon tyc' then print "eqtype" 
							else print "type";
			       printFormals arity; printSym name)
		       in nlindent 4;
			  case tyc'
			   of GENtyc{path=name::_,arity,
				     kind=DATAtyc(ref dcons),...} =>
	...
	and printOpen(strvl) =  
        ...
        and printBinding(b: Basics.binding): unit=
            case b of
	      FCTbind(functorVar)=> (printFunctorVar functorVar; newline())
	    | SIGbind(signatureVar)=> (printSigVar signatureVar; newline())
