...
fun shiftStamps {str: Structure,
		 ...
		 paramstr: Structure,
		 importing: bool} =
    let fun newTyc(tyc as RECORDtyc labels) = tyc
	  | newTyc(tyc as DEFtyc{path,tyfun=TYFUN{arity,body}}) =
	...
	fun newBinding(VARbind(VALvar{typ,name,access})) = 
		      VARbind(VALvar{typ= ref(newType(!typ)),
				     name=name,access=access})
	  | newBinding(CONbind(DATACON{typ,name,const,rep,sign})) = 
		CONbind(DATACON{typ=newType typ,name=name,const=const,	
				rep=rep,sign=sign})
	  | newBinding b = b
        fun newEnv(REL{s,t}) =
...
fun shiftFctStamps(FUNCTOR{paramName,paramVis,param,body,
	...
	val newbody = shiftStamps {str=body,
				   ...
				   transTycStamp=transTycStamp,
	    			   paramstamp=pstamp, paramsign=psign, paramstr=newparam,
				   importing=true}
