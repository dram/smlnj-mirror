	    ...
	    then tyc
			...
			kind= case kind
			       of DATAtyc(ref dcons) =>
					DATAtyc(ref(map (transDATACON depth) 
						        dcons))
				| k => k}
	  | newTyc _ _ = impossible "Parse.includeSig.newTyc"
...
fun makeSIGid (ID,err) _ (depth,stamps) =
    let val binding = getSIGbinding (ID,err)
     in if depth>0 then ModUtil.shiftSigStamps(stamps,binding,false) else binding
    end
    ...
    let fun predefine(id,arity,f) = 
		let val r = ref nil
      		 in (bindTYC(id,tNext(GENtyc{path=[id],arity=arity,
				...
				eq=ref MAYBE,kind=DATAtyc r})),
		     r,f)
		end
	fun redefine(b,r,f) = (r := f(tyclooker,[]); b)
	val pre = map predefine db
	val tycbinds = map redefine pre
        val tycons = map (fn (TYCbind t) => t) tycbinds
	fun collectdcons((_,ref dcons,_)::rest,dcbinds) =
