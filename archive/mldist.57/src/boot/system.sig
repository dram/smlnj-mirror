...
signature CINTERFACE =
  ...
  (* C functions *)
    ...
    val exec	    : string -> (int * int)
    val system      : string -> int
    val export      : int -> bool
    val blas	    : (int * 'a) -> 'a
    val salb	    : string -> 'a
    val gettime     : unit -> (time * time)
    val setitimer   : (int * time * time) -> int
...
signature SYSIO =
  sig
    ...
    val write		: (fd * bytearray * int) -> int
    val writei		: (fd * bytearray * int * int) -> int
    val writev		: (fd * (bytearray * int) list) -> int
    val send_obd	: (fd * bytearray * int) -> int
...
signature SIGNALS =
  sig
    ...
    val setHandler : (signal * ((int * unit cont) -> resume_act) option) -> unit
    val inqHandler : signal -> ((int * unit cont) -> resume_act) option
  end
