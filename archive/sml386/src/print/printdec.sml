...
open Basics BareAbsn PrtUtil PrtBasic PrtType PrintVal Access
 ...
 let val dec = pruneDec dec
  in fn lookup => 
	...
	and printEb(EBgen{exn=DATACON{name,...},etype}) =
		 ...
		 of NONE => ()
		  | SOME ty' => 
		      if BasicTyp.isArrowType ty'
		      then (print " of "; printType(BasicTyp.domain ty'))
		      else ();
	       newline())
	...
	and printStr(str as STRstr{table,env,kind,...}) =
	    let val tInC = case kind
			    of STRkind _ => (fn t => TypesUtl.typeInContext(t,env))
			     | SIGkind _ =>  TypesUtl.printableType str
		fun visibleDcons(tyc,dcons) =
			...
			fun find(DATACON{name,...}::rest) =
			    (let val dcon as DATACON{typ,const,...} =
				      EnvAcc.lookCONinTable(table,name)
			 	 val dcontyc = TypesUtl.tyconInContext env
				                 (dconTyc(typ,const))
			      in if TypesUtl.eqTycon(tyc,dcontyc)
				 then dcon :: find rest
				 else find rest
		...
		fun printBinder(VARbind(VALvar{name=[n],typ,...})) = 
		  ...
		  | printBinder(CONbind(DATACON{name,typ,rep=VARIABLE _,...})) = 
		     (nlindent 4; print "exception "; printSym name;
		      if BasicTyp.isArrowType typ
		      then (print " of "; 
			    printType(tInC(BasicTyp.domain typ)))
		      else ())
		  | printBinder(TYCbind tyc) = 
		      let val tyc' = TypesUtl.tyconInContext env tyc
		...
		val _ = IntStrMp.app (fn x => bindlist := x ::(!bindlist)) table
...
fun printBindingTbl(tbl: Basics.symtable)=
   (let val _ = resetPrtType(); 
	val bindlist = ref []
	val _ = IntStrMp.app (fn x => bindlist:= x :: (!bindlist)) tbl
	val binders = sortBinders (!bindlist)
