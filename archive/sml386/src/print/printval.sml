...
open System System.Unsafe Basics BasicTyp TypesUtl ErrorMsg PrtUtil
...
fun getSTRpath [id] = (EnvAcc.lookSTR id handle Env.Unbound => raise FAIL)
  | getSTRpath [] = raise FAIL
  | getSTRpath q = EnvAcc.lookPathSTR(q,(fn s => raise FAIL))
...
and printDcon(_,_,_,0,_,_) = print "#"
  | printDcon(obj:object, tyc as GENtyc{arity,path,...}, argtys,
	...
	then print dname
	  ...
	  let val fixity = EnvAcc.lookFIX name  (* may be inaccurate *)
	      val dom = case typ
	      ...
	      val dom = if is_relative(dom)
			then let val name::path' = path
				 val STRvar{binding=STRstr{table,env as REL{t,...},
					    ...
					    ...} =
				       getSTRpath(rev path')
				 val tyc' = case EnvAcc.lookTYCinTable(table,name)
					      of INDtyc i => t sub i
					       | tyc'' => tyc''
