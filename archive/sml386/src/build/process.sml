...
sig
  ...
  val process : string * (CPS.function * string -> unit) option -> unit
...
structure PrcFile : PROCESSFILE =
struct

  open Access Basics PrtUtil EnvAcc NewParse
 ...
 (* debugging aid--print the slots of a structure
  ...
  fun printslots s =
	  ...
	  val unsortedlist = (usl := nil; IntStrMp.app buildlist table; !usl)
  ...
  open ErrorMsg BareAbsn Lambda System.Timer
		...
		let val timer = start_timer()
         ...
         fun transStrb sb =
	     	let val timer = start_timer()
	     	    ...
	     	    val lam = Translat.transDec inputSource
	    ...
	    let val rec f = 
		  fn EOF => raise Eof
		   ...
		   | PARSE(absyn as SIGdec _) =>
			(PrtAbsyn.printDec(absyn,0,!printDepth);
			 newline())
      ...
      (primeEnv();
	   ...
	   let val STRvar{access=PATH[lvMath],...} =
	       ...
	       and STRvar{binding=STRstr{table=otable,...},...} =
		     lookSTR (Symbol.symbol "Overload")
	    ...
	    in Env.reset();
	       ...
	       IntStrMp.app (IntStrMp.add table) otable;
