...
functor Importer(structure FilePaths: FILEPATHS
		 ...
		 structure ModlComp: MODULE_COMPILER)
		: IMPORTER =
struct
  open PrtUtil Access Basics Stampset Env NewParse
 ...
 (* impliedPath: derived from FilePaths.impliedPath, but catches
  ...
  type LambDynModule = ModlComp.LambDynModule
  type CodeDynModule = ModlComp.CodeDynModule
  ...
  fun importModule(STATmodule{table,lvars}) : lvar list = 
	  ...
	  fun renBinding(SIGbind(SIGvar{name,binding})) =
       ...
       in IntStrMp.app (fn (i,s,b) => add(i,s,renBinding b)) table;
	  newlvars
      end
 ...
 (* We must do a syntactic check that the source declarations in a module
  ...
  val rec kosherModuleDecl =
	fn BareAbsn.FCTdec _ => ()
	 | BareAbsn.SIGdec _ => ()
	 | BareAbsn.MARKdec(dec,_,_) => kosherModuleDecl dec
	 | BareAbsn.SEQdec decs => app kosherModuleDecl decs
 ...
 fun getModule(pervasives, parents, indent, path, name) =
	...
	fun compSource0(source: instream, name:string) : statModule * CodeDynModule =
	  let val inputSource = ErrorMsg.newSource(name,source,false,std_out)
	      ...
	      val emptyDynModule = ModlComp.mkLambda(inputSource,
		...
		| loop(PARSE(BareAbsn.IMPORTdec(name::rest)),
		       dynMod, lvars, imports) =
		      let val (stat, codeDyn) =
			      getModule(pervasives, parents, indent+2,
					path, name)
			  val newLvars = importModule stat
			  val lambDyn = ModlComp.abstractDynModule
					  	(codeDyn, newLvars)
			  val dynMod' = ModlComp.importDynModule
					  	(lambDyn, dynMod)
		       in loop(PARSE(BareAbsn.IMPORTdec rest), 
			       dynMod', newLvars @ lvars, name :: imports)
		      end
		| loop(PARSE(BareAbsn.IMPORTdec nil), d, l, i) =
			loop(parser(), d, l, i)
		| loop(PARSE absyn, dynModule, lvars, imports) = (* normal program *)
			  ...
			  val newMod =ModlComp.importDynModule
			      (ModlComp.mkLambda(inputSource,absyn,newLvars),
			       dynModule)
					handle ModlComp.MkLambda =>
	      ...
	      val statModule= STATmodule{table=Env.popModule(pervasives),
					 lvars=lvars}
	      val dynModule = ModlComp.compileDynModule lambDynModule
			      handle ModlComp.CompileDynModule =>
  ...
  fun getAndExecModule pervasives filename : unit =
	  ...
	  val result = ModlComp.executeDynModule Linkage.lookup compDynModule
