...
functor ModlComp(structure Machm: CODEGENERATOR): MODULE_COMPILER =
struct
  ...
  exception MkLambda
  fun mkLambda(source, dec, lvars) =
       {lambda=Translat.transDec source dec,
  ...
  fun abstractDynModule(codeDynModule, lvars) =
   ...
   fun compileDynModule {lambda,imports,importLvars,lvars} =
      let val openLambda = lambda(Lambda.RECORD(map Lambda.VAR lvars))
          ...
          val finalLambda = Opt.closetop(lambda', PrcFile.getCore())
