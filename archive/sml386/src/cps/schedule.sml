structure Schedule : sig val schedule: CPS.function -> CPS.function
                     end =
struct

open CPS Access
val uniq = SortList.uniq
...
fun show nil = print "."
  | show (v::vl) = (print (v:int);  print ",";  show vl)

fun showl nil = print "]"
  | showl [x] = (print (x:int); print "]")
  | showl (x::rest) = (print (x:int); print "."; showl rest)
		      

fun showc (({defs=nil,path,...}:child)::vl) = (print "["; showl path;
					       print "branch,"; showc vl)
  | showc ({defs=w::_,path,...}::vl) = (print "["; showl path;
					print w; print ","; showc vl)
  | showc nil = print "."
	...
	let 
	 ...
	 in case e 
	     of SELECT(i,v,w,c) => usedef([v],[w],c, fn[c]=>SELECT(i,v,w,c))
	      ...
	      | RECORD(l,w,c) => usedef(map #1 l, [w], c, fn[c]=>RECORD(l,w,c))
	      | APP(v,vl) => branch(VAR(mem)::v::vl, nil, fn _ => e)
              | PRIMOP(args as (P.:=,_,_,_)) => storeop args
              | PRIMOP(args as (P.unboxedassign,_,_,_)) => storeop args
