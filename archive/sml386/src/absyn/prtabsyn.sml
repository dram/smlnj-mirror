...
sig
    ...
    val printPat : BareAbsn.pat * int -> unit
    val printExp : BareAbsn.exp * int * int -> unit
    val printRule : BareAbsn.rule * int * int -> unit
    val printVB : BareAbsn.vb * int * int -> unit
    val printRVB : BareAbsn.rvb * int * int -> unit
    val printDec : BareAbsn.dec * int * int -> unit
    val printStrexp : BareAbsn.strexp * int * int -> unit
end

structure PrtAbsyn : PRINTABSYN = struct
structure BareAbsn = BareAbsn
open BareAbsn Access Basics PrtUtil PrtType PrtBasic ErrorMsg Tuples
...
and printDconPat(_,_,_,0) = print "<pat>"
  ...
  | printDconPat(APPpat(DATACON{name,...},p),l,r,d) =
	...
	val fixity = EnvAcc.lookFIX name
	fun prdcon() =
...
fun printExp(_,_,0) = print "<exp>"
  ...
  | printExp(HANDLEexp(exp, HANDLER(FNexp rules)),ind,d) =
      (printExp(exp,ind,d-1); nlindent(ind); print "handle ";
       printvseq(ind+5) "| " (fn r => printRule(r,ind+7,d-1)) rules)
  ...
  | printExp(CASEexp(exp, rules),ind,d) =
      (print "(case "; printExp(exp,ind+5,d-1); nlindent(ind+3);
       print "of "; printvseq (ind+4) "| " (fn r => printRule(r,ind+4,d-1)) (trim rules);
       print ")")
  | printExp(FNexp rules,ind,d) =
      (print "(fn "; printvseq (ind+1) "| " (fn r => printRule(r,ind+3,d-1)) (trim rules);
       print ")")
...
and printAppExp(_,_,_,_,0) = print "<exp>"
  | printAppExp arg =
    let fun fixityprint(name,e,l,r,ind,d) =
		...
		val fixity = case name of [id] => EnvAcc.lookFIX id
					| _ => NONfix
		fun pr() =
