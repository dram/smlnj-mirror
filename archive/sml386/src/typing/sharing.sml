...
struct
  ...
  open ErrorMsg PrtUtil Basics EnvAcc TypesUtl
...
(* val _ = (print ">>> doSharing: ";
	 ...
	 in  print "base="; print base; print " "; print "limit="; print(!limit);
	  ...
	  val {union=tunion,find=tfind} = Unionfnd.new(freeTycStamp)
	  ...
	  fun tycunion(t1,t2) =
		  ...
		  fun un(t1 as GENtyc{stamp=s1,...},t2 as GENtyc{stamp=s2,...}) =
			((tunion(s1,s2); ())
			 handle Unionfnd.Union => 
			   if equalTycon(getTyc t1, getTyc t2)
			   then ()
			   else
((* PrtType.printTycon t1; print " ==> "; print(tfind s1); newline();
 PrtType.printTycon t2; print " ==> "; print(tfind s2); newline(); *)
...
((* PrtType.printTycon t1; newline(); PrtType.printTycon t2; newline(); *)
