...
struct
   ...
   open Access Basics BasicTyp Env NmSpace
...
(* primTypes structure *)
   ...
   fun entercon(s: string, c: datacon) =
       IntStrMp.add typesTable
         (varIndex(symbol s),s,CONbind c)

   fun entertyc(s : string, t : tycon) =
       IntStrMp.add typesTable 
   ...
   val primTypes = (
	...
	entertyc("option", optionTycon);
	entercon("SOME", SOMEDcon);
	entercon("NONE", NONEDcon);
...
(* inLine structure *)
   ...
   fun enter( s : string, p : primop ) =
       let val name = symbol s
        in IntStrMp.add inLineTable
              (varIndex name,
	       s,VARbind(VALvar{access=INLINE p, name=[name],typ= ref bottom}))
       end
