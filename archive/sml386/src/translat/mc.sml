...
signature MC = sig
	...
	structure L : LAMBDA
	val matchCompile : (A.pat * L.lexp) list * (string->unit) -> L.lexp
	val bindCompile : (A.pat * L.lexp) list * (string->unit) -> L.lexp
	end

structure MC : MC = struct

structure A : BAREABSYN = BareAbsn
...
val rec freevars =
	fn VARpat(VALvar{access=LVAR v,...}) => [v]
	 | RECORDpat{pats=ref pats,...} => SortList.foldmerge(map freevars pats)
	 | APPpat(_,pat) => freevars pat
	 | CONSTRAINTpat(pat,_) => freevars pat
	 | LAYEREDpat(l,r) => SortList.merge(freevars l,freevars r)
	 | _ => []
...
fun matchPrint [] _ _ = ()
  ...
  | matchPrint ((pat,_)::more) [] _ =
	(print "        "; PrtAbsyn.printPat(pat,!printDepth); print " => ...\n";
	 matchPrint more [] 0)
  | matchPrint ((pat,_)::more) (taglist as (tag::tags)) i =
	if i = tag
	then (print "  -->   "; PrtAbsyn.printPat(pat,!printDepth);
	      print " => ...\n"; matchPrint more tags (i+1))
	else (print "        "; PrtAbsyn.printPat(pat,!printDepth);
...
fun genCompile(flag1,warning1,flag2,test,warning2,printer) (rules,warn) = 
