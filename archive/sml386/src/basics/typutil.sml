...
structure TypesUtl : TYPESUTIL = struct

structure Basics = Basics

open PrtUtil Basics List2 ErrorMsg BasicTyp
...
(*      (case s sub i *)
	 ...
	 | NULLstr => (prIntPath (i::rest); newline();
		       ErrorMsg.impossible "TypesUtl.lookTycPath: NULLstr")
	 | INDstr _ => (prIntPath (i::rest); newline();
		        ErrorMsg.impossible "TypesUtl.lookTycPath: INDstr")
	 | SHRstr _ => (prIntPath (i::rest); newline();
		        ErrorMsg.impossible "TypesUtl.lookTycPath: SHRstr"))
  | lookTycPath _ = raise ErrorMsg.Cascade "TypesUtl.lookTycPath.2"
  ...
  (fn (RELtyc (p as [_])) => let val tyc = lookTycPath(p,env)
    ...
    | (RELtyc(i::rest)) => 
      let fun scan(STRbind(STRvar{name,binding=INDstr j,...}):: others) =
	    ...
	    | scan [] = impossible "TypesUtl.printableTycon 1"
       in scan bindings
      end
    | (INDtyc _) => impossible "TypesUtl.printableTycon: INDtyc"
    | (SHRtyc _) => impossible "TypesUtl.printableTycon: SHRtyc"
    | tyc => tyc)
...
local
	...
	CONty(GENtyc{stamp = Stampset.newStamp(#tycStamps Stampset.globalStamps),
		     path = [Symbol.symbol "dummy"], arity = 0,
