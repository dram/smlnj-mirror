...
open Basics Stampset ErrorMsg TypesUtl
	...
	let fun prop stamp' =
		for (depset(stamp')) (fn s =>
		    let val eqpold = applyMap(eqprop,s)
		     in if eqpold <> eqp
			then (join(eqp,eqpold);
			      updateMap eqprop (s,eqp);
			      if earlier s
			      then prop s
			      else ())
			      handle INCONSISTENT => 
...
fun isEqTycon(GENtyc{eq,...}) =
	...
	| NO => false
	| IND => false
	| DATA => (print "isEqTycon 1 DATA\n"; false)  (* DEBUGGING *)
	| UNDEF => (print "isEqTycon 1 UNDEF\n"; false)) (* DEBUGGING *)
  | isEqTycon(DEFtyc{tyfun as TYFUN{arity,...},...}) =
	isEqType(applyTyfun(tyfun,replicate(arity,BasicTyp.intTy)))
