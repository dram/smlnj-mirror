...
signature PRINTTYPE = sig
  ...
  val resetPrtType : unit -> unit
end

structure PrtType : PRINTTYPE = struct

structure Basics = Basics

open ErrorMsg Basics List2 PrtUtil
...
fun prkind (PRIMtyc) = print "PRIM/"
  | prkind (FORMtyc) = print "FORM/"
  | prkind (ABStyc _) = print "ABS/"
  | prkind (DATAtyc _) = print "DATA/"

fun printPath(p,tyc0) =
    let fun get([id],STRstr{table,env,...}) =
	      (case EnvAcc.lookTYCinTable(table,id)
		of INDtyc i =>
		     (case env
		       of REL{t,...} => t sub i
		        | DIR => impossible "printPath.get 1")
		 | SHRtyc p => TypesUtl.getEpathTyc(p,env)
		 | tyc => tyc)
	  | get(id::rest,STRstr{table,env,...}) =
	      let val STRvar{binding,...} = EnvAcc.lookSTRinTable(table,id)
		  val str' = case (binding,env)
			       of (INDstr i,REL{s,...}) => s sub i 
			        | (SHRstr (i::r),REL{s,...}) =>
				    TypesUtl.getEpath(r,s sub i)
				| (STRstr _, _) => binding
	...
	fun try(name::untried,[]) = 
	     ((if TypesUtl.equalTycon(EnvAcc.lookTYC name,tyc0)
		  then printSym name
		  else try(untried,[name]))
	      handle Env.Unbound => try(untried,[name])
	           | Env.UnboundTable => try(untried,[name]))
	  | try(name::untried,tried) =
	     (let val STRvar{binding,...} = EnvAcc.lookSTR name
	       in if TypesUtl.equalTycon(get(tried,binding),tyc0)
...
val GENtyc{stamp=arrowStamp,...} = BasicTyp.arrowTycon
...
fun printTycon(RECORDtyc labels) =
  ...
  | printTycon(NULLtyc) = print "NULLtyc"
...
and printType1(ty: ty, sign: {weakness:int,eq:bool} list) : unit =
	      ...
	      of VARty(ref(INSTANTIATED ty')) => prty(ty')
		   ...
		   (case tycon
		      of GENtyc{kind=ref(PRIMtyc), stamp,...} => 
			   ...
			   (printTypeArgs args; printTycon tycon)	
	...
	and printRECORDty([],[]) = print "unit"
	  | printRECORDty(lab::labels, arg::args) =
		 ...
		 (labels,args);
	       print "}")
	  | printRECORDty _ = ErrorMsg.impossible "PrtType.printRECORDty"
