...
fun emacsBeginDebug () =
    printL ["(emacs (sml-start-debugging))\n"]

fun emacsKillBuffer bufName =
    printL ["(emacs (kill-buffer \"", bufName, "\"))\n"]

fun emacsError s =
    printL ["(emacs (sml-error \"", s, "\"))\n"]

fun cdHook d =
(* In system.sml we rebind System.Directory.cd to call this hook. *)
    if !emacs then printL ["(emacs (cd \"", d, "\"))\n"] else ();
 ...
 * call trace list, or to NONE.  Update the display. *)
    ...
    in
	emacsModify (somes [oldEv, newEv]) (fn () => backtrace := new)
    end
...
(* ------ (almost) All display updating is done above this line. ------ *)
...
fun prWhere ww =
  let val SOME (s,loc,vis) = XeventDesc ww
  in
...
fun showEvents filename =
   let fun f loc =
	     ...
	     val SOME (_,(_,charno),_) = 
			XeventDesc ((hd whrl),0) (* may raise Hd *)
	     fun p whr =
	       let val SOME (s,_,_) = XeventDesc (whr,0)
 ...
 * might be useful in Emacs as well. *)
  let fun p (n,TIME whn) =
	...
	| p (n,EVENT eventn) =
		(print n; print "\t"; prWhere(eventn,0); print "\n")
...
fun usedbg file = if Xindebug() then
		print "Already running under debugger.\n"
	     else Xuse_file (FULLDEBUG "DEBUGZ") file
fun uselive file = if Xindebug() then
		print "Already running under debugger.\n"
	     else Xuse_file (LIVEDEBUG ("DEBUGZ","DEBUGN")) file
