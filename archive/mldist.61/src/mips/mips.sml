functor MipsCM(MipsC : MIPSCODER) : CMACHINE = struct
	...
	20		my_arithtemp (internal to CMACHINE)
				may contain anything except pointers
	21	yes	my_ptrtmp (internal to CMACHINE)
				may contain only pointers or tagged integers.
	...
	24--25		arithmetic temporaries 1 and 2 (exported by CMACHINE)
				may contain anything except pointers
    ...
    (* For the indexed fetch and store, arithtemp is not tagged--the
    ...
    fun checkLimit max_allocation = 
    	...
    	M.add(dataptr', Immed(max_allocation-4096), my_arithtemp');
            M.add(my_arithtemp', limit, Reg 0) (* overflow if out of room *)
        )
