...
(*       fun emit_fneg args = C.emit (SparcInstr.I_fneg args) *)
	...
	fun mkLabExp (lab, n) = LABELexp{base= C.baseLab, dst= lab, offset= (n-4096)}
      ...
      fun setBaseAddr (contReg, lab, tmpR) = let
	    val addr = ref PositionIndependent
	    in
	      C.emitSDI (
	        SparcInstr.SetBaseAddr(addr, contReg, mkLabExp(lab, 0), tmpR));
	      addr
	    end
  ...
  (** Dedicated registers **)
    ...
    val globalvar = SOME(Direct(REG 29))    (* %i5 *) (* only for debugging *)
    val arithtemps = nil
   ...
   *)
    fun beginStdFn (Direct closureReg, ImmedLab lab) = let
	  val tmpR = getTmpReg()
	  val addr = setBaseAddr (closureReg, lab, tmpR)
	  in
	    freeTmpReg tmpR; addr
	  end
      | beginStdFn _ = ErrorMsg.impossible "[SparcCM.beginStdFn]"
    val beginStdFn = SOME beginStdFn
   ...
   *)
     ...
     *)
      fun finishReal r = let val tmpR = getTmpReg()
	    in
	      loadImmed (System.Tags.desc_real, tmpR);
