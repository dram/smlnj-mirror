...
(*      | I_fneg of fops2                     (* floating-point negation *) *)
   ...
   *)
    ...
    datatype sdi
      = SetBaseAddr of (addressing ref * register * labelexp * register)
   ...
   * be consistent with expand in "sparccoder.sml" *)
    fun sizeOf I = let
	  ...
	  in
	    case I
	     of SetBaseAddr(ref PositionIndependent, _, _, _) => sz0max
	      | SetBaseAddr(ref Relative, _, labexp, _) => let
		  val x = span labexp
		  in if (4095 < x)
		    then if (8190 < x)
		      then sz16max
  ...
  (* expand SDIs into real instruction sequences. *)
    fun expand (SetBaseAddr(ref PositionIndependent, _, _, _), 0) = []
      | expand (SetBaseAddr(ref Relative, contReg, labexp, _), 8) = [
	  I_ld(contReg, IMrand 0, baseReg),
	  I_sub(baseReg, LABrand labexp, baseReg)]
      | expand
	(SetBaseAddr(ref Relative, contReg, LABELexp{base, dst, offset}, _), 12) = let
      ...
      | expand (SetBaseAddr(ref Relative, contReg, labexp, tmpR), 16) = [
	  I_ld(contReg, zeroRand, baseReg),
