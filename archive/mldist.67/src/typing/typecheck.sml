...
in
   ...
   fun Abs({lamd,wmax,outer=RATOR{absd,base,outer,...},...}: occ) =
         {lamd=lamd+1,absd=absd,base=base,wmax=wmax,outer=outer} 
     | Abs {lamd,absd,base,wmax,...} =
...
fun expType(exp: exp, occ: occ, loc) : ty =
    case exp
      of VARexp(r as ref(VALvar{typ,access,name})) => 
	   let val ty = applyPoly(!typ,abscount occ,base occ,wmax occ)
       ...
       | CONexp(DATACON{typ,...}) =>
	   applyPoly(typ,abscount occ,base occ,wmax occ)
       | INTexp _ => intTy
       | REALexp _ => realTy
