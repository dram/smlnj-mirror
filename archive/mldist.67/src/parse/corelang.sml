...
*)
    val lookID : Basics.env -> Symbol.symbol * ErrorMsg.complainer -> BareAbsyn.exp
    ...
    val pat_id:		Basics.env -> symbol -> pat
    ...
    val qid_pat:	Basics.env -> spath * ErrorMsg.complainer -> pat
    ...
    val makeEBeq:	symbol * spath * ErrorMsg.complainer
	                  -> eb list withenv epathvstamped
    ...
    val makeOPENdec:	structureVar list enved -> dec withenv epathvstamped
    ...
    val makeFUNdec:	rawclause list list enved uvars -> dec withenv epathvstamped
    ...
    val makeDB:		(symbol * int * datacon list withenv epathed) list * (bool -> tb list withenv epathvstamped) ->
				dec withenv epathvstamped
    val makeDB':	tyvar list * symbol * (Basics.env * ty -> (symbol * bool * ty) list) uvars
    ...
    val makeABSTYPEdec: (symbol * int * datacon list withenv epathed) list 
			  * (bool -> tb list withenv epathvstamped) 
			  * dec withenv epathvstamped   -> dec withenv epathvstamped
    ...
    val toplevelexp:	Basics.env * exp evstamped uvars * (BareAbsyn.linenum*BareAbsyn.linenum->ErrorMsg.complainer) * (BareAbsyn.linenum*BareAbsyn.linenum) -> dec withenv
end


structure CoreLang: CORELANG = struct    
open ErrorMsg Symbol PrintUtil EqTypes
open Access Basics BasicTypes TypesUtil Absyn TyvarSet
open EnvAccess Misc

type stampsets = Stampset.stampsets
(* type 'a stamped = stampsets -> 'a *)
type 'a susp = unit -> 'a
type 'a epathed = Basics.env * symbol list -> 'a
type 'a uvars = 'a * tyvarset
type 'a evstamped = Basics.env * tyvarset * stampsets -> 'a
type 'a epathvstamped = Basics.env * spath * tyvarset * stampsets -> 'a
...
fun lookID env (id,err) =
    varcon (lookVARCON env id)
    handle Env.Unbound =>
	(err COMPLAIN ("unbound variable " ^ Symbol.name id);
		        VARexp(ref(VALvar{access=PATH[0], name=[id],
					  typ=ref ERRORty})))
...
fun pat_id env id = (case lookVARCON env id
...
fun qid_pat env (qid,err) = CONpat(lookPathCON env (qid,err COMPLAIN))
...
val bogusCON = DATACON{name=Symbol.varSymbol "bogus",const=true,typ=ERRORty,
		       rep=UNDECIDED,sign=[]}
fun getEXN env (id,err) = 
   (case lookVARCON env id 
     of CONbind c => c
      | _ => raise Env.Unbound)
   handle Env.Unbound => (err COMPLAIN("unbound exn: " ^ Symbol.name id); 
			   bogusCON)
...
fun makeEBeq (id,qid,err) (env,_,_,_) =
  let val edef as DATACON{const,typ,rep,sign,...} =
	       case qid of [s] => getEXN env (s,err)
			 | q => lookPathCON env (q, err COMPLAIN)
      val exn = DATACON{name=id,const=const,typ=typ,sign=sign,
			rep=VARIABLE(PATH[namedLvar id])}
...
fun makeFUNdec (fb,fv) (env,_,tv,st) =
...
fun makeDB(db,tb) (env,path,tv,stamps as {tycStamps,...}:stampsets) =
  let val env' = ref(Env.empty: Basics.env)
      ...
      val (withtycs,env'') = tb false (Env.atop(!env',env),path,tv,stamps)
...
fun makeDB'(args,name,(constrs,cv),err) (env,path) =
  let val arity = length args
      val rangeType = CONty(lookPathArTYC env ([name],arity,err COMPLAIN), 
			    map VARty args)
...
fun makeABSTYPEdec(db,tb, ldecs) ($ as (env,path,tv,stamps)) =
    let val (DATATYPEdec{datatycs,withtycs},env0) = makeDB(db,tb) $
...
fun toplevelexp(env,(exp,tyvars),err,loc) =
    let val exp = exp (env,tyvars,Stampset.globalStamps)
