...
open ErrorMsg PrintUtil Access Basics Symbol BasicTypes TypesUtil
...
fun varApplied(VALvar{access,name,typ}: var, {path, strenv}, qid) : var =
...
fun lookVARCONinStr(STRstr{table,env,stamp,...},id,ap,qid,err): binding =
    ((case Env.look table id
       of VARbind v => VARbind(varApplied(v,{path=ap,strenv=env},qid))
...
fun transBinding (info as {path,strenv},qid) =
   (fn (CONbind c) => CONbind(dconApplied(c,info))
     | (VARbind v) => VARbind(varApplied(v,info,qid))
     | (STRbind v) => STRbind(strApplied(v,info,qid))
     | (TYCbind(INDtyc i)) =>
...
fun openStructureVar env (STRvar{access=PATH p,binding,...}) : env =
    (case binding
      of STRstr{table,env=strenv,kind,...} =>
	   let val qid =
	        case kind
		  of STRkind{path,...} => path
		   | _ => []
	    in Env.open' (table, transBinding({path=p,strenv=strenv},qid), env)
	   end
...
fun lookSTRinStr(STRstr{table,env,stamp,...},id,ap,qid,err) =
    (strApplied(lookSTR table id,{path=ap,strenv=env},qid)
     handle Unbound => 
	(if stamp=bogusStrStamp then ()
...
fun lookPathinStr
    ...
    let fun getStr([id],str,ap) = lookLast(str,id,ap,allButLast spath,err)
