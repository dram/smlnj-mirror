...
open Basics ErrorMsg TypesUtil
      ...
      of META{depth=d,weakness=w,eq=e} =>
	   let fun scan (e:bool) (ty: ty) : unit =
			...
			then raise Unify("circularity")
		      ...
		      | CONty(GENtyc{eq,...}, args) =>
			  (case !eq
			    of OBJ => app (scan false) args
			     | YES => app (scan e) args
			     | NO =>  if e then raise Unify "equality type required"
				      else app (scan e) args
			     | IND => if e then raise Unify "equality type required"
				      else app (scan e) args
			     | MAYBE => impossible "instantiate -- eq check")
