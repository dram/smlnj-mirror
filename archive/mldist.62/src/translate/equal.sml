	...
	(if !System.Control.debugging
	 ...
	 case ty
	  of VARty(ref(INSTANTIATED t)) => test t
	   ...
	   | CONty(tyc as GENtyc{kind=ABStyc _,eq=ref YES,...}, tyl) =>
	       atomeq tyc
	   | CONty(GENtyc{kind=ABStyc tyc,eq=ref NO,...}, tyl) =>
	       test(CONty(tyc,tyl))
	   ...
	   | CONty(tyc as
		    GENtyc{kind=DATAtyc(ref[DATACON{const=false,rep,typ,...}]),
		             ...},
		   tyl) =>
	       (case rep
		  of TRANSPARENT => 
	   ...
	   | CONty(GENtyc{kind=DATAtyc(ref dcons),...}, tyl) =>
	      (find ty
	       handle Notfound =>
	       let val v = mkLvar() and x=mkLvar() and y=mkLvar()
		   val (eqv, patch) = enter ty
