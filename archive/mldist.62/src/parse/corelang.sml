...
fun makeEBof (id,(etype,lv),err) (_,tv,_) =
  let val ty = etype tyArg
      val exn = DATACON{name=id,const=false,typ=(ty --> exnTy),
			rep=VARIABLE(LVAR(namedLvar id)), sign=[]}
      val localtyvars = get_tyvars(diff_tyvars(lv,tv))
   in if null localtyvars then ()
      else err COMPLAIN "unbound tyvars in exception declaration";
      bindCON(id,exn); [EBgen{exn=exn,etype=SOME ty}]
  end
  ...
  let fun predefine (id,arity,db') = 
	  let val r = ref nil
	      val tyc = GENtyc{path=id::path,arity=arity,
			      stamp=Stampset.newStamp tycStamps, eq=ref MAYBE,
			      kind = DATAtyc r}
	   in bindTYC(id,tyc);
	      (tyc,r,db')
	  end
      ...
      fun redefine (tyc,r,f) = (r := f(tyArg,path); tyc)
...
fun makeABSTYPEdec(db,ldecs) $ =
	...
	app bind withtycons;
	ABSTYPEdec{abstycs=ref datatycs,withtycs=withtycs,body=body}
    end
