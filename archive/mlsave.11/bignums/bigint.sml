...
*)
    ...
    exception Getbit
    fun getbit (a,i) = 
	if size a < i + 1 orelse i < 0 then raise Getbit
	else (fetch(a,i >> 3) >> (i bit_and 7)) bit_and 1
   ...
   fun plus (a,b) =
	  ...
	  fun add(i,carry) = 
		if i=la then carry
			    ...
			    + (fetch(b,i) handle Byte_array => vb) + carry
		         val d = x div 256
	  ...
	  fun extend x = (* make a new bigint with msd x *)
		 ...
		 val c' = create(n+1, 0)
		 fun copy i = (store(c',i,fetch(c,i)); copy(i-1))
	      in store(c',n,x); copy(n-1) handle Byte_array => c'
	     end
	...
	let val n = length big
	    ...
	    fun shift(0,_) = shifted
	      | shift(i,c) = let val digit = fetch(big,i+bytes-1)
				 ...
				 shift(i-1,digit bit_and mask)
			     end
	    val carry = case (lose,neg) of (true,_) => fetch(big,n-1) bit_and mask
					 | (false,true) => mask
					 | (false,false) => 0
...
in
    fun fullshift(num,0) = num
      | fullshift(num,n) =
		...
		else rightshift(num,n bit_and 7,n >> 3)
	    end
end
	...
	let fun bytestr(_,0) = ""
	      | bytestr(bits,i) = let val more = bytestr(bits>>1,i-1)
				      val bit = Integer.makestring(bits bit_and 1)
				  in more ^ bit end
