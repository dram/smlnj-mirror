...
fun share_map f nil = raise SHARE
  | share_map f (x::l) =
      (f x) :: ((share_map f l) handle SHARE => l)
      handle SHARE => x :: (share_map f l)
	    ...
	    let val ty = prune ty
	     in case ty
		  of VARty(tv as TYVAR{status = ref BOUND,...}) =>
		   ...
		   | UNKNOWNty => raise SHARE  (* propagate error *)
		   | _ => impossible "fresh in typesutil.freshTy"
	    end
     in fresh(ty) handle SHARE => ty
    end;
     ...
     in case (ty1,ty2)
	  of (VARty(tv1),VARty(tv2)) =>
	       ...
	       then ()
		     ...
		     handle Unify => instantiate(tv2,ty1)) (* tv1 is FIXED *)
