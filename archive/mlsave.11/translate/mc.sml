...
fun orS nil =  impossible "orSwitch nil in mc"
  | orS (arg as (p,result,tag)::more) =
	...
	in  markNotRedundant tag;
	    ((INTcon i,result,tag)::cases,default)
	end
    | REALpat r =>
	let val (cases,default) = orS (diffPats (fn REALpat s => r=s) arg)
	in  markNotRedundant tag;
	    ((REALcon r,result,tag)::cases,default)
	end
    | STRINGpat s =>
	let val (cases,default) = orS (diffPats (fn STRINGpat t => s=t) arg)
	in  markNotRedundant tag;
	    ((STRINGcon s,result,tag)::cases,default)
	end
    | WILDpat => (markNotRedundant tag;
		 (nil,SOME result))
    | VARpat(VALvar{access=LVAR v,...}) => (markNotRedundant tag;
...
fun conS nil = (nil,NONE)
  | conS (arg as (p,result,tag)::more) =
	...
	in  markNotRedundant tag;
	    ((DATAcon dcon,result)::cases,default)
	end
	...
	let fun divide nil = (nil,nil)
     ...
     | WILDpat => (maybeNotRedundant tag;
		  (nil,SOME result))
     | VARpat(VALvar{access=LVAR v,...}) => (maybeNotRedundant tag;
					    (nil,SOME(bind(x,v,result))))
...
and mcor nil = impossible "mcor.nil in mc"
  | mcor (arg as (p,result,tag)::more) =
      let val x = mkLvar()
      in  case p of
	      CONpat(DATACON{dcons=ref nil,...}) => (* exception *)
		let val (cases,default) = conSwitch x arg
		in  areNotRedundant();
		    FN(x,SWITCH(VAR x,cases,default))
		end
	    | APPpat (DATACON{dcons=ref nil,...},_) => (* exn *)
		let val (cases,default) = conSwitch x arg
		in  areNotRedundant();
		    FN(x,SWITCH(VAR x,cases,default))
		end
	    | CONpat(DATACON{dcons = ref [_],...}) =>
		(markNotRedundant tag; FN(x,result))
	    | CONpat(DATACON{dcons,...}) =>
		let val (cases,default) = conSwitch x arg
		in  FN(x,SWITCH(VAR x, cases,
		        (if length (!dcons) = length cases
...
fun bindPrint ((pat,_)::_) = (prstr "        "; printPat pat; prstr " = ...\n")
