...
structure Basics : BASICS = struct
  ...
  local exception Tycon in
	...
	case tycon
	  of ATOMtyc{name,arity,...} => ATOMtyc{stamp=stamp,name=name,arity=arity}
	   | TYPEtyc{name,params,def,...} =>
	       TYPEtyc{stamp=stamp,name=name,params=params,def=def}
	   | DATAtyc{name,params,dcons,...} =>
	       DATAtyc{stamp=stamp,name=name,params=params,dcons=dcons}
           | VARtyc{name,arity,...} => VARtyc{stamp=stamp,name=name,arity=arity}
	   | _ => raise Tycon
  ...
  (* type functions *)
    ...
    fun eqTy(ty,ty') =  (* BUG -- doesn't take account of abbreviations *)
  ...
  (* signature, structure, and functor functions *)
	...
	ErrorMsg.impossible "mkSIG obsolete in Basics"
...
(* governs printing of internal information *)
