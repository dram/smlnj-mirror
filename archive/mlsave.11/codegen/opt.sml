...
fun applyOnly (v,b) =
   let exception Found
       fun checkv vl =
	    ...
	    val rec any =
		  fn VAR w => if inSet w then raise Found else ()
	    ...
	    val rec check =
		  fn VAR w => if inSet w then raise Found else ()
		   ...
		   | APP(FN(w,b),FN(u,c)) =>
			    if ((any c; false) handle Found => true)
    ...
    in (checkv [v] b; true) handle Found => false
   end
...
*)
		...
		let fun f (DATAcon(DATACON{rep=ref(VARIABLE(PATH p)),
					    name,const,vtype,dcons}), e) =
		          (DATAcon(DATACON{rep=ref(VARIABLE(PATH(makp p))),
			       name=name,const=const,vtype=vtype,dcons=dcons}),
			   mak e)
		      | f (c,e) = (c, mak e)
