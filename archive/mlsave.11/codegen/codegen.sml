...
sig
  ...
  exception Notfound_Codenv of Access.lvar
...
struct 
  ...
  fun genswitch (gen,last,makecon,(a,l,default)) =
	...
	val (boxedinT,unboxedinT) = 
	    case l
	     of (DATAcon(DATACON{rep=ref(VARIABLE _),...}),_)::_ => 
			(infinity,0)
	      | (DATAcon(DATACON{dcons=ref dcons,...}),_)::_ =>
   ...
   let fun gen(e : lexp, last : bool) : unit =
	let val access =
		  fn PATH (p as (0::_::_)) =>(prof.closureaccess(length p -1);
		   ...
		   | KNOWNFUNC _ => ErrorMsg.impossible "KNOWNFUNC in codegen.access"
        ...
        | FIX (vl,el,b) => 
			...
			val env2 = sib(vl,0,env1)
		        fun frags (lab::rl, e::re, i) =
			     ...
			     frags(rl,re,i+1))
		          | frags (nil,nil,i) = ()
			  | frags _ = ErrorMsg.impossible "unbalanced in codegen"
