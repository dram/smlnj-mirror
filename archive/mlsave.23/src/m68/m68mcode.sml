...
fun emitjump (Jcc(opcode),2,s,d) =
  ...
  | emitjump (MODE,2,s,d) = if d-s-2 < 32768 then emitword(d-s-2) 
					     else raise Illegal
...
fun exg (Direct (DataReg a), Direct(DataReg b)) =
	emitword(49472+a*512+b)
  | exg (Direct (AddrReg a), Direct(AddrReg b)) =
	emitword(49480+a*512+b)
  | exg (Direct (DataReg a), Direct(AddrReg b)) =
	emitword(49544+a*512+b)
  | exg (Direct (AddrReg a), Direct(DataReg b)) =
	emitword(49544+b*512+a)
