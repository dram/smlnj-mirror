...
structure dict =
	...
	with
		...
		let fun search nil = raise LOOKUP
			  ...
			  if ((leq (key,k)) andalso (leq (k,key)))
			     then item
			     else (if leq(key,k)
					 then raise LOOKUP
				 	 else search entries)
...
with
	...
	fun getch (a as (BUF(s,{b,p}))) = 
		 if (!p = (length (!b)))
		   then (b := input s (max(1,min(2048,can_input s)));
			 p := 0;
			 if (length (!b))=0
			    then raise eof 
			    else getch a)
...
fun lexGen(infile) =
    ...
    let val sayln = fn x => (say x; say "\n")
	 ...
	 sayln "\t    let val newchars= if !yydone then \"\" else yyinput 2048";
	 sayln "\t    in if (length newchars)=0";
