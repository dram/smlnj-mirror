...
fun expop () =
    case !nextToken
      of EQUAL => lookFIX(Symbols.EQUALsym)
       | ASTERISK => lookFIX(Symbols.ASTERISKsym)
       | ID s => lookFIX(s)
       | _ => NONfix

fun patop () =
    case !nextToken
      of ASTERISK => lookFIX (Symbols.ASTERISKsym)
       | ID s => lookFIX(s)
       | _ => NONfix
...
fun ident() =
    case !nextToken
      of ID s => (advance();s)
       | ASTERISK => (advance();Symbols.ASTERISKsym)
       | EQUAL => (advance();Symbols.EQUALsym)
...
fun nonfix_ident() =
	if (case !nextToken of
	    ...
	    | ASTERISK => lookFIX(Symbols.ASTERISKsym)=NONfix
	    | _ => false)
	 then getSymbol()
...
fun tyParser(tyconQid) = 
	...
	and ty2() =
	    ...
	    let	fun qid_s(t) =
		    case !nextToken
		      of ID s => qid_s(CONty(lookPatchTYC(getSymbol(),1), [t]))
		       | IDDOT s => qid_s(CONty(tyconQid(1), [t]))
		       | _ => t
	     in qid_s(case !nextToken
		       of LPAREN =>
				...
				then let val tys = t1 :: ty_pc()
					 ...
					 case !nextToken
					   of ID s => CONty(lookPatchTYC(ident(),arity),
						          tys)
					    | IDDOT s => CONty(tyconQid(arity), tys)
			...
			| ID s	=> CONty(lookPatchTYC(ident(),0),[])
			| IDDOT s => CONty(tyconQid(0),[])
...
and eb1x() =
	...
	val edef = if at(EQUAL)
	           then SOME(case !nextToken
			       of IDDOT _ => qid lookFixedEXNinStr
			        | ID s => getEXN(fixExnName(getSymbol()))
...
and ops() =
  let fun ops1() =
	   ...
	   | EQUAL => (Symbols.EQUALsym) :: (advance(); ops1())
	   | ASTERISK => (Symbols.ASTERISKsym) :: (advance(); ops1())
           | _ => nil
   in case ops1()
