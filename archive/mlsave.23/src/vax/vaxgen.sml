...
struct
    ...
    fun path1 [0] = ()
      ...
      | path1 nil = ErrorMsg.impossible "path1 in vax/vaxgen.sml"
    ...
    fun path [i] = (pusha(displace((!offset+i)*4,sp)); inc offset)
      ...
      | path (i::(more as _::_)) = (push(displace((!offset+i)*4,sp));
				    path1 more; inc offset)
      | path nil = ErrorMsg.impossible "path in vax/vaxgen.sml"
    ...
    fun testboxed boxl =
	bbc(immed 0, direct r1, address boxl)
    ...
    fun endrecord () =
	(movl(immed(power_tags*(!savewords)+tag_record), displace(~4,r12));
	 push(direct r12);
	 addl2(immed(4*(!savewords)+4), direct r12);
	 inc offset)
    fun endclosure () =
	(movl(immed(power_tags*(!savewords)+tag_closure), displace(~4,r12));
	 push(direct r12);
	 addl2(immed(4*(!savewords)+4), direct r12);
	 inc offset)
