...
local val b = mkSTR([bogusID],SOME bogusStrStamp,
		    Table.new(): symtable,emptyStrenv)
...
val bogusSIGbody = STRstr{stamp=genStrStamp(),sign=SOME(genSigStamp()),
		    table=Table.new() : symtable,
		    env=emptyStrenv,
...
fun lookSIG id = 
    ...
    handle Unbound => (complain("unbound signature: " ^ name id);
		       bogusSIG)
    ...
    let fun getStr([id],str,ap) = lookLast(str,id,ap)
	  | getStr(id::rest,STRstr{table,stamp,env={s,...},...},ap) =
	      let val STRvar{access=SLOT n,binding,...} = 
		      ...
		      handle Table.Notfound_Table => 
			(if stamp=bogusStrStamp then ()
		         else (complain("unbound intermediate structure: " ^ name id);
...
fun iterFct(gen : unit -> binder*info) : trans list * symtable * strenv =
	...
	local open Intmap
	      exception StampMap
	      val smap = new StampMap :int list intmap
