...
init_gc()
{
 ...
 lastbreak=sbrk(0);
 arenabase=findbreakless(sbrk(0));
...
restart_gc()
{int gamma = (ratio<7?7:ratio>401?401:ratio)>>1;
 arenasize = (old_high-arenabase)*gamma;
 lastbreak = (int)&edata;
 arenasize = (findbreak(arenabase+arenasize)-arenabase);
 arstart = (((arenabase+arenasize+old_high)/2)+3)&(~3);
 freestart = (char *)arstart;
}    

/* for gtest and callgc0 */
int pctest, *sptest;
extern char afterg[];
...
int pctest, *sptest;
extern char afterg[];
int regloc[17]; int sp_offset;

callgc0(arg) int arg;
{int *regptrs[17]; int i;
 int *args = (&arg)-1; int sp;
 int (*inthandle)();
 if (!sp_offset)
  {sp_offset= sptest-args;
    for(i=0;i<sp_offset;i++)
       if ((args[i] & ~ 0xf) == 0x99000)
          regloc[args[i]&0xf]=i;
        else if (args[i]>=pctest && args[i]<pctest+18) regloc[16]=i;
    for(i=0;i<17;i++)
     if (i!=7 && regloc[i]==0)
	    die("Can't find register r%d\n",i);
   args[regloc[16]] = (int)afterg;
   return;
  }

 suspendtimers();
 for(i=0;i<17;i++) regptrs[i] = args + regloc[i];
 sp = (int)(args+sp_offset);
 regptrs[7] = &sp;

 if (!specialgc)	/* don't care if exporting */
 if (*regptrs[16] == (int)after2) *regptrs[16]= (int)before2;
 else  if (*regptrs[16] == (int)after1) *regptrs[16]= (int)before1;
 else  {if(wrongopcode(*(unsigned short *)(*regptrs[16]))) *regptrs[16] -= 2;
	if(wrongopcode(*(unsigned short *)(*regptrs[16]))) *regptrs[16] -= 2;
	if(wrongopcode(*(unsigned short *)(*regptrs[16])))
...
#ifdef	CPS
	*rootsptr++ = regptrs[8];				/* d0 */
	for (i = 11; i <= 13; i++)   *rootsptr++ = regptrs[i];	/* d3..d5 */
	*rootsptr++ = regptrs[15];	/* d7 */
	for (i = 0; i <= 5; i++) *rootsptr++ = regptrs[i];	/* a0..a5 */
	if ((opcode & 0177700) == 021000)	/* movl d1, ... */
		*rootsptr++ = regptrs[9];
#else	/* !CPS */
 ...
 *rootsptr++ = &current;
	*rootsptr=0;
	callgc(*regptrs[7],pc,roots,regptrs[6],storeptrs);
}
#endif	/* M68 */
...
{arenasize = findbreakless(arenabase+new_size)-arenabase;
 if (gcmessages >= ML_INT(2)) chatting("\n[Decreasing heap to %dk]\n",arenasize/1024);
 return arenabase+arenasize;
}

int increase_heapsize()
{arenasize = findbreak(arenabase+new_size)-arenabase;
...
{int oldsize=arenasize;
 int gamma = (ratio<7?7:ratio>401?401:ratio)>>1;
 new_size=(amount_desired+arenasize+arenabase-old_high)*gamma;
...
{int gamma = (ratio<7?7:ratio>401?401:ratio)>>1;
 ...
 if (specialgc) amount_desired = 0;
           else amount_desired = arend-((*arptr) - 4);
...
#endif
{int need_major = 0, cut = gamma; /* 2 <= cut <= gamma */
 if (restart || (new_high+amount_desired) > arenabase+arenasize-arenasize/cut)
    need_major = 1;
 else {new_size=(amount_desired+new_high-old_high)*gamma;
 ...
 if (need_major)
	...
	chatting(" %d%% used (%d/%d), %d msec]",(100*a)/b, a, b, msec);
     collected += 2*((a+512)/1024);
     collectedfrom += 2*((b+512)/1024);
     majorcollections+=2;
    }
     new_size = (amount_desired+new_new_high-old_high)*gamma;
     old_high=arenabase+new_new_high-old_high;
     if (new_size > arenasize)
        {int end = increase_heapsize();
         if ((end-old_high)/2 <= amount_desired) die("\nRan out of memory\n");}
...
findbreak(p) int p; /* p > lastbreak */
{RESTART:
 p = (p + pagesize-1 ) & ~(pagesize-1);
 if (p<=lastbreak) {chatting("\nWarning: tight memory"); return lastbreak;}
 else if (brk(p-4)) {p=(lastbreak+(p-pagesize))/2; goto RESTART;}
 lastbreak=p;
 return lastbreak;
}

findbreakless(p) int p;
{p = (p + pagesize-1 ) & ~(pagesize-1); brk(p-4); lastbreak=p; return lastbreak;}
