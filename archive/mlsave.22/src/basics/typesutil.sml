...
fun prpath(p:int list) = 
    PrintUtil.printClosedSequence ("[",",","]") (fn(i:int) => (print i; ())) p

fun tycStamp(tycon) =
    case tycon
      of TYCON{stamp,...} => stamp
       | INDtyc p =>
	   (print "tycStamp: INDtyc "; prpath p; print "\n";
	    impossible "tycStamp")
...
fun setTycStamp(stamp,tycon) =
    case tycon
      of TYCON{path,arity,eq,kind,...} =>
       ...
       | INDtyc p =>
	   (print "setTycStamp: INDtyc "; prpath p; print "\n";
	    impossible "setTycStamp")

fun tycName(tycon) =
    case tycon
      of TYCON{path=name::_,...} => name
       | INDtyc p =>
	   (print "tycName: INDtyc "; prpath p; print "\n";
	    impossible "tycName")
...
fun setTycPath(path,tycon) =
    case tycon
      of TYCON{stamp,arity,eq,kind,...} =>
	   TYCON{stamp=stamp,path=path,arity=arity,eq=eq,kind=kind}
       | INDtyc p =>
	   (print "setTycName: INDtyc "; prpath p; print "\n";
	    impossible "setTycName")

fun tyconArity(tycon) =
    case tycon
      of TYCON{arity,...} => arity
       | INDtyc p =>
	   (print "tyconArity: INDtyc "; prpath p; print "\n";
	    impossible "tyconArity")
...
fun lookTycPath([i],{s,t}) = (t sub i)
  | lookTycPath(i::rest,{s,t}) =
      (case s sub i
        of STRstr{env,...} => lookTycPath(rest,env)
	 | _ => raise LookTyc)  (* missing substructure *)
  | lookTycPath _ = ErrorMsg.impossible "lookTycPath"

fun tyconInContext env =
    fn (INDtyc path) => lookTycPath(path,env)
     | tyc => tyc

fun printableTycon path (STRstr{env as {t,s},kind=SIGkind{bindings,...},...}) =
  fn (INDtyc (p as [_])) => 
   ...
   | (INDtyc(i::rest)) => 
     let fun scan(STRbind(STRvar{name,binding=INDstr j,...}):: others) =
		if i=j then printableTycon (name::path) (s sub i) (INDtyc rest)
		       else scan others
	   | scan _ = impossible "typesutil.printableTycon"
      in scan bindings
     end
   | tyc => tyc
...
fun rewriteType getTycon =
	...
	let fun mapTycons ty =
	 ...
	 in mapTycons ty
	    handle LookTyc => ERRORty
	end
     in typeInContext
    end

fun typeInContext(ty,env) = 
    if isEmptyStrenv env
    then ty
    else rewriteType (tyconInContext env) ty
...
fun applyPoly(POLYty{sign,tyfun},abs: int) : ty =
      applyTyfun(tyfun,
      		 map (fn {weakness=w,eq=e} => 
		       VARty(ref(META{weakness = if w >= infinity
					         then infinity
						 else w+abs,
				     depth = infinity,    
				     eq = e})))
...
fun printPath(p: Symbol.symbol list) = printSequence "." printSym p
