...
and translate (INTexp i) = INT i
  ...
  | translate (VARexp (ref(VALvar{access=INLINE P.eql,vtype=ref ty,...}))) = 
	      Equal.equal(getEqualElem ty)
  | translate (VARexp (ref(VALvar{access=INLINE P.neq,vtype=ref ty,...}))) = 
	      composeNOT(Equal.equal(getEqualElem ty))
  | translate (VARexp (ref(VALvar{access=INLINE P.:=,vtype=ref ty,...}))) = 
	      PRIM(unboxedAssign ty)
  | translate (VARexp (ref(VALvar{access=INLINE P.update,vtype=ref ty,...}))) = 
	      PRIM(unboxedUpdate ty)
  | translate (VARexp (ref(VALvar{access=INLINE(n),vtype=ref ty,...}))) = PRIM n
