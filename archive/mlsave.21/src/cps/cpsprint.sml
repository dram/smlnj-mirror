	...
	let fun space 0 = () | space k = (say " "; space(k-1))
	    fun nl() = say "\n"
	     ...
	     fn RECORD(vl,v,c) =>
	      ...
	      | PRIMOP(i,vl,wl,[c]) =>
		   (space n; say(nth(inLineNames,i)); say "("; sayvlist vl;
		    say ") -> "; sayvlist wl; nl(); f c)
	      | PRIMOP(i,vl,nil,[t,f]) =>
	          (space n; say "if "; say(nth(inLineNames,i));
	      ...
	      | PRIMOP(i,vl,wl,cl) =>
	          (space n; say "case "; say(nth(inLineNames,i));
			 say "("; sayvlist vl; say ") -> ";
			sayvlist wl; say " of\n";
		   let fun g(i,c::cl) =
			(space(n+1); say(makestring(i:int)); say " =>\n";
			    indent (n+3) c; g(i+1,cl))
			 | g(_,nil) = ()
		    in g(0,cl)
		   end)
