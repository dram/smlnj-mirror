...
*)
  ...
  local 
      open Intmap
      exception Modname
      val m : string intmap = new Modname
      val lookup = map m
  ...
  in
	...
	(reset();
	 ...
	 load "boot/dummy.sml";
	 EnvAccess.setPervasives
		[EnvAccess.lookSTR (Symbols.stringToSymbol "Core")];
	 load "boot/perv.sig";
  ...
  in
    fun topLevel() =
	    ...
	    val [c,i,m] = bootEnv false
	    (* bind runtime structure Initial *)
	    val {core,math,initial} = !System.pstruct
	    val _ = (bind(c,core); bind(i,initial); bind(m,math))
	    val _ = Env.commit()
	    fun toploop() =
		let val absyn = parse()
