...
val intequalNum = position "ieql"
val realequalNum = position "feql"
...
fun atomeq tyc =
	  if eqTycon(tyc,!intTycon) 
	     ...
	     orelse eqTycon(tyc,!arrayTycon) then SELECT(intequalNum, VAR 0)
	  else if eqTycon(tyc,!realTycon) then SELECT(realequalNum, VAR 0)
	  else if eqTycon(tyc,!stringTycon) 
	    then let val VALvar{access=PATH p,...} = 
			EnvAccess.lookVARinBase stringequalSym
		  in translatepath p
		 end
	  else raise Poly
