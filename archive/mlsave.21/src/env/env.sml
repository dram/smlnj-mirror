...
functor EnvFunc (Bind : BIND) : ENV = struct
  ...
  in
    ...
    val layers = ref(nil: layer list);
    val remarks = ref(nil: remark list);
    val markCount = ref 0;
	...
	let fun save rems =
		case rems
		  of ENTRY i :: r =>  
		      (* could do all in s using Table.app (except for order?)
		         could also just look at layers instead of remarks *)
		       let val OPEN s ::_ = !layers
