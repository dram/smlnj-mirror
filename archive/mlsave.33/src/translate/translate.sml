...
val [bogusID,matchsym,bindsym] =
	    map Symbols.stringToSymbol ["bogus","Match","Bind"]

fun raisematch() = (WILDpat,RAISE(CON(!EnvAccess.exnMatch,unitLexp)))
...
fun polyequal() = translatepath(!EnvAccess.polyequalPath)
...
and makedec (VALdec vbl) =
    fold (fn (VB{pat=VARpat(VALvar{access=INLINE(_),...}),...},b) => b
	   ...
	   | (VB{pat,exp,...},b) => 
	      (fill pat; 
	       APP(MC.bindCompile 
		   [(pat,b),(WILDpat,RAISE(CON(!EnvAccess.exnBind,unitLexp)))],
	           translate exp)))
	 vbl
...
fun transStrb(STRB{def,thin,strvar=STRvar{name,...},...})=thinStr(transStr def,thin)
