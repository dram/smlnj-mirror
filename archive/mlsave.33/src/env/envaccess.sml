...
val getDebugVar = ref(mkVALvar(Symbols.stringToSymbol "getDebug",
...
fun setCore(STRvar{access=PATH p,binding,...}) =
    let fun extractPath name = 
	    let val VARbind(VALvar{access=PATH p,...}) =
		      lookVARCONinStr(binding, Symbols.stringToSymbol name, p)
	     in p
	    end
     in exnBind := lookCONinStr(binding, Symbols.stringToSymbol "Bind", p);
	exnMatch := lookCONinStr(binding, Symbols.stringToSymbol "Match", p);
	...
	toplevelPath := extractPath "toplevel";
        getDebugVar := let val VARbind x = lookVARCONinStr(binding,
				    Symbols.stringToSymbol "getDebug", p)
			in x
		       end
    end
