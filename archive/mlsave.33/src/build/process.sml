...
struct
 ...
 (* debugging aid--print the slots of a structure
  ...
  fun buildlist (_,_,VARbind(VALvar{name,access,...})) =
		  usl := {name=name,access=access} :: !usl
    | buildlist (_,_,STRbind(STRvar{name,access,...})) =
		  usl := {name=name,access=access} :: !usl
  ...
  fun getCore () = if !is_core then [] else tl(!EnvAccess.stringequalPath)
  ...
  fun bootEnv (loader:string -> unit) =
      (primeEnv();
       ...
       let val svCore as STRvar{access=PATH[lvCore],...} =
	         lookSTR (Symbols.stringToSymbol "Core")
        in setCore(svCore);
	   load "boot/perv.sig";
	   ...
	   let val STRvar{access=PATH[lvMath],...} =
		     lookSTR (Symbols.stringToSymbol "Math")
	       and svInitial as STRvar{access=PATH[lvInitial],
				       binding=strInitial as STRstr{table,...},...} =
		     lookSTR (Symbols.stringToSymbol "Initial")
	       and STRvar{binding=STRstr{table=otable,...},...} =
		     lookSTR (Symbols.stringToSymbol "Overloads")
	       val sigs = map (fn s => lookSIG(Symbols.stringToSymbol s))
			      ["REF","LIST","ARRAY","BYTEARRAY","BASICIO",
			       "IO","BOOL","STRING","INTEGER","REAL","GENERAL"]
	       val NJsymbol = Symbols.stringToSymbol "NewJersey"
	    in Env.reset();
