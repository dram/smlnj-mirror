...
val stringToSymbol = Symbols.stringToSymbol

(* Profiling globals *)
val profiling = System.Control.Profile.profiling
val profileList = ref([]: (Access.lvar * string) list)
		
val anonSym = stringToSymbol "anon"

val intreftype = CONty(refTycon,[intTy])
val alpha = VARty(mkTyvar(IBOUND 0))

fun topCCa() = VALvar{name = stringToSymbol "toplevel.CC",
		      access = PATH(!EnvAccess.toplevelPath),
		      vtype = ref intreftype}
val assop = VALvar{name = stringToSymbol "unboxedassign",
	           access = INLINE P.unboxedassign,
		   vtype = ref(tupleTy[intreftype,intTy] --> unitTy)}
val derefop = VALvar{name = stringToSymbol "!",
		     access = INLINE P.!,
		     vtype = ref(CONty(refTycon,[alpha]) --> alpha)}
val addop = VALvar{name = stringToSymbol "iadd",
		   access = INLINE P.+,
		   vtype = ref(tupleTy[intTy,intTy] --> intTy)}
fun currentvar() = VALvar{name = stringToSymbol "current",
		          access = PATH(!EnvAccess.currentPath), 
			  vtype = ref intreftype}

fun tmpvar str = 
	let  val lvar = mkLvar()
	     val name = stringToSymbol str
	in (VALvar{name=name, access=LVAR(lvar), vtype = ref UNDEFty},
	    VALvar{name=name, access=PATH[lvar], vtype = ref UNDEFty})
	end
...
fun instrdec(sp as (names,ccvara), VALdec vbl) = 
    let fun instrvb (vb as VB{pat=VARpat(VALvar{access=INLINE _,...}),...}) = vb
	  ...
	  | instrvb (VB{pat as VARpat(VALvar{access=LVAR v,name,...}),exp,tyvars}) =
	      VB{pat=pat,exp=instrexp(name::clean names,ccvara)false exp,
								  tyvars=tyvars}
  ...
  | instrdec(sp as (names,ccvara), VALRECdec rvbl) = 
    let fun instrrvb (RVB{var=var as VALvar{access=LVAR v, name,...},
							exp,resultty,tyvars}) =
               RVB{var=var, exp=instrexp (name::clean names, ccvara) false exp, 
 ...
 let fun istail tail =
	 ...
	 val rec instr:(exp->exp) =
		...
		let fun dot (a,[z]) = Symbol.name z :: a
		    ...
		    val ccsym = stringToSymbol (name^".CC")
