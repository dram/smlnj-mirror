...
{int fd= -1; char *p, *q; int i;
 {struct datalist *d; mlstring ss=(mlstring)mak_str(s);
  ...
  p=(char *)saved_dataptr-4;
  saved_dataptr += sizeof(int);
  while (i=read(fd,saved_dataptr-4,4096)) saved_dataptr+=i;
  q=(char*)saved_dataptr-4;
  while ((saved_dataptr-4) & 3) saved_dataptr++;
