...
struct
 local open InLine Ref PreString PrimTypes in
  ...
  local
    val buf = Assembly.A.create_b 8
    val TIOCGETP : string =
	  if isV9 then cast 29704
	  else InLine.!(cast (if bigEndian
			      then "\064\006\116\008"
			      else "\008\116\006\064"))
		     (* The above will cause a garbage collection error
			 in systems with more than a gigabyte. *)
  in  fun isatty filid = 
	  case Assembly.A.syscall(54,[cast filid,TIOCGETP, buf],3)
	    of ~1 => false
	     | _ => true
  end
	    ...
	    let open Assembly.A
		 ...
		 of READ =>
		     (case syscall(5,[name,cast 0],2)
		       of ~1 => raise Assembly.SystemCall "open"
			| n => n)
		  | WRITE =>
		     (case syscall(8,[name, cast 438],2)
		       of ~1 => raise Assembly.SystemCall "creat"
			| n => n)
		  | APPEND =>
			...
			| n => (case syscall(19,[cast n,cast 0,cast 2],3)
				 of ~1 => (close n; raise Assembly.SystemCall "lseek")
				  | _ => n))
	    end          
      else fn (name,mode) =>
	    let open Assembly.A
  ...
  fun open_out name = open_o WRITE name
			handle Assembly.SystemCall s => raise Io("open_out: " ^ s)
  ...
  fun read(filid, buf) =
      case Assembly.A.syscall(3,[cast filid, buf, cast(slength buf)],3)
       of ~1 => raise Io "input: read failed"
	| n => n
  ...
  fun write(filid,s,l) = 
      case Assembly.A.syscall(4,[cast filid, s, cast l],3)
       of ~1 => raise Io "output: write failed"
	| n => n
  ...
  fun close_out ({closed = ref true,...} : outstream) =
	raise Io "close_out: closed stream"
  ...
  fun input(f as {filid,pos,len,buf,closed,...} : instream) =
	  ...
	  fun get i =
		 ...
		 if ieql(i,1) then
		    ...
		    raise Io "input: negative character count"
		 else let val s = substring(buf,!pos,i)
		       in pos := !pos+i; s
		      end
	      else if !closed then raise Io "input: closed stream"
	      else let val remaining = !len - !pos
		       val s = substring(buf,!pos, remaining)
		       val j = i - remaining
		   in  if (j >= bufsize) andalso (filid > ~1)
		       then let val k = if ieql(j,bufsize) then j
					   else let val c = fionread filid
						 in if j<c then j else c
						end
			     in if k<=1 then s ^ get k
				else let val s' = biginput(filid,k)
				      in if ieql(slength s', 0)
					 then s ^ s'
					 else s ^ s' ^ get(k - slength s')
				     end
			    end
		       else (filbuf f;
  ...
  fun input_line({closed = ref true,...} : instream) =
	raise Io "input_line: closed stream"
    | input_line(f as {pos,len,buf,...}) =
	let val l = !len
	    val filbuf = fn f => (filbuf f
				  handle Assembly.SystemCall s =>
				  raise Io("input_line: " ^ s))
  ...
  fun output(f as {filid,buf,pos,tty,closed,...} : outstream) =
      let fun put s =
	      if !closed then raise Io "output: closed stream"
  ...
  fun can_input ({closed = ref true,...} : instream) =
	raise Io "can_input: closed stream"
    | can_input {filid = ~1,pos,len,...} = !len - !pos
    | can_input {filid,pos,len,...} =
	!len - !pos + fionread filid
		      handle Assembly.SystemCall s => raise Io("can_input: " ^ s)
  ...
  local val buf = Assembly.A.create_b 8
   in fun pipe() =  (* won't work for more than 256 file descriptors! *)
	  (case Assembly.A.syscall(42,[buf],1)
	    of ~1 => raise Assembly.SystemCall "pipe"
  ...
  fun blast_write({filid,...} : outstream, obj) = (blas(filid,obj); ())
...
local open InLine
 in fun imakestring i =
	  if i<0 then PreString.^("~", imakestring(~i))
	  else if i<10 then InLine.cast(InLine.cast "0" + i)
	  else let val j = i div 10
	       in  PreString.^(imakestring j, imakestring(i-j*10))
	       end
end
  ...
  struct
	...
	structure Print : PRINTCONTROL =
	  ...
	  struct
	    ...
	    fun setitimer t = Assembly.A.syscall(83,[cast 1,t,cast 0],3)
		...
		field(decimal(imakestring (n div k),w1)
	    ...
	    fun printReport f l = 
		 ...
		 List.app (fn (n,m,s,cumm) =>
			    (pr(decfield(m*10000,ticks,2,6));
			     pr(decfield(2*(ticks-cumm+m),1,2,9));
			     pr(field(imakestring n,10));
	    ...
	    val getvalue = cast (
		  fn (obj as unboxed) => INTval(cast obj)
		   | obj => 
			...
			else if ieql(tag,tag_bytearray) then STRINGval(cast obj)
    ...
    open PreLim
    val version = "Standard ML of New Jersey, Version 0.33, 1 April 1989"
	...
	case Unsafe.syscall(12,[PreString.^(s,c00)],1)
	 of ~1 => raise Assembly.SystemCall "chdir"
	  | _ => ()
