...
local val save = (!saveLvarNames before saveLvarNames := true)
      val closure = namedLvar(Symbols.stringToSymbol "closure")
...
fun printEnv(Env e) =
  let val seen : lvar list ref = ref nil
      ...
      val ip = fn i => pr (Integer.makestring i)
      fun sp() = pr " "
...
val lookup =
  fn (env as Env e,target) =>
	  ...
	  fun bfs((Closure{functions,
		...
		fun cnt l =
		  let val more : (object * int list) list ref = ref nil
		      fun save c = more := c :: !more
		      fun cnt'(nil,i) = bfs(m @ !more)
		        | cnt'((v,c as Closure{stamp,...})::t,i) =
			      if target=v then (element i,0,c)
			      else (if saw stamp then () else save(c,element i);
			            cnt'(t,i+1))
		        | cnt'((v,Value)::t,i) =
			      if target=v then (element i,0,Value)
			      else cnt'(t,i+1)
			| cnt'((_,Function _)::_,_) =
				error "Function in closure in lookup"
		  in  cnt' l
		  end
		fun fns(nil,i) = cnt(contents,i)
		  | fns((v,l)::t,i) =
			if target=v
...
fun link(env,free) =
  ...
  in  if length direct = length contents then direct
		...
		let fun getc ((m as (v,Closure _))::_) = m
		      | getc (_::tl) = getc tl
		      | getc nil = error "No closure in closureStrat"
		    val c = getc (rev l)
		in  c::direct
		end
  end
...
(* CAUTION: should be merge(free,[c]); works because free is always nil. *)
		if callc then (v,Function{label=label,free=free@[c]})
	...
	= map (fn{v,free,callc,args,body,env=env',label} =>
		let val env =
		    ...
		    val _ = COMMENT(fn () => (print "\nEnvironment at known ";
					      vp v; print ":\n";
					      printEnv env))
		    val args = args @ free @ if callc then [cname] else nil
...
in  case contents
      of nil => (fn ce => ce,registerFrags,fold augment fnMap env)
       | _ =>
