...
fun record(vl, z) =
	...
	fun f(i,nil) = ()
	  ...
	  | f(i,(direct r, OFFp j)::rest) = (V.moval(displace(j*4,r),
						 ...
						 f(i-1,rest))
	  | f(i,(x,p)::rest) = (V.movl(x,ptrtemp); f(i,(ptrtemp,p)::rest))
      in f(len - 2, rev vl);
	 V.movl(dataptr,z);
...
fun mstoreindexl(v,w,immed 1) = 
    (V.movl(storeptr, defer dataptr);
     V.movl(v, defer w);
     V.movl(w, displace(~4,dataptr'));
     V.movl(dataptr,storeptr);
     V.addl2(immed(8),dataptr))
  | mstoreindexl(v,w,i) = 
    (V.ashl(immed ~1,i,arithtemp);
     V.movl(storeptr, defer dataptr);
     V.movl(v,index(defer w, arithtemp'));
     V.moval(index(defer w, arithtemp'), displace(~4,dataptr'));
     V.movl(dataptr,storeptr);
     V.addl2(immed(8),dataptr))
fun fetchindexl(v,w,i) = 
	    (V.ashl(immed ~1,i,arithtemp);
