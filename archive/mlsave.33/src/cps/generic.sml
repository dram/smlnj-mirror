...
let 
  ...
  (* generate a new code label *)
    and genlab(lab, cexp, NONE) = (root1 := root cexp; define lab; gen cexp)
    |   genlab(lab, cexp, SOME closureReg) =
        (root1 := root cexp; define lab; beginStdFn(closureReg); gen cexp)
	  ...
	  | APP(f,args) =>
	    (case (map regbind args,
	      ...
	      of (args',KNOWN(_,ref(SOME formals))) =>
	       ...
	       | (args', KNOWN((vl,cexp), r as ref(NONE))) =>
		    	  (r := SOME(allocparams(args',vl));
		    	   comment(Access.lvarName f ^ ":\n");
			   genlab(regbind f, cexp, NONE))
	  ...
	  | SWITCH(v,l) => 
		let val lab = newlabel()
		    ...
		    fun f(i, s::r) = (emitlab(i, s); f(i+4, r))
		      | f(_, nil) = ()
		    fun h(lab::labs, e::es) = (genlab(lab,e,NONE); h(labs,es))
		      | h(nil,nil) = ()
...
(* the next case must be done (by all machines) with v and x in
   root registers (for offset computations in "boot") *)
	 ...
	 alloc(x,e,any, fn x' => 
	   ...
	   addl3(immed(~1),regbind v,arithtemp2);
           ashlt(arithtemp, arithtemp2, x');
	   orb(immed 1, x', x');
	   gen e)))
       | P.rshift => (fn ([v,w],[x],[e]) =>
	 alloc(x,e,regbind v, fn x' => 
	 ...
	 in case (isimmed v', isimmed w') of
       ...
       | P.* => (fn ([v,w],[x],[e]) =>
          alloc(x,e,any,fn x' => (arithprof 3; ashr(immed 1, regbind v, arithtemp);
	  ...
	  alloc(x, e,any, fn x' =>
			(arithprof 4;
			 ...
			 ashr(immed 1, regbind w, arithtemp2);
			 divl2(arithtemp2,arithtemp);
			 ashlt(immed 1, arithtemp, arithtemp);
			 orb(immed 1, arithtemp,x');
			 gen e)))
       | P.! => (fn ([v],[w],[e]) => gen(SELECT(0,v,w,e)))
       | P.:= => (fn ([v,w],[],[e]) =>
	     (mstoreindexl(regbind w, regbind v, immed 1); gen e))
       | P.unboxedassign => (fn ([v,w],[],[e]) =>
              (storeindexl(regbind w, regbind v, immed 1); gen e))
       | P.~ => (fn ([v],[w],[e]) =>
	 alloc(w,e,any,fn w' => (arithprof 0;subl3(regbind v,immed 2,w');gen e)))
       | P.makeref =>
	 (fn ([v],[w],[e]) =>
	    alloc(w, e,any, fn w' =>
       ...
       | P.update => (fn ([a, i, v], [], [e]) =>
	(arithprof 1;
	 mstoreindexl(regbind v, regbind a, regbind i);
	 gen e))
...
(*	     orb(immed 1, arithtemp, w');
	this didn't work on the mc68020, dammit! *)
       ...
       | P.ordof => (fn ([s,i], [v], [e]) =>
		...
		ashl(immed 1,arithtemp2,arithtemp2);
       ...
       | P.boxed => (fn ([x],[],[a,b]) =>
		    let val lab = newlabel()
		     in bbs(immed 0, regbind x, lab); gen a; genlab(lab,b,NONE)
		    end)
       | P.gethdlr => (fn ([],[x],[e]) =>
       ...
       | P.fmul =>  (fn ([x,y], [z], [e]) =>
		alloc(z,  e,any, fn z' =>
		 (mulg3(regbind x, regbind y, z');
		  gen e)))
       | P.fdiv =>  (fn ([x,y], [z], [e]) =>
		alloc(z,  e,any, fn z' =>
		 (divg3(regbind x, regbind y, z');
		  gen e)))
       | P.fadd => (fn ([x,y], [z], [e]) =>
		alloc(z,  e,any, fn z' =>
		 (addg3(regbind x, regbind y, z');
		  gen e)))
       | P.fsub => (fn ([x,y], [z], [e]) =>
		alloc(z,  e,any, fn z' =>
		 (subg3(regbind x, regbind y, z');
		  gen e)))
