...
val beginStdFn : EA -> unit (* Mark beginning of standard function *)
...
val fetchindexl : EA * EA * EA -> unit   (* fetchindexb(x,y,z) fetches a word:
					   y <- mem[x+2*(z-1)] *)
val storeindexl : EA * EA * EA -> unit   (* storeindexb(x,y,z) stores a word:
	    					mem[y+2*(z-1)] <- x *)
val mstoreindexl : EA * EA * EA -> unit   (* mstoreindexb(x,y,z) stores a word
					and records y+2*(z-1) in store-list *)

val ashl : EA * EA * EA -> unit  (* shift left: count, src, dest;
				     shift count is non-negative *)
val ashlt : EA * EA * EA -> unit  (* shift left: count, src, dest;
				     shift count is non-negative *)
