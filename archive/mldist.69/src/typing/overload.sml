...
(* overload.sml *)
(* functor Overload (structure PrintType:PRINTTYPE): OVERLOAD = struct
...
fun resolveOverloaded env  =
 let fun resolveOVLDvar(rv as ref(OVLDvar{name,options,...}),err,context) =
	(let fun findFirst({indicator, variant}::rest) =
		 ((softUnify(applyPoly(indicator,0,0,infinity), context), variant, rest)
		   handle SoftUnify => findFirst(rest))
	       | findFirst(nil) = 
		   (err COMPLAIN("overloaded variable \"" ^ Symbol.name(name) ^
			     "\" not defined at type:");
		    print "   ";
		    resetPrintType(); printType std_out env context; newline();
		    raise Overld)
	     fun findSecond({indicator, variant}::rest) =
		 ((rollBack(softUnify(applyPoly(indicator,0,0,infinity), context));
		   err COMPLAIN("overloaded variable \"" ^ Symbol.name(name) ^
			     "\" cannot be resolved");
		   raise Overld)
