...
functor Importer(structure FilePaths: FILEPATHS
		...
		: IMPORTER =
struct
  open PrintUtil Access Basics Stampset EnvAccess NewParse
 ...
 (* impliedPath: derived from FilePaths.impliedPath, but catches
  ...
  datatype statModule =
      STATmodule of {env: Basics.env, lvars: lvar list}
  ...
  fun importModule(STATmodule{env,lvars}) : (env * lvar list) = 
	  ...
	  fun renBinding(SIGbind(SIGvar{name,binding})) =
	    ...
	    | renBinding(FCTbind(FCTvar{name,access=PATH[lvar], binding})) =
       ...
       in (Env.map renBinding env,newlvars)
      end
      ...
      let val {newPath, validName} = impliedPath(path, name)
       in case (timeFile(validName ^ ".sml"), timeFile(validName ^ ".bin"))
	    of (SOME srcTime, SOME binTime) => 
	      ( srcTime < binTime	(* binary is up to date *)
...
-- this doesn't work right *)
 ...
 fun getModule(pervasives, parents, indent, path, name) =
  ...
  fun getAndExecModule pervasives filename : Basics.env =
    if !System.Control.interp then fail("interpreter cannot import") else
    let val (statModule, compDynModule) =
	...
	PrintDec.printBindingTbl newenv;
	newenv
    end
