...
fun Xcaller (time:when) : (wherewhen*wherewhen) outcome =
    (* return top of function containing time, caller of this function *)
  protect (fn () =>
    if !running then
      let fun lastfunc(0,{evn,...}) = (0,evn,0)
	    | lastfunc(t,{evn,lbt,...}) =
		 let fun f (FNev (RULE(pat,_))::_,n) = (t,evn,n)
		       | f (HANDLEev (RULE(pat,_))::_,n) = (t,evn,n)
		       | f (evt::rest,n) = f(rest,n+1)
		       | f (nil,_) = lastfunc(lbt,evdataAt lbt)
		 in f (eventsFor evn,0)
		 end
          val (t,evn,n) = lastfunc(time,evdataAt time)
          val (t',evn') = if t > 0 then
	                    (t-1,#evn(evdataAt(t-1))) 
			  else (0,0)
      in checkint(((evn,n),t),((evn',0),t-1))
      end
    else NOTRUNNING)
...
fun XprintVal(boundval:value,vtype:valtype) : unit =
	 PrintVal.printVal(!debugEnv) (boundval,vtype,pdepth)
  ...
  (case nth(eventFor evn,c) of
   ...
   | FCTENTev(FCTB{fctvar=FCTvar{access,name=fname,...},
	param=STRvar{name=pname,...},def,...}) =>
	 (print "functor "; printSym fname; PrintBasics.printAccess access;
	  print " ("; print(formatQid pname); print ") = "; nlindent(indent+4);
...
fun XgetVal (name:string) : valinfo = BOUND(DebugBindings.getVal name) 
                                           handle Unbound => UNBOUND

(*  old getVal stuff follows:
local
...
fun checkInStruct (symbol,lookf) (n:string, s:Basics.Structure) =
     let val res = 
       case s of 	
         STRstr {table,...} => ((((lookf table (symbol n); true)
...
val checkValInStruct = checkInStruct (Symbol.varSymbol,EnvAccess.lookVARCON)
val checkStructInStruct = checkInStruct (Symbol.strSymbol,EnvAccess.lookSTR)
...
val _ = System.Control.Debug.getDebugf := 
	 (System.Unsafe.cast)  (fn firstevn =>
		(times,eventtimeArray firstevn,!break,
			HistoryStore.hcreater,System.Unsafe.Weak.weak,
...
val _ =
  ...
  in System.Control.Debug.interface := 
   (fn 
     ...
     | 15 => System.Unsafe.cast (DebugUtil.debugdebug,DebugUtil.sizing)
     | 16 => System.Unsafe.cast (HistoryStore.updatedAList,HistoryStore.updatedRList,HistoryStore.createdList,HistoryStore.hcreatea,HistoryStore.hcreater)
     ...
     | 28 => System.Unsafe.cast (HistoryStore.activate,HistoryIO.activate)
     | 29 => System.Unsafe.cast Xprovoke
     | 30 => System.Unsafe.cast Xcaller
     | 31 => System.Unsafe.cast Xeffect
     | q => old_interface q)
  end (* let ... *)
