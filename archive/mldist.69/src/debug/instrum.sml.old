signature DEBUGINSTRUM = sig
   val instrumDec: DebugStatic.filename * Absyn.dec -> Absyn.dec
   val commit: unit -> unit
   val reset:unit -> unit
   val hideFile: DebugStatic.filename -> unit
   val eventsAfter: DebugStatic.filename * DebugStatic.charno -> 
		(*	(DebugStatic.evnum * DebugStatic.evindex) list list *)
			DebugStatic.evnum list
   val eventsBefore:DebugStatic.filename * DebugStatic.charno -> 
		(*	(DebugStatic.evnum * DebugStatic.evindex) list list *)
			DebugStatic.evnum list
   val locOfEvent: DebugStatic.event -> DebugStatic.charno
...
datatype eventstatus =  (* info about events executed in evaluating expression,
    ...
    FIRST_EV of evnum   (* the first event executed reports all sites/vars,
...
type binddata = exp *         (* representing time of last binding event *)
                evnum *       (* event number of last binding event *)
...
fun instrexp(b as (btexp:exp,   (* last binding time *)
		   bev:evnum,   (* last binding event number *)
  ...
  let
    fun instr (dec as MARKdec(VALdec vbl,_,_)) =
      ...
      | instr (dec as MARKdec(DATATYPEdec _,_,_)) =
            ((btexp,bev,TYPEev dec::bsites,bvars),dec)
      | instr (dec as MARKdec(ABSTYPEdec _,_,_)) =
            ((btexp,bev,TYPEev dec::bsites,bvars),dec)
...
in
   ...
   LOCALdec(VALdec [VB {pat = TUPLEpat [VARpat timesvar,
					VARpat eventtimes,
					VARpat breakentry,
					VARpat hcreater,
					VARpat weakvar,
