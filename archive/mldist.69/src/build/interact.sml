 ...
 open Linkage
    ...
    (* interesting cases:
       (outer loop): baseEnv = std. pervasives
		   ...
		   lookupLvar = std lookup
       (debugger use): baseEnv = dbg. pervasives
                   topEnv = everything else so far
		   lookupLvar = std lookup
       (inner loop): baseEnv = std. pervasives + 
                               everything prior to inner loop +
			       special lookup
		     topEnv = everything defined in inner loop.
		     lookupLvar = std lookup + special lookup
    *)
	...
	let val _ = print("[opening " ^ fname ^ "]\n")
	    ...
	    val savedBaseEnv = !baseEnv
	    fun getBaseEnv () =
	      let fun getEnv d =
	            EnvAccess.openStructureVar 
		                   stdPervEnv
				   (EnvAccess.lookSTR 
				      (Env.atop(!topEnv,!baseEnv))
				      (Symbol.strSymbol d))
	      in case dbglevel of
	 	   FULLDEBUG d => getEnv d
		 | LIVEDEBUG d => getEnv d
                 | NODEBUG => stdPervEnv
	      end
	in hideFile();
	   baseEnv := Env.atop(getBaseEnv(),!baseEnv);
	   (* kludge: we just hide standard pervasives with debugger version*)
           toploop(parser,inputSource,dbglevel)
              handle exn =>
	        (baseEnv := savedBaseEnv;
		 case exn
