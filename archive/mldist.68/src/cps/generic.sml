	...
	*) 
	let open SortedList
	    ...
	    val unusedfpr = difference (uniq savedFPRegs,
					uniq (assigned_FPregs assign1))
	    val final = pass2(assign1, unusedfpr)
	in
	 ...
	 *)
	  ...
	  fun nofpr_handle () =
	      let val _ = if !System.Control.CG.misc4 = 16
			      then print "force_fpgetscratch/nofpr_handle\n"
			  else ()
		  val dups = duplicates free
	      in 
		  case pref_dup dups
		    of SOME(v,fp,gp) => 
    ...
    and parallel_gen (f1, f2) =
	let val bindings = Intmap.intMapToList regbindtable
	in f1(); app (fn kv => addregbinding kv) bindings; f2()
	end
	  ...
	  | APP(func as LABEL f, args) =>
	    (case know f
	      of KNOWN(_, ref(SOME formals)) =>
	       ...
	       | KNOWN(_, ref(SOME formals)) =>
			  shuffle(func, args, formals)
	       | KNOWN((vl,cexp), r as (ref NONE)) =>
		     let val formals = 
			     if (!CG.argrep) 
			     then let val pref = 
			                  if !System.Control.CG.floatreg_params 
					  then preferred_register_asgn(vl,cexp)
					  else map (fn _ => GPReg) vl
				  in allocparams(args,vl,pref)
				  end
			     else
				 stupidargs(func,args,vl,map (fn _ => GPReg) vl)
		     in r := SOME formals;
...
(*		    align();   temporarily removed so 68020 will work. *)
	 ...
	 alloc(w,any,fn w' => (M.subt(gpregbind v,immed 2,w'); gen e))
	   ...
	   let val false_lab = newlabel()
		...
		parallel_gen(fn () => gen d, fn () => genlab(false_lab, e))
	   end
	  ...
	  alloc(x,any, fn x' => tempreg(x', fn x'' =>
       ...
       | PRIMOP(P.boxed, [x],[],[a,b]) =>
		    let val lab = newlabel()
			 ...
			 parallel_gen(fn () => gen a, fn () => genlab(lab,b))
		    end
...
(* still to implement: 
	...
	in branch(test,gpregbind v, gpregbind w, lab); 
	    parallel_gen(fn () => gen d, fn () => genlab(lab, e))
	end
