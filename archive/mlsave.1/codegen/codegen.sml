...
fun translatepath [v] = VAR v
  ...
  fun genswitch (gen,last,makecon,(a,l,default)) =
	...
	val (BinT,UinT) = 
	    case l
	     of (DATAcon(DATACON{tycon=DATAtyc{dcons= ref dcons,...},...}),_)::_ =>
   ...
   let fun gen(e : lexp, last : bool) : unit =
	...
	| APP (f,a) => (let val true = !CGoptions.tailrecur
	  ...
	  | FN (v,b) => let val (env1,close) = makenv ([v],[b],1,env) 
			...
			machine.startrecord(1 + length(close));
...
(*		         end   *)
  ...
  | codegen(L, STRING s, _) =
      (machine.definelabel(L); machine.stringconst(s))
  | codegen( L : Label, REAL r, env: Env) =
      (machine.definelabel(L); machine.realconst(r))
  | codegen(_,e,_) = ErrorMsg.Impossible "83785 in codegen"
