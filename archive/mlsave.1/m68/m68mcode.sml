signature BASICM68 = sig
    ...
    datatype JumpKind = Jcc of int | LEA of int | FJcc of int
...
exceptionx TooBig (* pc relative addressing only has 16-bit displacement *)
...
fun reg (Direct(DataReg d)) = d
  ...
  | reg (Abs i) = (case sizeint i of
			...
			| Long => 1)
  | reg (Immed _) = 4
  | reg (Address _) = raisex BadLabelUse
...
fun mode (Direct(DataReg _)) = 0
  ...
  | mode (Abs _) = 7
  | mode (Immed _) = 7
  | mode (Address _) = raisex BadLabelUse
...
fun emitext (Immed i) = emitlong i
  ...
  | emitext (Address _) = raisex BadLabelUse
...
fun subl (_,Immed _) = raisex illegal
  ...
  | subl (src as (Immed i),dest) =
	if i <= 8 andalso i >= 1 then
		...
		(emitword(1152 + reg dest + 8 * mode dest);
		 emitext src;
		 emitext dest)
  (* general SUB *)
...
fun movb (_,Immed _) = raisex illegal
  (* labels not implemented *)
  | movb (Address _,_) = raisex illegal
  | movb (_,Address _) = raisex illegal
  (* CLR *)
...
fun shift dir =
 let val dr = dir * 256
	 ...
	 if  i<1 orelse i>7 then raisex illegal
	   else emitword(57472+dr + 512*i + d)
      | F (Immed 1, Direct _) = raisex illegal
      | F (Immed 1, dest) =
...
fun pea (Direct _) = raisex illegal
  ...
  | pea (Address L) = (jump(LEA(18496),L))
  | pea (src) =
...
fun lea (Direct _,_) = raisex illegal
  ...
  | lea (Address L,Direct(AddrReg a)) =
	jump(LEA(16832+512*a),L)
  | lea (src,Direct(AddrReg a)) =
	(emitword(16832 + 512 * a + 8 * mode src + reg src);
	 emitext src)

fun jne (Address L) = jump (Jcc(26112),L)
fun jeq (Address L) = jump (Jcc(26368),L)
fun jgt (Address L) = jump (Jcc(28160),L)
fun jge (Address L) = jump (Jcc(27648),L)
fun jlt (Address L) = jump (Jcc(27904),L)
fun jle (Address L) = jump (Jcc(28416),L)

fun jra (Address L) = jump (Jcc(24576),L)
  | jra (arg as (Displace(AddrReg _,_))) = 
	    (emitword (20160 + 8 * mode arg + reg arg); emitext arg)

fun jbsr (Address L) = jump (Jcc(24832),L)
...
fun fjne (Address L) = jump (FJcc(14),L)
fun fjeq (Address L) = jump (FJcc(1),L)
fun fjgt (Address L) = jump (FJcc(18),L)
fun fjge (Address L) = jump (FJcc(19),L)
fun fjlt (Address L) = jump (FJcc(20),L)
fun fjle (Address L) = jump (FJcc(21),L)
