...
fun within (APPpat (DATACON{rep=r1,...},_),plist) =
	let fun	app_inside (APPpat (DATACON{rep=r2,...},_)::more) =
			if r1=r2 then true
  ...
  | within (CONpat (DATACON{rep=r1,...}),plist) =
	let fun con_inside (CONpat(DATACON{rep=r2,...})::more) =
			if r1=r2 then true
...
fun relevant (VARpat _) = false
  ...
  | relevant (CONpat(DATACON{tycon=DATAtyc{dcons = ref [_],...},...})) = false
		(* if only one data constructor, no need to test *)
  | relevant (APPpat(DATACON{tycon=DATAtyc{dcons = ref [_],...},...},pat)) =
		relevant pat
  | relevant _ = true
...
fun	  relfactor nil nil = (nil,nil,nil,nil)
	...
	*)
	...
	| relfactor _ _ = Impossible " in relfactor:  #fields and #x's different."
