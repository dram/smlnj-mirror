   ...
   let
    ...
    fun test(tycl as (env, ty)) =
     case ty
      of VARty(TYVAR{status=ref(INSTANTIATED t),...}) => test(env,t)
       ...
       | CONty(ref(SYMtyc(spath,symbol)),tyl) =>
	    Condemn "Can't compare symbolic type for equality"
    ...
    and atomeq(name,tyl,env) =
	 let val eqsym = SymbolTable.StringToSymbol( name ^ "equal")
	     ...
	     fun apply nil = translatepath p
	       | apply (a11::b11) = APP(apply b11, test (env,a11))
          in apply (rev tyl)
