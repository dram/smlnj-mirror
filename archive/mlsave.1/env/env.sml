...
signature ENV = sig
  ...
  and lookBase : (symtable * Table.Symbol.symbol -> 'a)
...
functor NewEnv (Bind : BIND) (* : ENV *) = struct
  ...
  in
    ...
    val baseLayer1 = ref(STRLAYER)
    val baseLayer2 = ref(STRLAYER)
    val layers = ref nil;
    val remarks = ref nil;
    val markCount = ref 0;
    ...
    fun initBase (it1,it2) : unit =
	(baseLayer1 := CLOSED it1; baseLayer2 := CLOSED it2)
...
(* Bind. can be removed when bug is fixed *)
    ...
    fun openStr () : marker =
	(layers := !baseLayer2 :: !baseLayer1 :: STRLAYER :: (!layers);
	 remarks := CLOSEDr :: STRLAYERr :: (!remarks);
	 mark())
	...
	let fun close1 (remark) =
		case remark
		  of ENTRY i =>
		   ...
		   | STRLAYERr => true
		   | MARKER s =>
			if s=marker then false else true
	    fun loop nil = Impossible "marker lost in Env.close"
	      | loop (remark::rest) =
		  if close1(remark) then loop rest else rest
...
(*        ;    (print(!debugLook)); () *)
    ...
    fun lookBase (tblSearch) id = 
       let val CLOSED(info,tbl) = !baseLayer1
        in (tblSearch(tbl,id),info)
       end
...
structure EnvBind = struct
  ...
  type info = int list * Basics.context
  val defaultInfo = ref([]:int list, Basics.TOPctx)
end
