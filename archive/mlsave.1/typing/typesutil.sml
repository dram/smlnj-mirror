...
fun expandTy (ty as CONty(ref(TYPEtyc{params,def,context,...}), args)) =
    let val rec expand =
         fn v as VARty(tv) =>
		 let fun look ([],[]) = v
	   ...
	   | CONty(reftycon, args') =>
	       CONty(EnvAccess.tyconInCtx(reftycon, context), map expand args')
...
fun share_map f nil = raise e_SHARE
  | share_map f (x::l) =
      (f x) :: ((share_map f l) handle e_SHARE => l)
      handle e_SHARE => x :: (share_map f l)
	    ...
	    let val ty = prune ty
	     in case ty
		  of VARty(tv as TYVAR{status = ref BOUND,...}) =>
		       VARty(freshTyvar(tv))
		   | VARty _ => raise e_SHARE
		   | CONty(tyconref,args) =>
		       CONty(tyconref, share_map fresh args)
		   | UNKNOWNty => raise e_SHARE  (* propagate error *)
		   | _ => Impossible "fresh in typesutil.freshTy"
	    end
     in fresh(ty) handle e_SHARE => ty
    end;
