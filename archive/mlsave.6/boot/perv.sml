   ...
   struct
	...
	| substr("",_,_) = raisex substr
        | substr(s,i,0) = (* if i<0 orelse i >= slength s
			    then raisex substr (* BUG--this should be okay *)
			    else*) ""
    ...
    struct
	  ...
	  See filbuf and input_line as well. *)
       fun input({closed=ref true,...},_) =
	       raisex io_failure with "Input stream is closed"
         | input(f as {filid,pos,len,buf,closed}, i) =
	   if i < 0 then raisex io_failure with
			 "input called with negative character count"
	   else if (!len - !pos) >= i
...
*)
    ...
    struct
      ...
      exceptionx floor:unit and sqrt:unit and exp:unit and ln:unit
      exceptionx float : string = Assembly.Float
