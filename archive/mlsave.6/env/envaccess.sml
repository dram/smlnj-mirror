...
fun lookBase (tblSearch) id = 
    let fun f ((info,tbl)::r) = (f r) handlex Table.notfound_Table => 
					      (tblSearch(tbl,id),info)
	  | f nil = raisex Table.notfound_Table
     in f (!pervasives)
    end
...
(* what about paths of length > 1? *)
       | tyc => tyc)
    handlex Table.notfound_Table => condemn("unbound tycon in str: " ^ name id)
...
fun lookCONinStr(STRstr{table,env,...},id,ap): datacon =
    ...
    handlex Table.notfound_Table => condemn("unbound data constructor in str: " ^ name(id))
...
fun lookVARCONinStr(STRstr{table,env,...},id,ap): binding =
    ...
    handlex Table.notfound_Table => unboundVAR id
...
fun lookEXNinStr(STRstr{table,env,...},id,ap) =
    ...
    handlex Table.notfound_Table => unboundEXN id
...
fun lookSTRinStr(STRstr{table,env,...},id,ap) =
    ...
    handlex Table.notfound_Table => condemn("unbound str in str: " ^ name id)

val lookSTRinTable = fn x => lookSTRinTable x 
	    handlex Table.notfound_Table => 
