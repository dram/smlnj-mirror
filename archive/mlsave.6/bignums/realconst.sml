...
fun makebits (f,n) =
	...
	fun onebit b = Bigint.getbit(f,s-2-b) = 1
	fun copybit n = (update(bits,n,onebit n); copybit (n+1))
			handlex Bigint.getbit => () || subscript => ()
    in  copybit 0;
	bits
    end
...
fun round (float as {frac=f,exp=e},n) =
    let val shift = Bigint.size f - n
    in
	if shift <= 0 then float
	else {frac = case Bigint.getbit(f,shift-1)
			 of 0 => f rshift shift
			  | 1 => (f rshift shift) plus one,
	      exp = e + shift}
    end
end
...
structure DFA = Dynamic(struct  type float = {frac : Bigint.bigint, exp : int}
				type array = (unit->float) array
...
in
    val pos10 = array(fn () => raisex unknown)	(* 10^2^n *)
    val _ = update(pos10,0,makelem ten)
    val neg10 = array(fn () => raisex unknown)	(* 10^~2^n *)
    val _ = update(neg10,0,makelem tenth)
    fun access(arr,n) = (arr sub n) ()
			handlex unknown => let val last = access(arr,n-1)
    ...
    fun raisepower(f,0) = f
      | raisepower(f,e) =
	    let val sign = case e < 0 of true => ~1 | false => 1
		fun power(f,p) = mult f (pow10_2(sign*p))
		fun raisep(f,0,_) = f
		  | raisep(f,e,p) =
		    if (e bit_and 1) = 1 then raisep(power(f,p),e>>1,p+1)
		    else raisep(f,e>>1,p+1)
	    in  raisep(f,abs e,1)
	    end
end
...
fun reducefrac f =
	...
	fun getwhole nil = zero
	  ...
	  | getwhole ("0"::tl) =
		  ten times getwhole tl
	  | getwhole (n::tl) =
		  bigint(ord n - ord "0") plus (ten times getwhole tl)
	val backwards = rev f
...
fun getparts s =
	...
	val exp10 = (case exp_s of nil => 0
				 | "~"::more => ~(atoi more)
				 | other => atoi other)
		    handlex overflow => raisex toobig
...
val realconst = fn f => emitreal(makereal f)
		    handlex toobig => outofrange()
