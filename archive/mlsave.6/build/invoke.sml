...
\	.long 0\n");
		        ...
		        handlex Amachine.notfound_Codenv with v
			 => (MCprint.printLexp (Lambda.VAR v);
			     raisex Amachine.notfound_Codenv with v);
			output(outfile,"end:\n");
			close_out outfile
		    end))
	 ...
	 fun use1 () =
	  (while true do 
	   (let val _ = (ErrorMsg.anyErrors := false;
		...
		val a = Boot.create_s(size+4)
		val pos = ref 0
	        fun writebyte i = (Boot.store_s(a,!pos,i); inc pos)
		(* might not need the garbage word here; would need a
		   different boot *)
	    ...
	    handle Stop => raisex ErrorMsg.Syntax
		  | Parse.Eof => raisex Parse.Eof
