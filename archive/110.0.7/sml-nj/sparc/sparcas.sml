...
struct
  ...
  (** utility routines **)
    ...
    fun emitOffset 0 = ()
      | emitOffset i = (if (i < 0)
	  then (emit "-"; emit(AsmUtil.atoi(~i)))
	  else (emit "+"; emit(AsmUtil.atoi i)))


    fun emitLong i = (emit "\t.long\t"; emit(AsmUtil.atoi i); newLine(); advance4())
    ...
    in
    fun emitString s = (
	  emit "\t.ascii "; emit(AsmUtil.printableString s);
	  emit "\n";
	  emit "\t.align\t4\n";
	  advance(size s))
    end (* local *)
    ...
    fun emitAddr (info as INFO{addrOf,...}) (lab, k) = (
	  ...
	  emit "\t| "; emit(AsmUtil.atoi(k + addrOf lab - !loc)); newLine();
	  advance4())

    fun define (info as INFO{addrOf,...}) lab = (
						       
         emitLabel info lab;  emit ":\t| .="; emit(AsmUtil.atoi(!loc)); 
         emit ", "; emit(AsmUtil.atoi(addrOf lab)); newLine())
    ...
    fun mark () = (
	  ...
	  emit (AsmUtil.atoi (make_desc((!loc + 4) quot 4, tag_backptr))); newLine();
	  advance4())
    end (* local *)
   ...
   fun emitInstr info = let
      ...
      fun emitReg (REG i) = if (i < 16)
	    then if (i < 8)
	      then (emit "%g"; emit (Int.toString i))
	      else (emit "%o"; emit (Int.toString (i-8)))
	    else if (i < 24)
	      then (emit "%l"; emit (Int.toString (i-16)))
	      else (emit "%i"; emit (Int.toString (i-24)))

      fun emitFReg (FREG i) = (emit "%f"; emit (Int.toString i))
	...
	| emitCC CC_GU  = emit "gu"  | emitCC CC_LU  = emit "lu"
	...
	| emitRand (IMrand i) = emit (AsmUtil.atoi i)
    ...
    fun init (n : int) = (
	  loc := 0;
	  emit "| code size = "; emit(Int.toString n); emit " bytes\n")
