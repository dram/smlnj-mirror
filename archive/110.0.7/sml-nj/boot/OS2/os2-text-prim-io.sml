(* os2-text-prim-io.sml
 *
 * COPYRIGHT (c) 1995 AT&T Bell Laboratories.
 *
 * This implements the OS/2 version of the OS specific text primitive
 * IO structure.  It is implemented on top of special text-mode
 * (translating) I/O functions in the runtime-system.
 *
 * NOTE:  For text readers, getPos and findPos return translated
 * positions and setPos isn't supported (raises an exception).
 * For text writers, getPos and setPos returns/uses the `real'
 * (untranslated) position in the underlying text file.
 *
 * Peter Bertelsen, October 1995.
 *
 *)

structure OS2TextPrimIO : sig
    ...
    exception NoRandomAccess   (* Should this be in IO? *)
   ...
   *)
    ...
    fun mkReader {fd, name, initPos, initBlkMode} = let
      ...
      fun readVec n = let 
        val v = (ensureOpen(); announce "readTxtVec" OS2IO.readTxtVec(fd, n))
        in
          incPos (Vec.length v); v
        end
      fun readArr arg = let
        val k = (ensureOpen(); announce "readTxtArr" OS2IO.readTxtArr(fd, arg))
        in
          incPos k; k
        end
      ...
      fun setPos _ = raise NoRandomAccess
      ...
      in
        if initBlkMode
          then PrimIO.Rd{
            readaNoBlock = NONE,
            readNoBlock = NONE,
            readaBlock = SOME(readArr),
            readBlock = SOME(readVec),
            block     = NONE,
            canInput  = NONE,
            name      = name,
            chunkSize = bufferSzB,
    ...
    fun mkWriter {fd, name, initBlkMode, appendMode, chunkSize, lineBuf} = let
      ...
      fun writeVec arg = (ensureOpen();
        announce "writeTxtVec" OS2IO.writeTxtVec(fd, arg))
      fun writeArr arg = (ensureOpen();
        announce "writeTxtArr" OS2IO.writeTxtArr(fd, arg))
      fun close () = if !closed 
        then ()
        else (closed:=true; announce "close" OS2IO.close fd)
    ...
    fun stdErr() = mkWriter{
      ...
      appendMode  = false,
      lineBuf     = NONE,
      chunkSize   = 1	(* don't want to buffer this stream!! *)
    }

  end; (* structure OS2TextPrimIO *)


(*
 * $Log: os2-text-prim-io.sml,v $
