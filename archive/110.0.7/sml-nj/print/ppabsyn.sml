	  ...
	  | ppDconPat'(APPpat(DATACON{name,...},_,p),l,r,d) =
		  ...
		  fun prNon pat =
		      (begin_block ppstrm INCONSISTENT 2;
		       ...
		       ppDconPat'(pat,infFix,infFix,d-1);
		       end_block ppstrm)
	       in case thisFix
		    of INfix _ =>
		       (case p  (* ASSERT: p is a pair *)
			  of RECORDpat{fields=[(_,pl),(_,pr)],...} =>
			      let val atom = strongerL(l,thisFix)
				       orelse strongerR(thisFix,r)
				  val (left,right) =
				      if atom then (nullFix,nullFix)
				      else (l,r)
			       in (begin_block ppstrm INCONSISTENT 2;
				    lpcond(atom);
				    ppDconPat' (pl,left,thisFix,d-1);
