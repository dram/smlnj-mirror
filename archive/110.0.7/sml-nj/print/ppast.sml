...
fun ppPat ppstrm =
	...
	fun ppPat' (_,0) = ppsay "<pat>"
	  ...
	  | ppPat' (IntPat i,_) = ppsay i
	  | ppPat' (WordPat w,_) = ppsay w
...
and ppExp ppstrm =
	...
	fun ppExp'(_,_,0) = ppsay "<exp>"
	  ...
	  | ppExp'(SeqExp exps,noparens,d) =
		 ...
		 style=INCONSISTENT}
		exps
	  | ppExp'(IntExp i,_,_) = ppsay i
 	  | ppExp'(WordExp w,_,_) = ppsay w
...
and ppSigexp ppstrm =
	...
	fun ppSigexp'(_,_,0) = ppsay "<sigexp>"
	  ...
	  | ppSigexp'(BaseSig specs,sigflag,d) = 
	      (begin_block ppstrm CONSISTENT 0;
...
and ppFsigexp ppstrm =
	...
	fun ppFsigexp'(_,_,0) = ppsay "<fsigexp>"
	  ...
	  | ppFsigexp'(BaseFsig{param,result},SOME name,d) =
	      (begin_block ppstrm CONSISTENT 2;
	       begin_block ppstrm INCONSISTENT 2;
	       ppSym ppstrm name;
	       ppsay " ";
	       List.app (fn (pname,sign) => 
	         ...
	         param;
	       end_block ppstrm;
	       ppsay " = "; ppSigexp ppstrm (result,true,d-1);
	       end_block ppstrm)
	  | ppFsigexp'(BaseFsig _,_,_) = ppsay (errorHandler "<fsigexp>")
...
and ppSpec ppstrm =
	...
	fun ppSpec'(_,0) = ppsay "<spec>"
	  | ppSpec'(StrSpec strspecs,d) = 
		...
		(fn ppstrm => fn (s,sigexp,defOp) =>
		  (ppSym ppstrm s;
		   ppsay " : ";
		   ppSigexp ppstrm (sigexp,true,d-1);
		   case defOp
		     of NONE => ()
		      | SOME p => (ppsay " = "; ppSymPath ppstrm p))),
		strspecs);
	       end_block ppstrm)
	  ...
	  | ppSpec'(IncludeSpec includesig,d) = 
	      (ppsay "include ";
	       (* ppsay Symbol.name includesym)*)
	       ppSigexp ppstrm (includeSig,true,d))
...
and ppFctb ppstrm =
	...
	fun ppFctb'(_,0) = ppsay "<fctb>"
          ...
          | ppFctb'(Fctb{name,def=BaseFct{params,body,constraint}},d) =
	       ...
	       end_block ppstrm;
	       case constraint
		 of SOME(fsig) => (ppsay " : "; ppSigexp ppstrm (fsig,true,d-1))
		  | NONE => ();
 ...
 *
 * Revision 1.7  1997/11/24 20:29:07  dbm
 *     Ast constructor name changes.
 *
 * Revision 1.6  1997/10/26  23:17:20  dbm
 *   Change in type of StrSpec.  Added printing of definition in StrSpec
 *   when it exists.
 *
 * Revision 1.5  1997/08/02  02:26:55  dbm
 *   Eliminated commented out code (LocalSpec, OpenSpec).  Modified case
 *   for IncludeSpec to allow sigexp instead of symbol.
 *
 * Revision 1.4  1997/05/20  12:27:22  dbm
 *   SML '97 sharing, where structure.
 *
 * Revision 1.3  1997/04/14  21:39:03  dbm
 *   Added second argument for ValDec, ValrecDec, FunDec in ppDec'.  These
 *   are required for explicit type variable bindings.
 *
 * Revision 1.2  1997/01/31  15:06:48  dbm
 * Change in Ast types for opaque signature constraints.  Affects printing of
 * VarFct, FctFct, AppFct, Strb.
 *
