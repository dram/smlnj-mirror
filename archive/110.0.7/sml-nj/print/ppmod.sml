	    ...
	    case spec
		  ...
		  let val strEnt = EE.lookStrEnt(entities,entVar)
		   in SE.bind(sym,B.STRbind(M.STR{sign=sign,rlzn=strEnt,
                                      access=A.nullAcc, info=II.nullInfo}),env)
		  end
 ...
 *)
fun is_ppable_ConBinding (T.DATACON{rep=A.EXNFUN _, ...}, _) = true
  | is_ppable_ConBinding (T.DATACON{rep=A.EXNCONST _, ...}, _) = true
...
fun ppLambdaty ppstrm (lambdaty,depth) =
    add_string ppstrm "<lambdaty>"
...
fun ppTycExp ppstrm (tycExp,depth) =
    ...
    case tycExp
	  ...
	  (add_string ppstrm "TE.C:"; add_break ppstrm (1,1);
       ...
       | M.FMGENtyc (tycon, _) =>
	  (add_string ppstrm "TE.FG:"; add_break ppstrm (1,1);
	   ppTycon SE.empty ppstrm tycon)
       | M.FMDEFtyc tycon =>
	  (add_string ppstrm "TE.FD:"; add_break ppstrm (1,1);
	   ppTycon SE.empty ppstrm tycon)

fun ppStructureName ppstrm (str,env) =
    let val rpath =
	    case str
	      of M.STR{rlzn={rpath,...},...} => rpath
	       | _ => bug "ppStructureName"
	fun look a = LU.lookStr(env,a,(fn _ => raise Env.Unbound))
		      ...
		      end_block())
			   ...
			   (case name
				  ...
				  ((if MU.eqSign
				       (sign,
					LU.lookSig
					 (env,sym,(fn _ => raise Env.Unbound)))
    ...
    let fun pr first (sym,spec) =
	   case spec
		   ...
		   begin_block ppstrm CONSISTENT 0;
		    case entityEnvOp
		      of NONE => ppSignature0 ppstrm (sign,env,depth-1,NONE)
		       | SOME eenv =>
			  let val M.STRent{entities,...} = EE.look(eenv,entVar)
		 ...
		 (if first then () else add_newline ppstrm;
	      ...
	      | M.CONspec{spec=dcon as T.DATACON{rep=(A.EXNCONST _ | A.EXNFUN _),
						 ...},
                          ...} =>
		 (if first then () else add_newline ppstrm;
...
and ppSignature0 ppstrm (sign,env,depth,entityEnvOp) = 
	...
	case sign
	  of M.SIG {name,stamp,elements,typsharing,strsharing,...} =>
	     if !internals then 
	       (begin_block CONSISTENT 0;
		 pps "SIG:";
		 nl_indent ppstrm 2;
		 begin_block CONSISTENT 0;
		  pps "stamp: "; pps (Stamps.stampToShortString stamp);
		  add_newline();
		  pps "name: ";
		  case name
...
and ppFunsig ppstrm (sign,env,depth) =
    let val {begin_block,end_block,pps,add_break,add_newline} = en_pp ppstrm
	fun trueBodySig(orig as M.SIG{elements=[(sym,M.STRspec{sign,...})],...}) =
    	      if Symbol.eq(sym,resultId)
	      then sign
	      else orig
	  | trueBodySig sign = sign
...
and ppStrEntity ppstrm ({stamp,entities,lambdaty,rpath},env,depth) =
    let val {begin_block,end_block,pps,add_break,add_newline} = en_pp ppstrm
     in if depth <= 1 
	then pps "<structure entity>"
		...
		pps "lambdaty:";
		nl_indent ppstrm 2;
		ppLambdaty ppstrm (lambdaty,depth-1);
	       end_block ();
	      end_block ())
    end

and ppFctEntity ppstrm ({stamp,closure,lambdaty,tycpath,rpath},env,depth) =
    let val {begin_block,end_block,pps,add_break,add_newline} = en_pp ppstrm
     in if depth <= 1 
...
and ppTycBind ppstrm (tyc,env) =
	...
	then (begin_block CONSISTENT 0;
	      ...
	      end_block())
	       ...
	       of T.GENtyc{path, arity, eq=ref T.ABS,...} =>
		   (* abstype *)
		   (begin_block CONSISTENT 0;
		     pps "type";
		    ...
		    end_block())
		| T.GENtyc{path, arity, eq,
			   kind=T.DATATYPE{index,members,...}, ...} =>
		   (* ordinary datatype *)
