...
signature COERCE = sig
  ...
  val unwrapOp : wpEnv * LtyKernel.lty * LtyKernel.lty * DebIndex.depth
                   -> (Lambda.lexp -> Lambda.lexp) 

  val wrapOp   : wpEnv * LtyKernel.lty * LtyKernel.lty * DebIndex.depth
                   -> (Lambda.lexp -> Lambda.lexp) 
...
fun wcLook([], t) = bug "unexpected wenv in wcLook"
  | wcLook((_, z as ref m)::_, t) = 
      (let fun loop((t',x)::rest) = if lt_eq(t,t') then SOME x else loop rest
             | loop [] = NONE
  ...
  getWTC(wflag, nx, ox, 
   (fn (TC_BOX nz, _) => 
          let (* major gross hack mode ON ----- *)
              val nz = case LE.tcWrap ox (* was nz *)
                        of NONE => nz
                         | SOME x => 
                             if tc_eq(x, nx) then nz 
...
*)
     | (TC_ABS _, TC_ABS _) => 
          if LE.tcEquiv(nx, ox) then NONE 
          else (say " Type nx is : \n"; say (LT.tc_print nx);
                say "\n Type ox is : \n"; say (LT.tc_print ox); say "\n";
                bug "unexpected abs tycs in tcLoop")

     | (TC_ABS nz, _) => 
          let val nt = LE.tcWrap nz
           in case nt
               of NONE => 
...
*)
     ...
     | (TC_ARROW (nx1, nx2), TC_ARROW (ox1, ox2)) => 
          let val wp1 = tcLoop (not wflag) (nx1, ox1)
...
fun ltLoop wflag (nx, ox) = 
  getWLT(wflag, nx, ox, 
   (fn (LT_TYC nz, LT_TYC oz) => tcLoop wflag (nz, oz)
     ...
     | (LT_FCT(nx1,nx2), LT_FCT(ox1, ox2)) => 
          let val wp1 = ltLoop (not wflag) (nx1, ox1)
 ...
 *
 * Revision 1.5  1997/11/14 21:04:54  dbm
 *   Fix for bug 1308 [Zhong].
 *
