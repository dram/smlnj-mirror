...
fun mkDcon depth (DATACON {name, rep, typ, ...}) = 
     (name, rep, TT.toLty depth typ)

fun toCon(pcon, depth) =
  (case pcon
    of DATApcon (dc, ts) => DATAcon (mkDcon depth dc)
     | INTpcon i => INTcon i
     | INT32pcon i => INT32con i
     | WORDpcon w => WORDcon w
     | WORD32pcon w => WORD32con w
     | REALpcon r => REALcon r
     | STRINGpcon s => STRINGcon s
     | VLENpcon(i, t) => VLENcon i)
...
fun genRHSFun ([], rhs, _) = FN(mkv(), LT.ltc_int, rhs)
  | genRHSFun ([v], rhs, d) = 
  ...
  | genRHSFun (vl, rhs, d) =
      let val argvar = mkv()
       ...
       in FN(argvar, LT.ltc_tup tt, body)
      end
...
fun generate (dt, matchRep, rootVar, depth) =
  let val (subtree, envout) = pass1(dt, [(0, [ROOTPATH])], matchRep)
      ...
      fun genpath (RECORDPATH paths, env) =
        ...
        | genpath (DELTAPATH(pcon, path), env) = 
            let fun g (DATApcon(dcon, ts)) = (mkDcon' dcon, map toTyc' ts)
                  | g _ = bug "genpath-DECON on non data constructors"

                fun h (((_, DA.REF, lt), ts), e) = 
                        APP (PRIM (PO.DEREF, LT.lt_swap lt, ts), e)
(**
                  | h (((name, rep as DA.EXNFUN _, lt), []), e) = 
                        (* a hack to add wrapper when applying exceptions *)
                        let val (ax, _) = LT.tcd_arw(LT.ltd_tyc lt)
                         in DECON((name, rep, LT.ltc_pexn), [ax], e)
                        end
                  | h (((name, rep as DA.EXNFUN _, lt), _), e) =
                        bug "unexpected case in generate-h"
  *)
                  | h ((dc, ts), e) = DECON(dc, ts, e)

             in h(g pcon, VAR(lookupPath(path, env)))
            end
        | genpath (VPIPATH(n, t, path), env) =
            let val tc = toTyc' t
                val lt_sub = 
                  let val x = LT.ltc_tyc(tc_vector (LT.tcc_tv 0))
                   in LT.ltc_poly([LT.tkc_mono], 
                          LT.ltc_arw(LT.ltc_tup [x, LT.ltc_int], LT.ltc_tv 0))
                  end
        ...
        | genpath (VLENPATH (path, t), env) = 
            let val tc = toTyc' t
                val lt_len = LT.ltc_poly([LT.tkc_mono], 
                                         LT.ltc_arw(LT.ltc_tv 0, LT.ltc_int))
                val argtc = tc_vector tc
      ...
      fun pass2 (BIND(path, subtree), env, rhs) =
        ...
        | pass2 (CASETEST(path, sign, cases, NONE), env, rhs) = 
            SWITCH(VAR(lookupPath(path, env)), sign, 
        ...
        | pass2 (ABSTEST0(path, con as (dc, _), yes, no), env, rhs) =
            ...
            then SWITCH(VAR(lookupPath(path, env)), DA.CNIL, 
      ...
      and pass2cases(nil, env, rhs) = nil
        | pass2cases((pcon,subtree)::rest, env, rhs) = 
            let val res = (toCon(pcon, depth), pass2(subtree, env, rhs))
             in res::(pass2cases(rest,env,rhs))
            end
 ...
 *
 * Revision 1.9.2.2  1999/07/20 14:22:38  dbm
 * change name of bindContainsVar to bindNoVariableWarn
 *
 * Revision 1.9.2.1  1999/06/21 18:40:36  dbm
 * change spelling of Control.MC.bindExhaustive (to bindNonExhaustiveWarn)
 *
 * Revision 1.9  1997/09/30 02:37:53  dbm
 *   Replaced matchExhaustive flag by two flags matchNonExhaustiveWarn
 *   and matchNonExhaustiveError.  The latter allows users to have
 *   nonexhaustive matches reported as errors.
 *
