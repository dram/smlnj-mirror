  ...
  let fun last (DA.LVAR x) = x 
      ...
      val rec find =
        fn VAR w => if (v=w)
         ...
         | SELECT(_,l) => find l
         | CON((_, DA.EXNFUN p, _), _, e) => (find(VAR(last p)); find e)
         | CON((_, DA.EXNCONST p, _), _, e) => (find(VAR(last p)); find e)
         | DECON((_, DA.EXNFUN p, _), _, e) => (find(VAR(last p)); find e)
         | DECON((_, DA.EXNCONST p, _), _, e) => (find(VAR(last p)); find e)
         | CON(_,_,e) => find e
         | DECON(_,_,e) => find e
