...
signature REORDER = 
sig
  val reorder : Lambda.lexp -> Lambda.lexp 
end 
...
and loop : lexp -> lexp info =
  fn e as SVAL sv => 
   ...
   | e as EXNF _ => I{regs=1, side=true, fetch=false, alloc=true, exp=e}
   | e as EXNC _ => I{regs=1, side=true, fetch=false, alloc=true, exp=e}
...
val reorder = fn x => getexp (loop x)
