...
fun int_switch(e: 'value, l, default, inrange) =
  ...
  fun isexn (L.DATAcon(_,A.EXNFUN _,_)) = true
    | isexn (L.DATAcon(_,A.EXNCONST _,_)) = true
    | isexn _ = false

 fun exn_switch(w,l,default) =
   E_getexn(w, fn u =>
      let fun g((L.DATAcon(_,A.EXNCONST p,_),x)::r) =
	          E_path(p, fn v => E_getexn(v, fn z =>
					      E_branch(E_pneq,u,z,g r, x)))
            | g((L.DATAcon(_,A.EXNFUN p,_),x)::r) =
     ...
     | {exp,sign,cases as (c,_)::_,default} =>
      case c
       of L.INTcon _ => 
        ...
        | L.DATAcon(_,A.EXNFUN _,_) => exn_switch(exp,cases,default)
        | L.DATAcon(_,A.EXNCONST _,_) => exn_switch(exp,cases,default)
