...
local structure LV = LambdaVar
      open Array List M System.Tags Access CPS
in
	...
	*) 
	...
	(align(); 
	 ...
	 handle M.BadReal r =>
	  ...
	  align(); 
	  mark();
	  emitlong(make_desc(size s, tag_string));
	  define lab; 
	  emitstring s; 
...
(*        | RECORD(RK_FBLOCK,[(u as REAL _,OFFp 0)],w,e) =>
          ...
          | RECORD(RK_FBLOCK,vl,w,e) =>
              alloc(w,any,fn w' => let 
                ...
                val desc = 
                  if k=1 then desc_reald else make_desc(k, tag_realdarray)
                val vl' = 
	  ...
	  | RECORD(RK_I32BLOCK,vl,w,e) => 
	      alloc(w,any, fn w' =>
		      (record((immed(make_desc(length vl * 4,tag_string)),
                                     OFFp 0)::
	  ...
	  | RECORD(RK_CONT,vl,w,e) =>
	       ...
	       (alloc(w,any, fn w' =>
		      ...
		      val desc = make_desc(length vl, tag_cont)
...
(*		    align();   temporarily removed so 68020 will work. *)
	  ...
	  | SETTER(P.free,[x],e) => 
		 ...
		 in fetchindexl(x', arithtemp, immed ~1);
		    andb(immed(power_tags-1), arithtemp,arithtemp);
		    ibranch(NEQ,arithtemp,immed(make_desc(0, tag_cont)),join);
...
(*
          let val join = newlabel() and x' = gpregbind x
           in fetchindexl(x', arithtemp, immed ~1);
	      andb(immed(power_tags-1), arithtemp,arithtemp);
              ibranch(EQL,arithtemp,immed(make_desc(0, tag_cont)),skip);
