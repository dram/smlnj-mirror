...
val mergeLty = which(LT.lt_merge, LT.ltc_void)
...
val seqTy = LT.ltc_arw(LT.ltc_tup[LT.ltc_void,LT.ltc_void],LT.ltc_bool)
...
fun cmpop(stuff,argt) = 
  (case stuff
    of {oper=AP.EQL,kind=AP.INT 31} => 
	 if LK.lt_eq(argt,LT.ltc_tup[LT.ltc_void,LT.ltc_void])
...
fun isFloatRec lt = 
  (case LK.lt_out lt
    of (LK.LT_TYC tc) =>
         (case LK.tc_out tc
           of LK.TC_TUPLE (l as _::_) =>
               let fun h [] = flatfblock
                     | h (x::r) = 
                           if LK.tc_eq(x, LT.tcc_real) then h r else false
                in h l
               end
            | _ => false)
     |  _ => false)
...
*)
   | Lambda.APP(Lambda.PRIM(AP.THROW,_,_), v) => 
        let val kv = convsv v
            ...
            val f = mkv(t)
         in PURE(P.wrap,[kv],f,ctype(t),c(VAR f))
        end
		...
		pure(1,P.real{tokind=numkind tokind,
	     ...
	     | AP.MARKEXN => getargs(2, Lambda.SVAL a,fn[x,m']=>
	  	  let val bty = LT.ltc_void
                      val ety = LT.ltc_tup[bty,bty,bty]
   ...
   | Lambda.EXNF(v,lt) =>
       let val u = convsv v
           val x = mkv(LT.ltc_void) (* and y = mkv(lt) *)
        in PURE(P.makeref,[u],x,BOGt,c(VAR x))
                 (****>> PURE(P.cast,[VAR x],y,FUNt,c(VAR y)) <<****)
       end
   | Lambda.EXNC v => 
       let val u = convsv v
           val x = mkv(LT.ltc_void) and z = mkv(LT.ltc_void)
           and y = mkv(LT.ltc_tup[LT.ltc_void,LT.ltc_int,LT.ltc_void])
