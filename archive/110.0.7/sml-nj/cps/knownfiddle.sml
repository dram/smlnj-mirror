...
struct
 ...
 fun pass1 cexp : lvar -> lvar list =
   ...
   let exception Amap
	 ...
	 fn APP(VAR v,args) => [v]
	  ...
	  | SELECT(_,v,w,_,ce) => applied ce
	  | OFFSET(_,v,w,ce) => applied ce
	  | SETTER(_,vl,e) => applied e
	  | LOOKER(_,vl,w,_,e) => applied e
	  | ARITH(_,vl,w,_,e) => applied e
	  | PURE(_,vl,w,_,e) => applied e
	  | BRANCH(_,vl,c,e1,e2) => SortedList.merge(applied e1, applied e2)
	  | FIX(nil,e) => applied e
	  | FIX(fl as (_,f,_,_,_)::_, e) =>
		let val body_applied = 
		        SortedList.foldmerge(map (fn (_,f,vl,_,b) => applied b) fl)
 ...
 fun fiddle (fkind,func,args,ctyl,cexp) =
  let open IntmapF
	 ...
	 fn e as APP(VAR v,args) => (APP(VAR(rename v),args) 
	  ...
	  | SELECT(i,v,w,t,ce) => SELECT(i,v,w,t,rewrite ce)
	  | OFFSET(i,v,w,ce) => OFFSET(i,v,w, rewrite ce)
	  | SETTER(p,vl,e) => SETTER(p,vl, rewrite e)
	  | LOOKER(p,vl,w,t,e) => LOOKER(p,vl,w,t,rewrite e)
	  | ARITH(p,vl,w,t,e) => ARITH(p,vl,w,t,rewrite e)
	  | PURE(p,vl,w,t,e) => PURE(p,vl,w,t,rewrite e)
	  ...
	  | FIX(fl as (_,f,_,_,_)::_, e) =>
		...
		fun redefine (f,vl) = 
		           let val f'::vl' = map LambdaVar.dupLvar (f::vl)
		            in (ESCAPE,f',vl',map (fn _ => BOGt) vl,
                                APP(VAR f, map VAR vl'))
			   end

		val newdefs = map redefine (test (applies f))
		  
		val rename_map' = 
		    fold (fn ((_,f',_,_,APP(VAR f, _)),m) => add(m,f,f')) 
		         newdefs
			 rename_map

		val knowns' = fold (fn ((_,f,vl,_,b),kn) => add(kn,f,vl)) 
                                 fl knowns

		fun rewrite_body(k,f,vl,cl,b) = 
		       (k,f,vl,cl,rewrite_with(rename_map',knowns') b)
  ...
  in (fkind, func, args, ctyl, rewrite_with(empty,empty) cexp)
 end
