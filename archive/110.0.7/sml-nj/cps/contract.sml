  ...
  | argLty [t] = 
      (case LK.lt_out t 
        of LK.LT_TYC tc =>
            (case LK.tc_out tc
              of LK.TC_TUPLE (_::_) => 
                  if (LT.lt_length(t) < MachSpec.maxRepRegs)
                  then LT.ltc_tup [t] else t
               | _ => t)
         | LK.LT_STR (_::_) => 
            if (LT.lt_length(t) < MachSpec.maxRepRegs)
            then LT.ltc_str [t] else t
         | _ => t)
...
fun mkfnLty(_,_,nil) = bug "mkfnLty in nflatten"
  | mkfnLty(k,CNTt::_,x::r) = 
      (case LK.lt_out x
	of LK.LT_CNT t2 => (k,LT.ltc_fun(argLty r,t2))
         | LK.LT_TYC tc => 
             (case LK.tc_out tc
               of LK.TC_CONT t2 => (k,LT.ltc_fun(argLty r, LT.ltc_tyc t2))
                | _ => (k, LT.ltc_fun(argLty r,x)))
	 | _ => (k, LT.ltc_fun(argLty r,x)))
   ...
   | APP(f, vl) =>
	  ...
	  fun newvl NONE = vl'
	    | newvl (SOME live) =
		  ...
		  case z(vl',live)
		    of nil => [INT 0]
		     | [u] => (case LK.lt_out(grabty(u))
			        of (LK.LT_CNT _) => [u,INT 0]
                                 | (LK.LT_TYC tc) => 
                                      (case LK.tc_out tc
                                        of (LK.TC_CONT _) => [u, INT 0]
                                         | _ => [u])
				 | _ => [u])
		     | vl'' => vl''
	      end
