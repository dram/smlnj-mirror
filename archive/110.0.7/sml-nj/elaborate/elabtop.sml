        ...
        (case S.nameSpace name
          of S.VALspace =>
			 ...
			 the static environment. *)
                    ...
                    | V.CON(T.DATACON{rep=DA.EXNFUN _, ...}) => 
                        ExceptionDec [EbDef{exn=name,
					    edef=([localStrName,name])}] :: dl

                    | V.CON(T.DATACON{rep=DA.EXNCONST _, ...}) => 
                        ExceptionDec [EbDef{exn=name,
					    edef=([localStrName,name])}] :: dl
...
fun elab(SeqDec decs, env0, top, region) =
  ...
  | elab(MarkDec(dec,region'), env, top, region) = 
		...
		in (if !Control.markabsyn then A.MARKdec(d,region') else d, env)
	       end
 ...
 *
 * Revision 1.6  1997/10/01 18:11:48  dbm
 *   Changed elab to preserve Mark around top-level declarations.  Part of
 *   fix for bug 1133.
 *
 * Revision 1.5  1997/09/10  22:13:52  dbm
 *   Fix regression failure on bug 233 due to new pickleless top level.
 *
 * Revision 1.4  1997/04/14  21:30:43  dbm
 *   Added parameter to ValDec (associated with explicit type variable bindings).
 *
 * Revision 1.3  1997/04/10  14:37:28  dbm
 *   Bug fix for 788.  Narrowed environment used to elaborate rebindings for
 *   top level opens.
 *
 * Revision 1.2  1997/01/31  15:07:31  dbm
 * Change in Ast types for opaque signature constraints.
 *
