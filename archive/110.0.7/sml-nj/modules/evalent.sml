...
fun evalTyc (entv, tycExp, entEnv, epc, rpath, 
      ...
      case tycExp
       of CONSTtyc tycon => tycon
        | FMGENtyc (T.GENtyc {kind=T.DATATYPE{index=0, members, ...},
                              arity, eq, path, ...}, NONE) => 
            let val vizty = MU.transType entEnv
                val newmbs = MU.mapDtMbers (mkStamp, vizty) members
                val nst = #stamp(Vector.sub(newmbs,0))
                val _ = EPC.bindPath(epc,MI.TYCid(nst),entv)

             in T.GENtyc{stamp=nst, arity=arity, eq=eq,
                         kind=T.DATATYPE{index=0, members=newmbs,
                                         lambdatyc=ref NONE},
                         path=IP.append(rpath,path)}
            end
        | FMGENtyc (T.GENtyc {kind=T.DATATYPE{index=i, ...},
                              arity, eq, path, ...}, SOME rootev) =>
            let val rtmbs =
                  case EE.lookTycEnt(entEnv, rootev)
                   of (T.GENtyc{kind=T.DATATYPE{members,...}, ...}) =>
                        members
                    | _ => bug "unexpected case in evalTyc-FMGENtyc"
                val nst = #stamp(Vector.sub(rtmbs,i))
                val _ = EPC.bindPath(epc,MI.TYCid(nst),entv)

             in T.GENtyc{stamp=nst, arity=arity,
                         kind=T.DATATYPE{index=i, members=rtmbs,
                                         lambdatyc=ref NONE},
                         path=IP.append(rpath,path), eq=eq} 
            end
        | FMDEFtyc (T.DEFtyc{stamp,tyfun=T.TYFUN{arity, body},strict,path}) =>
            let val nst = mkStamp()
                val _ = EPC.bindPath(epc,MI.TYCid(nst),entv)
