...
fun exnRep (DA.EXNFUN _, dacc) = DA.EXNFUN dacc
  | exnRep (DA.EXNCONST _, dacc) = DA.EXNCONST dacc
  ...
  let val specName = S.name(TU.tycName specTycon)
   in case specTycon
       of GENtyc{stamp=s,arity,kind=specKind,eq=ref eqprop,...} =>
            ...
            else (case (specKind, TU.unWrapDefStar (* bugs 1364, 1432 *) strTycon)
                 ...
                 of VALspec{spec=acttyp, slot=actslot} =>
                  ...
                  | CONspec{spec=DATACON{typ=acttyp, name, const,
                   ...
                   let val spectyp = typeInMatched("$specty(val/con)", spectyp)
                       ...
                       (* exceptions can't possibly be polymorphic ! *)
                       val (decs', bindings') =
                         case slot 
                          of NONE => 
                               let val con = DATACON{typ=acttyp,
                                                     name=name,const=const, 
                                                     rep=rep,sign=sign}
                                in (case TU.headReduceType acttyp
                                     of POLYty _ => 
                                        let val acc = DA.namedAcc(name, mkv)
                                            val specvar = 
                                              VALvar{path=SP.SPATH[name], 
                                                     typ=ref spectyp,
                                                     access=acc, 
                                                     info=II.nullInfo}
                           ...
                           | SOME s => 
                               let val dacc = DA.selAcc(rootAcc, s)
                                   ...
                                   val con = DATACON{typ=acttyp, name=name,
                                                     ...
                                                     sign=sign}
                                in (decs, (B.CONbind(con))::bindings)
                               end
 ...
 *
 * Revision 1.17.2.3  1998/09/14 17:41:05  dbm
 * Fix bug 1432, reinstating bug 1364
 *
 * Revision 1.17.2.2  1998/08/19 18:20:57  dbm
 * bug fixes for 110.9 [dbm]
 *
 * Revision 1.17.2.1  1998/07/21 21:11:20  dbm
 * fix compStr in checkSharing
 *
