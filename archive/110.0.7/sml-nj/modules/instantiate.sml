...
(*     | FinalTyc(ref(VISITED)) => "FinalTyc.VISITED" *)
...
(* strip tycInst constructors *)
fun tycInstToTyc(INST tyc) = tyc
  | tycInstToTyc(NOTINST tyc) = tyc
	     ...
	     (case strDef
		   ...
		   (fn (sym,STRspec{sign,entVar,def,slot}) =>
			...
			before debugmsg ("<<getElemDefs.C: STRspec " ^ Symbol.name sym))
		     | (sym,TYCspec{spec=tyc,entVar,scope}) =>
...
(* ED.withInternals(fn () =>
			     ED.debugPrint debugging
			      ("#getElemDefs:TYCspec",
			       PPType.ppTycon StaticEnv.empty,
			       tyc'));
*)
...
fun mkElemSlots(SIG{elements,...},slotEnv,rpath,epath,sigDepth) =
    let fun mkSlot((sym,STRspec{sign as SIG{closed,...},entVar,def,...}),slotEnv) = 
	  ...
	  | mkSlot((sym,TYCspec{spec=tycon,entVar,scope}),slotEnv) = 
	     (case tycon
			   ...
			   (let val tf = 
				 T.TYFUN{arity=arity, 
					 body=MU.transType entEnv body}
                                val s = mkStamp()
			        val tc =
				 T.DEFtyc{tyfun=tf, strict=strict, 
					  stamp=s, path=IP.append(rpath,path)}
			    in debugType("#instToTyc(NOTINST/DEFtyc)",tc);
(* ED.withInternals(fn () =>
			        ED.debugPrint debugging
				 ("#instToTyc(NOTINST/DEFtyc)",
				  PPType.ppTycon StaticEnv.empty,
				  tc));
*)
			       r := INST tc;
			...
			| T.GENtyc{stamp, arity, eq, path, 
				   kind=z as T.DATATYPE {index,members,...}} =>
			   (* the domains of dataconstructors need to be
			      MU.transType-d (ZHONG) 
			    *)
			  (let fun h (x as {name, rep, domain=NONE}) = x
				 | h ({name, rep, domain=SOME ty}) = 
				    {name=name, rep=rep,
				     domain=SOME(MU.transType entEnv ty)}

                               (* major gross hack; needs clean up [ZHONG] *)
                               (*** "stamp" needs to be cleaned up ***)

                               val s = mkStamp()
			       fun g {tycname, stamp, arity, dcons, eq, sign,
				      lambdatyc} = 
				 {tycname=tycname, stamp=s, (* was stamp *)
				  arity=arity, dcons=map h dcons, eq = eq,
				  sign=sign,lambdatyc=ref NONE}
...
and getTkFct{sign as M.FSIG{paramvar, paramsig, bodysig, ...}, entEnv,
      ...
      let val (arg_eps, res_eps) =
       ...
       in LT.tkc_fcts(LT.tkc_seqs arg_tks, LT.tkc_seqs res_tks)
      end

  | getTkFct _ = LT.tkc_fcts(LT.tkc_seqs [], LT.tkc_seqs [])
 ...
 *
 * Revision 1.24.2.2  1999/07/20 21:34:11  dbm
 * make "multiple defs" warnings conditional on multDefWarn flag
 *
 * Revision 1.24.2.1  1998/08/19 18:20:55  dbm
