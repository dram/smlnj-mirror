  ...
  in
    case instr
     of I.STORE{st=I.STW, b, d, r, mem}	=> store("\tSTW\t", b, d, r, mem)
      ...
      | I.B(lab)			=> (emit "\tB,n\t"; eLabel lab; 
					    emit "\n\tNOP")
      | I.FBCC{t, ...}			=> (emit "\tFBCC,n\t"; eLabel t;
					    emit "\n\tNOP")
      ...
      | I.BLE{d, sr, b, t, ...} => 
	   ...
	   emit "\n\tCOPY %r31,"; eReg t)
      | I.BL{x=I.LabExp(LE.LABEL lab, fs), t, ...} => 
	  (emit "\tBL,n\t"; emitFieldSel fs; eLabel lab; comma(); eReg t;
	   emit "\n\tNOP")
      | I.BL _ => error "emitInstr:bl"
      | I.BV{x, b, ...} => 
	  (emit "\tBV,n\t"; eReg x; paren(fn () => eReg b); emit "\n\tNOP")
      ...
      | I.NOP => emit "\tNOP"
      | I.COPY(rds, rss, _) => let
	  fun mvInstr(rd, rs) = I.ARITH{a=I.OR, r1=rs, r2=0, t=rd}
          val instrs = Shuffle.shuffle(mvInstr, rmap, C.asmTmpR, rds, rss)
	in app (fn I => (emit "\t"; emitInstr(I, regmap))) instrs
        end
      | I.FCOPY(fds, fss, _) => let
	  fun mvInstr(fd, fs) = I.FUNARY{fu=I.FCPY, f=fs, t=fd}
	  val instrs = Shuffle.shuffle(mvInstr, rmap, C.fasmTmp, fds, fss)
	in app (fn I => (emit "\t"; emitInstr(I, regmap))) instrs
	end
    (*esac*);
    emit "\n"
