...
val intequalSym = Symbols.stringToSymbol "intequal"
val boolequalSym = Symbols.stringToSymbol "boolequal"
val refequalSym = Symbols.stringToSymbol "refequal"
val arrayequalSym = Symbols.stringToSymbol "arrayequal"
val realequalSym = Symbols.stringToSymbol "realequal"
val stringequalSym = Symbols.stringToSymbol "stringequal"

exception Poly

fun atomeq tyc =
    let val z =
	  if eqTycon(tyc,!intTycon) then intequalSym
	  else if eqTycon(tyc,!boolTycon) then boolequalSym
	  else if eqTycon(tyc,!refTycon) then refequalSym
	  else if eqTycon(tyc,!arrayTycon) then arrayequalSym
	  else if eqTycon(tyc,!realTycon) then realequalSym
	  else if eqTycon(tyc,!stringTycon) then stringequalSym
	  else raise Poly
     in case EnvAccess.lookVARinBase(z)
	 ...
	 case ty
	  of VARty(ref(INSTANTIATED t)) => test t
	   ...
	   | CONty(ref(tyc as
	       ...
	       (case rep
		  of TRANSPARENT => 
		       let val ty' = argType(typ,tyl)
			in (* bug -- doesn't cope with INDtycs
			   if equalType(ty',ty)
			   then (complain("equality on nonwellfounded datatype");
				 print "  type: "; PrintType.printType ty;
				 print "\n";
				 raise Syntax)
			   else  *) test ty'
		       end
