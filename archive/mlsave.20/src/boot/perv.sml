...
structure System : SYSTEM =
  struct
    ...
    val version = "Standard ML of New Jersey, Version 0.20, 13 June 1988"
...
in  fun polyequal(a : 'a, b : 'a) = ieql((cast a):int,(cast b):int)
      orelse
      (InLine.boxed a andalso InLine.boxed b
       andalso
	  (* assumes identical tags, with string=embedded *)
	  let val tag = InLine.subscript(cast a,~1)
	      val tag = Integer.mod(InLine.+(InLine.*(cast tag,2),1),power_tags)
	      val alen = InLine.alength(cast a)
	      fun maprec () =
		    let fun m i = if ieql(i,alen) then true
	 			  else if polyequal(sub(cast a,i), sub(cast b,i))
				    then m(InLine.+(i,1))
				  else false
		    in  m 0
		    end
	  in  if ieql(tag,tag_array) then false
	      ...
	      else ieql(alen,InLine.alength(cast b)) 
