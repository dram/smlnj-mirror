     ...
     in case !nextToken
	  of ID => sel1(ident())
	   | INT => let val i = !intValue
			val s = makestring i
		     in if i < 1
			then condemn("nonpositive integer label in selector: " ^ s)
			else sel1(Symbols.stringToSymbol(s))
		    end
	            before advance()
    ...
    then let fun lablist () = 
		   ...
		   of ID => field(ident(),abbrev)
		    | INT => let val i = !intValue
				 val s = makestring i
			      in if i < 1
				 then condemn("nonpositive integer label: " ^ s)
				 else field(Symbols.stringToSymbol(s),
				        (fn id =>
