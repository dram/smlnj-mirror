...
/*
	...
	printf("Closure elements were accessed %d %s through %d %s:\n",
		num_closure_accesses, s,
		num_links_traced, s1);

	printf("Size     Accesses   %% accesses       Links   %% links\n");
	for (k = 1; k < LINKSLOTS; k++)
		if (links(k) > 0)
			printf("%4d%13d%12.1f%%%12d%9.1f%%\n",
				k,links(k),100.0*links(k)/num_closure_accesses,
				k*links(k),100.0*k*links(k)/num_links_traced);
	if (links(0) > 0)
		printf(">%3d%13d%12.1f%%%12d%9.1f%%\n",
			LINKSLOTS - 1,links(0),100.0*links(0)/num_closure_accesses,
			links(BIGLINKS),100.0*links(BIGLINKS)/num_links_traced);
	f = (float)num_links_traced/num_closure_accesses;
