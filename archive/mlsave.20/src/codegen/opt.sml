...
fun mapfree lookfree =
   let val m = Intmap.new() : lexp Intmap.intmap
...
fun reduce exp =
	  ...
	  val t = Intmap.new() : lexp Intmap.intmap
	  ...
	  and mak =
		...
		((case e
		  of VAR v => mak(switch(SWITCH(imap v
				handle Intmap.Intmap => raise BadSwitch
				  , l, d)))
		   | _ => raise BadSwitch
		) handle BadSwitch =>
		let val e' = mak e
...
fun mix0free e =
let val vars : (lvar list * lvar list) Intmap.intmap = Intmap.new()
    val setvars = Intmap.add vars
    val rec freevars =
	 fn VAR 0 => (nil,nil)
