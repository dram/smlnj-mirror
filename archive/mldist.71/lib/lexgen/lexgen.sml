...
val pr_err = fn x => (outputc std_out ("mlex: syntax error in line "^
			    (makestring (!LineNum))^
			    ": "^x^"\n"); raise error)
...
fun lexGen(infile) =
    ...
    let val sayln = fn x => (say x; say "\n")
	 ...
	 sayln "\tval {fin,trans} = Array.sub(Internal.tab, s)";
	 sayln "\tval NewAcceptingLeaves = fin::AcceptingLeaves";
	 sayln "\tin if l = !yybl then";
	 sayln "\t     if trans = #trans(Array.sub(Internal.tab,0))";
	 ...
	 sayln "\t  else let val NewChar = ordof(!yyb,l)";
	   ...
	   case (!CharFormat,!CharSetSize)
	    of (true,129) => sayln "if NewChar<128 then ordof(trans,NewChar) else ordof(trans,128)"
	     ...
	     | (false,129) => sayln "if NewChar<128 then Array.sub(trans, NewChar) else Array.sub(trans, 128)"
	     | (false,256) => sayln "Array.sub(trans, NewChar)";
