...
signature CALCTYPE = sig
  ...
  val deabsstr : (unit -> encfunct) -> Basics.Structure -> Basics.Structure
end  (* signature CALCTYPE *)

structure CalcType : CALCTYPE = struct

open Array List Basics BasicTypes Unify TypesUtil BareAbsyn Overload ErrorMsg
   ...
   fun patType(pat: pat, kind: tvkind) : ty =
    case pat
      of WILDpat => VARty(mkTyvar(kind))
       ...
       | CONpat(DATACON{typ,...}) => mapTy (*??*)(applyPoly(typ,0,0,infinity))
	   ...
	   let val argty = patType(arg,kind)
	       val opty = applyPoly((case rep of Access.REF => refPatType | _ => typ),
				    0,0,infinity)
 ...
 fun expType(exp: exp, occ: occ) : ty =
    case exp
      of VARexp(ref(VALvar{typ,...})) => 
	           mapTy(applyPoly(!typ,abscount occ,base occ,wmax occ))
...
local 
  ...
  val adhocStrs = ref nil:(int * Stampset.stamp * Basics.Structure) list ref
  val adhocTycs = ref nil:(int * Stampset.stamp * Basics.tycon) list ref
in
  ...
  let fun instance ef stamp = 
	...
	| detyc (GENtyc{kind=ref(ABStyc hiddentyc),...}) = detyc hiddentyc
	   ...
	   (case encfunct() of
	    ...
	    | NONE => tyc)
 	| detyc (DEFtyc{path,tyfun}) = DEFtyc{path=path,tyfun=detyfun tyfun}
	| detyc tyc = tyc
