...
sig
  ...
  *)
  ...
  val chaseType: (br * evindex) -> BareAbsyn.exp -> Basics.ty
  val chaseTycon: br -> Basics.tycon -> Basics.tycon
  val chaseStr: br -> Basics.Structure -> Basics.Structure
...
struct
  ...
  open Array List DebugUtil DebugNStatic DebugNKernel
    ...
    let
      ...
      val br' = case hd (eventsAt br') of
		 STRVARev (MARKstr(VARstr(STRvar{name,...}),_,_)) =>
		   enterStruct (findSTRBind (rev (map Symbol.name name),br',0))
