...
in 
fun realfloor n = #2(if n < zero then nton n else pton n)
end
...
in  fun S y = y + y * sin__S(y*y)
	...
	let val x = drem(x,PI2)
	    fun lessPI2 x = if x>PIo4 then C(PIo2-x) else S x
	    fun positive x = if x>PIo2 then lessPI2(PI-x) else lessPI2 x
	 in if x>0.0 then positive x else ~(positive(~x))
	end
