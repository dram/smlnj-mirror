...
struct
  ...
  val exnOrd = ref(bogusCON)
  val exnRange = ref(bogusCON)
  val exnSubscript = ref (bogusCON)
  val exnFpSubscript = ref bogusCON
  ...
  fun resetCore () = 
      (exnBind := bogusCON; exnMatch := bogusCON;
       exnOrd := bogusCON; exnRange := bogusCON; exnSubscript := bogusCON;
       exnFpSubscript := bogusCON;
  ...
  fun setCore(STRvar{access=PATH p,binding,...}) =
      let fun extractPath name = 
		  ...
		  val VARbind(VALvar{access=PATH p,...}) =
			lookVARCONinStr(binding, sym, p, ErrorMsg.impossible)
	       in p
	      end
	  fun coreCon name = 
	      let val CONbind c = lookVARCONinStr(binding,name,p, ErrorMsg.impossible)
	       in c
	      end
       in exnBind := coreCon(Symbol.varSymbol "Bind");
	  exnMatch := coreCon(Symbol.varSymbol "Match");
          exnOrd := coreCon(Symbol.varSymbol "Ord");
          exnRange := coreCon(Symbol.varSymbol "Range");
          exnSubscript := coreCon(Symbol.varSymbol "Subscript");
	  exnFpSubscript := coreCon(Symbol.varSymbol "FpSubscript");
