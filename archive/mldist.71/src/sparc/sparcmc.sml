...
(*      val emit_fneg = emitFOp2 5        (* 000000101 *) *)
      ...
      val emit_fmov = emitFOp2 1        (* 000000001 *)
    in
    ...
    fun emitInstr (I_nop) = emitLong 16777216 (* really "sethi 0,%g0" *)
      ...
      | emitInstr (I_bcc(CC_A, lab)) = emit_ba lab
      | emitInstr (I_bcc(CC_E, lab)) = emit_be lab
      | emitInstr (I_bcc(CC_NE, lab)) = emit_bne lab
      | emitInstr (I_bcc(CC_L, lab)) = emit_bl lab
      | emitInstr (I_bcc(CC_LE, lab)) = emit_ble lab
      | emitInstr (I_bcc(CC_G, lab)) = emit_bg lab
      | emitInstr (I_bcc(CC_GE, lab)) = emit_bge lab
      | emitInstr (I_bcc(CC_GEU, lab)) = emit_bgeu lab
      | emitInstr (I_bcc(CC_LEU, lab)) = emit_bleu lab
      | emitInstr (I_fbcc(CC_E, lab)) = emit_fbe lab
      | emitInstr (I_fbcc(CC_NE, lab)) = emit_fbne lab
      | emitInstr (I_fbcc(CC_L, lab)) = emit_fbl lab
      | emitInstr (I_fbcc(CC_LE, lab)) = emit_fble lab
      | emitInstr (I_fbcc(CC_G, lab)) = emit_fbg lab
      | emitInstr (I_fbcc(CC_GE, lab)) = emit_fbge lab
      | emitInstr (I_jmpl args) = emit_jmpl args
      | emitInstr (I_call2) = emit_call2()
...
(*      | emitInstr (I_fneg args) = emit_fneg args *)
      ...
      | emitInstr (I_fmov args) = emit_fmov args
      | emitInstr _ = ErrorMsg.impossible "[SparcMCEmit.emitInstr]"
