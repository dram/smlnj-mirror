...
(* functor Sharing (structure EnvAccess:ENVACCESS
		 structure PrintType:PRINTTYPE
		   sharing EnvAcess.Env = PrintType.Env) : SHARING =
struct
  structure Env = EnvAccess.Env *)

structure Sharing : SHARING =
struct

  open Array List ErrorMsg PrintUtil Basics EnvAccess TypesUtil
  infix 9 sub
  ...
  fun getStr([],str,err) = str
    | getStr(id::rest,STRstr{table,env=strenv,...},err) =
	let val STRvar{binding,...} = lookSTR table id
  ...
  fun findStr(id::rest,env,table,strenv,err) : Structure =
       (let val STRvar{binding,...} = lookSTR table id
	    val str = case (binding,strenv)
			of (INDstr i,REL{s,...}) => s sub i
			 | (SHRstr(i::r),REL{s,...}) => getEpath(r,s sub i)
	...
	handle Env.Unbound =>  (* look for global structure *)
	  let val STRvar{binding,...} =
		    (lookSTR env id) handle Env.Unbound =>
		      (err COMPLAIN("unbound structure id in sharing specification: "
			      ^ Symbol.name id);
		       raise DontBother)
  ...
  fun findTycon(path,env,table,strenv,err) : tycon =
      let val (id::rpath) = rev path 
       in case rev rpath
	    of [] => ((case lookTYC table id
			 of INDtyc i =>
			      (case strenv
				of REL{t,...} => t sub i
				 | DIR => impossible "Sharing.findTycon")
			  | SHRtyc p => getEpathTyc(p,strenv)
			  | tyc => tyc)
		      handle Env.Unbound =>
		      (lookTYC env id)
		      handle Env.Unbound =>
		      (err COMPLAIN("unbound type in sharing spec: "^ Symbol.name id);
		       raise DontBother))
	     | path' => lookTYCinStr(findStr(path',env,table,strenv,err),id,(),
				     err COMPLAIN)
      end

  fun doSharing(env,
		STRstr{stamp,sign,table,env=strenv as REL{s=senv,t=tenv},
		       kind=SIGkind{share={s=strShare,t=typeShare},
			            bindings,
				    stamps=stamps as {strStamps,tycStamps}}},
	        err) =
      let 
...
((* PrintType.printTycon std_out env t1; print " ==> "; print(tfind s1); newline();
 PrintType.printTycon std_out env t2; print " ==> "; print(tfind s2); newline(); *)
...
((* PrintType.printTycon std_out env t1; newline(); 
  PrintType.printTycon std_out env t2; newline(); *)
			 ...
			 | TYCbind(tycon) =>
			    (let val tyc1 = case tycon
					      of INDtyc i => tenv1 sub i
					       | _ => tycon
			         val tyc2' = lookTYC table (tycName tyc1)
				 val tyc2 = 
				     case env2
				       of REL{t=tenv2,...} =>
					   (case tyc2'
	  ...
	  fun shareSig(REL{s,t}) =
		 ...
		 let val str as STRstr{stamp,sign,table,env=strenv as REL{s=s',...},kind} =
			   s sub i
		  in case kind
		      of SIGkind _ => 
	  ...
	  val typePathPairs = ref [] : (spath*spath) list ref
       ...
       in for strShare (fn p as (p1,p2) =>
	    let val str1 as STRstr{stamp=s1,...} = findStr(p1,env,table,strenv,err)
		and str2 as STRstr{stamp=s2,...} = findStr(p2,env,table,strenv,err)
	  ...
	  for typeShare (fn p as (p1,p2) =>
	     (case (findTycon(p1,env,table,strenv,err),
		    findTycon(p2,env,table,strenv,err))
	      ...
	      handle DontBother => ());
	  shareSig strenv;
	  STRstr{stamp=stamp,sign=sign,table=table,env=strenv,
		 kind=SIGkind{share={s= !strPathPairs,t= !typePathPairs},
			      bindings=bindings,stamps=stamps}}
      end  (* doSharing *)

  fun checkSharing(env,table,strenv,{s=strShare,t=typeShare},err) =
      (for strShare (fn p as (p1,p2) =>
	 let val STRstr{stamp=s1,...} = findStr(p1,env,table,strenv,err)
	     and STRstr{stamp=s2,...} = findStr(p2,env,table,strenv,err)
	  in if s1 <> s2
	 ...
	 let val tyc1 = findTycon(p1,env,table,strenv,err)
	     and tyc2 = findTycon(p2,env,table,strenv,err)
