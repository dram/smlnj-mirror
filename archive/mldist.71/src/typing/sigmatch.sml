...
(* functor SigMatch (structure EnvAccess:ENVACCESS
		  structure PrintType:PRINTTYPE
		  structure Sharing:SHARING
		    sharing EnvAccess.Env = PrintType.Env = Sharing.Env)
							: SIGMATCH = struct
...
open Array List List2 PrintUtil ErrorMsg Access Stampset Basics 
     BareAbsyn BasicTypes EnvAccess TypesUtil PrintType ModUtil EqTypes
infix 9 sub
...
fun compareTypes (env:env, spec: ty, actual: ty, name, err) : unit =
    let fun error() =
	    (err COMPLAIN "value type in structure doesn't match signature spec";
	     ...
	     print "\n  actual: ";
	     PrintType.printType std_out env actual; newline())
...
fun matchx (parent: Structure)
    	   ...
    	   (baseEnv, abs, path, stamps,
	    sgn as STRstr{stamp,sign,...},
	    str as STRstr{stamp=stamp',sign=sign',table,env=strenv,...},
	    param: Structure,
	    err) 
	   : Structure * thinning =
	  ...
	  (* don't use open' here, it's silly *)
	       val env = Env.open'(table,
				   transBinding{path=[v],strenv=strenv},
				   Env.empty)
	       val (str',transl) =
		     realizex parent mapfns 
			  (baseEnv,env,abs,path,stamps,stamp',sgn,param,err)
	    in (str',SOME(v,transl))
	   end)
  | matchx _ _ _ = impossible "843 in sigmatch"

and realizex (parent: Structure)
    	     (mapfns as {mapstr1,maptyc,mapstr})
	     (baseEnv, env, abs, path, stamps, strStamp,
    ...
    let val fullEnv = Env.atop(env,baseEnv)
        val sNew = array(Array.length sSig, NULLstr)
	val tNew = array(Array.length tSig, NULLtyc)
	...
	fun checkDataconSign(name,DATACON{sign=s1,...}::_,
	    ...
	    case spec
		  ...
		  (let val STRvar{access,binding=str',...} = lookSTR env id
		       val (str,thin) = matchx newstr mapfns
					    (baseEnv, false, id::path, stamps,
			...
			update(sNew,i,
				let val STRvar{binding,...}=EnvAccess.bogusSTR
				 in binding
				end);
			nil))
	       | TYCbind(INDtyc i) =>
		 ...
		 ((case lookVARCON env name
		    of CONbind(DATACON{typ=typ',rep=VARIABLE(access),...}) =>
		      (compareTypes(env ,typeInContext(typ,newenv),typ',name,err);
		       [VALtrans access])
		     | _ => raise Env.Unbound)
		   handle Env.Unbound => 
		      (err COMPLAIN("unmatched exception spec: "^symName(name));
			nil))
	       | CONbind(DATACON{name,typ,rep=VARIABLEc _,const,...}) =>
		 ((case lookVARCON env name
		    of CONbind(DATACON{typ=typ',rep=VARIABLEc(access),...}) =>
		      (compareTypes(env ,typeInContext(typ,newenv),typ',name,err);
		       [VALtrans access])
		     | _ => raise Env.Unbound)
		   handle Env.Unbound => 
		  ...
		  ((case lookVARCON env name
		     of CONbind (DATACON{typ=typ',...}) =>
			 compareTypes(fullEnv,typeInContext(typ,newenv),
					typ',name,err)
 	              | _ => raise Env.Unbound)
		    handle Env.Unbound =>
	...
	val _ = Sharing.checkSharing(fullEnv,table,newenv,share,err)
