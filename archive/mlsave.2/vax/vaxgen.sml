...
(*   (pop(displace(4*i,r1)); dec offset) *)

    fun startrecord(words) =
	(savewords := words)
(*      (movl(immed(4*words),direct r0); callprim(0)) *)

    fun endrecord () =
	(movl(immed(8*(!savewords)+1), displace(~4,r12));
	 push(direct(r12));
	 addl2(immed(4*(!savewords)+4), direct(r12));
	 inc offset)
(* (push(direct r1); inc offset) *)
