...
(*
    ("create", 1)
	::= (fn () =>
	       (pop(Direct d0);    (* length *)
		asrl(Immed 1, Direct d0);
		movl(Direct d0, Direct d2);
		addl(Immed(11), Direct d0);
	        asrl(Immed 2,Direct d0);
		asll(Immed 2,Direct d0);
		movl(Direct d0, Direct d1);
		addl(Direct d1, Direct d1);
		subl(Immed 3, Direct d1);
	        movl(Immed 0,Index(a6,~8,d0,Long));
		movl(Direct d2, Displace(a6,0));
		movl(Direct d1, Displace(a6,~4));
		push(Direct a6);
		addl(Direct d0, Direct a6) ));
*)		
    ("ordof", 2) ::= (fn () =>
       (pop(Direct d1);    (* index *)
...
(* fneg, fadd, fsub, fmul, and fdiv all might add a new gc fault *)
    ("fneg", 1)
       ::= (fn () =>
		(pop(Direct a0);
		 fnegd(Displace(a0,0),Direct fp0);
		 fmoved(Direct fp0,Displace(a6,0));
		 movl(Immed 21,Displace(a6,~4));
		 push(Direct a6);
