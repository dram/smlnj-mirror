...
and printExp(_,_,_,0) = print "<exp>"
  ...
  | printExp(MARKexp (exp,s,e), cp2lnf,ind, d) = 
      (if !markprint then 
        (print "MARKexp("; printExp(exp,cp2lnf,ind,d-1); print ","; prlineno(cp2lnf s); print ","; prlineno(cp2lnf e); print ")")
...
and printVB(VB{pat,exp,tyvars},cp2lnf,ind,d) =
    (print "VB{pat="; printPat(pat,d-1); print ",exp="; printExp(exp,cp2lnf,ind+2,d-1);
	print ",tyvars=["; (printvseq 0 "," (fn x=> printTyvar(x)) tyvars); print "]}")

and printRVB(RVB{var,exp,resultty,tyvars},cp2lnf,ind,d) = 
...
and printDec(_,_,_,0) = print "<dec>"
  ...
  | printDec(DATATYPEdec{datatycs,withtycs},cp2lnf,ind,d) =
      (print "DATATYPEdec{datatycs=[";
       printvseq (ind+5) ", "
         (fn GENtyc{path=name::_, arity, kind=DATAtyc(ref dcons),...} =>
	     (case arity
		of 0 => ()
		 | 1 => (print "'a ")
		 | n => (printTuple print (typeFormals n); print " ");
  ...
  | printDec(ABSTYPEdec _,cp2lnf,ind,d) = print "ABSTYPEdec(...)"
  ...
  | printDec(MARKdec (dec,s,e), cp2lnf,ind, d) = 
      if !markprint then
        (print "MARKdec("; printDec(dec,cp2lnf,ind,d-1); print ","; prlineno(cp2lnf s); print ","; prlineno(cp2lnf e); print ")")
...
and printStrexp(_,_,_,0) = print "<strexp>"
  ...
  | printStrexp(MARKstr(body,s,e),cp2lnf,ind,d) = 
	...
	(print "MARKstr("; printStrexp(body,cp2lnf,ind,d); print ","; 
		prlineno (cp2lnf s); print ","; prlineno (cp2lnf e); print ")")
      else printStrexp(body,cp2lnf,ind,d)

and prlineno (file:string,line:int,pos:int) = 
		(print line; print "."; print pos)
