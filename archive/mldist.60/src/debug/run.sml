...
struct
  ...
  *)
	  ...
	  states := delete(!states,zaptime)  (* exception here impossible *)
       end
     else ();
     inc usenum;
  ...
  fun zap () =
    (update(times,0,!initialTime);  (* currentTime *)
     finalTime := !initialTime;
     HistoryIO.zap (!initialTime + 1);
     HistoryStore.zap (!initialTime + 1); 
     zapStates (!initialTime + 1))
  ...
  fun init f =
    let fun rununit ()=
	  let val result = NORMAL (f()) handle exn => EXCEPTION exn
	  in case result of
	       NORMAL _ =>
    ...
    in
      let val p as (ptime,pstate) = findPrevState(!initialTime)
      in restore p;
	 if (ptime < !initialTime) then 
	   (update (times,1,!initialTime);   (* targetTime *)
	    HistoryIO.silent := true;
  ...
  fun bbreak(args:System.Unsafe.object array) = 
     (callcc(fn cont =>  (currentCont := cont; 
			  ...
			  case (hd o eventFor o #evn) (!currentEvdata) of
			    NULLev => if currentTime() > !initialTime then
					finalTime := currentTime()
				      else ()
			  | _ => ();
			  throw (!debugCont) ()));
  ...
  fun gotoTime(time:int,error:int,basetime:int,stopOnEnd:bool) =
    ...
    let	fun forceStop () =
	  case (hd o eventFor o #evn) (!currentEvdata) of
	    NULLev => (currentTime() > !initialTime) andalso stopOnEnd
	...
	in if (!debugdebug) then
	   ...
	   then (* move forwards *)
	     (stoppedByException := false;
	      while (time > currentTime() andalso (not (!stoppedByException))
  ...
  fun advanceTime (time:int, error:int, capture:bool, stopOnIO:unit->bool) =
    (HistoryIO.silent := false;
     ...
     gotoTime (time, error, HistoryIO.nexttime(), capture))

  fun restoreTime (time:int, error:int) =
    (HistoryIO.silent := true;
     HistoryIO.usebreak := (fn () => false);
     stopOnException := true;
     gotoTime (time, error, infinity, true) before
       (if not (currentTime() = time) then
