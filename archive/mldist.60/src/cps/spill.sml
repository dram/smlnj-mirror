...
let 
  val constant = let val s = Intset.new()
		  in app (Intset.add s o #1 o #1) carg;
		     Intset.mem s
	         end
	...
	let exception SpillFreemap
	    ...
	    val _ = app (freemap (Intmap.add m) o #3 o #1) carg
         in sublist (not o constant) o (Intmap.map m)
        end
  ...
  fun f(results : lvar list,
	  ...
	  cexp : cexp) =
    let fun constant'(VAR v) = constant v
	  | constant' _ = true
	 ...
	 let val rec free =
	      fn SWITCH(v,l) => foldmerge(clean[v] :: map free l)
	       ...
	       | APP(f,vl) => clean(if constant' f then vl else f::vl)
	   in case cexp
