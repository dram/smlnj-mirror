...
fun contract last (fvar, fargs, cexp) =
 let datatype arity = BOT | COUNT of int | TOP | NOTUSED
     ...
     let val rec g' =
	  ...
	  in if esc = sum flatfun fl andalso !CG.deadvars
	    ...
	    (case (used w, !CG.deadvars, ren v, !CG.selectopt)
	      of (false, true, _, _) => (click "Sel "; g' e)
	       | (_,_, v' as VAR y, true) => 
		      (case get y
	...
	| PRIMOP(i, vl, wl, el as [e1,e2]) => 
     ...
     and primops =
	fn (P.boxed, [INT _],_,_::b::_) => (click "A"; b)
	 | (P.boxed, [STRING s],_,a::b::_) =>
			    (click "A"; if size s = 1 then b else a)
	 | (P.boxed, [VAR v],_,a::_) => 
             (case get v
	       of  {info=RECinfo _, ...} => (click "A"; a)
	        | _ => raise ConstFold)
         | (P.<, [INT i, INT j],_,[a,b]) =>
	 ...
	 | (P.-, [INT i, INT j], [w], [c]) =>
		  (let val x = i-j
		   in x+x; newname(w,INT x); click "K"; c
		   end handle Overflow => raise ConstFold)
	 | (P.rshift, [INT i, INT j],[w],[c]) =>
			   (click "L"; newname(w,INT(Bits.rshift(i,j))); c)
	 | (P.rshift, [INT 0, _],[w],[c]) =>
			   (click "L"; newname(w,INT 0); c)
	 | (P.rshift, [v, INT 0],[w],[c]) =>
			   (click "L"; newname(w,v); c)
         | (P.slength, [INT _],[w],[c]) =>
			 (click "M"; newname(w, INT 1); c)
	 | (P.slength, [STRING s], [w],[c]) =>
			 (click "M"; newname(w, INT(size s)); c)
         | (P.ordof, [STRING s, INT i],[w],[c]) =>
			 (click "N"; newname(w, INT(ordof(s,i))); c)
         | (P.~, [INT i], [w], [c]) =>
		     (let val x = ~i
	 ...
	 | (P.lshift, [INT i, INT j],[w],[c]) =>
			   (let val x = Bits.lshift(i,j)
			    in x+x; newname(w,INT x); click "P"; c
			    end handle Overflow => raise ConstFold)
	 | (P.lshift, [INT 0, _],[w],[c]) =>
			   (click "P"; newname(w,INT 0); c)
	 | (P.lshift, [v, INT 0],[w],[c]) =>
			   (click "P"; newname(w,v); c)
	 | (P.orb, [INT i, INT j],[w],[c]) =>
			(click "Q"; newname(w,INT(Bits.orb(i,j))); c)
	 | (P.orb, [INT 0, v],[w],[c]) =>
			(click "Q"; newname(w,v); c)
	 | (P.orb, [v, INT 0],[w],[c]) =>
			(click "Q"; newname(w,v); c)
	 | (P.xorb, [INT i, INT j],[w],[c]) =>
			 (click "R"; newname(w,INT(Bits.xorb(i,j))); c)
	 | (P.xorb, [INT 0, v],[w],[c]) =>
			(click "R"; newname(w,v); c)
	 | (P.xorb, [v, INT 0],[w],[c]) =>
			(click "R"; newname(w,v); c)
	 | (P.notb, [INT i], [w], [c]) =>
			 (newname(w,INT(Bits.notb i)); click "S"; c)
	 | (P.andb, [INT i, INT j],[w],[c]) =>
			 (click "T"; newname(w,INT(Bits.andb(i,j))); c)
	 | (P.andb, [INT 0, _],[w],[c]) =>
