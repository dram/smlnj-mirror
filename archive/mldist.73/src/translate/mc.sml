...
signature MC = sig
(*	structure Env *)
	val matchCompile : Modules.env -> (BareAbsyn.pat * Lambda.lexp) list * (string->unit) -> Lambda.lexp
	val bindCompile : Modules.env -> (BareAbsyn.pat * Lambda.lexp) list * (string->unit) -> Lambda.lexp
	end

(* functor MC (structure MCprint:MCPRINT
	    structure PrintAbsyn:PRINTABSYN
	      sharing MCprint.Env = PrintAbsyn.Env): MC = struct

structure Env = PrintAbsyn.Env *)

structure MC : MC = struct

open BareAbsyn Lambda
open Variables Types BareAbsyn Lambda
open Access ErrorMsg

fun constant(DATAcon(_,CONSTANT _)) = true
  | constant(DATAcon(_,VARIABLEc _)) = true
  | constant(DATAcon(_,_)) = false
  | constant _ = true
...
fun layer (x,CONSTRAINTpat(pat,_),rhs) = layer(x,pat,rhs)
  | layer (x,VARpat(VALvar{access=PATH[v],...}),rhs) = bind(x,v,rhs)
...
val rec freevars =
    fn VARpat(VALvar{access=PATH[v],...}) => [v]
...
fun andS [] = ([],[])
  | andS ((p::fields,rhs)::more) =
	...
	in  (addto cases,default)
     ...
     | CONpat(dcon as DATACON{rep=r1,...}) =>
	let val (cases,default) = andS more
	  fun addto ((switch as (d as DATAcon(_,r2), pats))::more) =
		  if r1=r2
		    then (d,(fields,rhs)::pats)::more
		    else switch :: addto more
	    | addto [] = [(DATAcon (transDcon dcon),(fields,rhs)::default)]
	    | addto _ = impossible "87 type error in match compiler"
	in  (addto cases,default)
	end
     | APPpat(dcon as DATACON{rep=r1,...},p) =>
	let val (cases,default) = andS more
	fun addto ((switch as (d as DATAcon(_,r2), pats))::more) =
		if r1=r2
		then ((d, (p::fields,rhs)::pats)::more)
		else switch :: addto more
	  | addto [] =
		let fun addwild (fields,rhs) = (WILDpat::fields,rhs)
		in  [(DATAcon(transDcon dcon), (p::fields,rhs)::(map addwild default))]
		end
	...
	in  (addto cases,default)
     ...
     | WILDpat =>
	let val (cases,default) = andS more
	fun addto ((con,pats)::more) =
               if constant con then (con,(fields,rhs)::pats) :: addto more
		else (con,(WILDpat::fields,rhs)::pats) :: addto more
	  | addto [] = []
	in  (addto cases,(fields,rhs)::default)
	end
     | VARpat(VALvar{access=PATH[v],...}) => 
...
let fun diffPats samefn =
	let fun diff [] = []
		 ...
		 case p of
		    WILDpat => [hd]
		  | VARpat(VALvar{access=PATH[v],...}) => [(WILDpat,bind(x,v,rhs))]
		  | LAYEREDpat(v,p) =>
...
fun orS [] =  impossible "orSwitch [] in mc"
  | orS (arg as (p,rhs)::more) =
    ...
    | VARpat(VALvar{access=PATH[v],...}) => ([],SOME(convert(bind(x,v,rhs))))
...
fun mcand (arg as ([_],_)::_,[x]) =
  ...
  | mcand (arg as (p::fields,rhs)::more,xl as x::xs) =
    let fun mconto (con,pats) =
	    if constant con 
            then (con,mcand (MCopt.opt (pats,xs)))
            else let val DATAcon con1 = con
	             val new = mkLvar ()
	          in  (con,APP(FN(new,mcand (MCopt.opt (pats,new::xs))),
			  DECON'(con1, VAR x)))
	         end
    in
     case p of
       WILDpat => mcand([(fields,rhs)],xs)
     | VARpat(VALvar{access=PATH[v],...}) => mcand([(fields,bind(x,v,rhs))],xs)
	...
	in  APP(FN(newx,mcand(MCopt.opt(list,newx::xs))),
		DECON'(dcon,VAR x))
	end
...
fun conS [] = ([],NONE)
  | conS (arg as (p,rhs)::more) =
	...
	let fun diff [] = []
	      | diff ((hd as (p,rhs))::more) =
		 case p of
		  CONpat(DATACON{rep=r2,...}) =>
			if r1=r2 then diff more
			else hd::diff more
	...
	in  ((DATAcon(transDcon dcon),convert rhs)::cases,default)
	end
     | APPpat(dcon as DATACON{rep=r1,...},_) =>
	let fun divide [] = ([],[])
...
and mcor [] = impossible "mcor.[] in mc"
  | mcor (arg as (p,rhs)::more) =
      ...
      in  case p of
		...
		in  areNotRedundant();
		    FN(x,SWITCH(VAR x,nil,cases,default))
		end
	    ...
	    | CONpat(DATACON{sign,...}) =>
		let val (cases,default) = conSwitch x arg
		in  FN(x,SWITCH(VAR x, sign, cases,
		...
		in  FN(x,SWITCH(VAR x,nil,cases,default))
		end
	    | REALpat _ =>
		let val (cases,default) = orSwitch x arg
		in  FN(x,SWITCH(VAR x,nil,cases,default))
...
open PrintUtil
fun matchPrint _ [] _ _ = ()
  | matchPrint _ [(pat,_)] _ _ = () (* never print last rule *)
  | matchPrint env ((pat,_)::more) [] _ =
	(print "        "; PrintAbsyn.printPat env (pat,!printDepth); print " => ...\n";
	 matchPrint env more [] 0)
  | matchPrint env ((pat,_)::more) (taglist as (tag::tags)) i =
	if i = tag
	then (print "  -->   "; PrintAbsyn.printPat env (pat,!printDepth);
	      print " => ...\n"; matchPrint env more tags (i+1))
	else (print "        "; PrintAbsyn.printPat env (pat,!printDepth);
	      print " => ...\n"; matchPrint env more taglist (i+1))

fun bindPrint env ((pat,_)::_) =
      (print "        "; PrintAbsyn.printPat env (pat,!printDepth); print " = ...\n")
  | bindPrint _ _ = impossible "bindPrint in mc"
...
fun genCompile (env,flag1,warning1,flag2,test,warning2,printer) (rules,warn) = 
let val _ = if !printArgs
	then (warn "MC called with:"; MCprint.printMatch env rules)
	else ()
    val (header,rules') = setup rules
...
fun bindCompile env =
    genCompile(env,
	       ...
	       fn(rules,unused) => bindPrint env rules)

fun matchCompile env =
    genCompile(env,
               matchExhaustive,
    	       "match",
	       matchRedundant,
	       ...
	       fn(rules,unused) => matchPrint env rules unused 0)
