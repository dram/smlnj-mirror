...
(* commented out because this check appears useless *)
	  fun length (Assembly.FUNC(_,_,r)) = InLine.+(1,length r)
	    | length _ = 0
	  exception Foo
	  in
(*	    if InLine.ieql(length Assembly.external,51) then *)
...
structure Signals : SIGNALS = 
   ...
   *)
    ...
    val maskSignals = Assembly.A.masksigs
...
structure Compile =
  struct
    open InLine Ref
    val compile_ref :(string -> string -> unit) ref =
          ref (fn (_:string) => (fn (_:string) => ()))
    val compile_and_load_ref :(string -> string -> unit) ref =
          ref (fn (_:string) => (fn (_:string) => ()))
    val load_target_ref :(string -> unit) ref = ref (fn (_:string) => ())
    val load_source_ref :(string -> unit) ref = ref (fn (_:string) => ())
    val validTarget_ref : (string -> bool) ref = ref (fn (_:string) => true)
    val openCompilingEnv_ref :(unit -> unit) ref = ref (fn () => ())
    val resetCompilingEnv_ref :(unit -> unit) ref = ref (fn () => ())

    fun clearRefs () =
     (compile_ref  := (fn (_:string) => (fn (_:string) => ()));
      compile_and_load_ref  := (fn (_:string) => (fn (_:string) => ()));
      load_target_ref := (fn (_:string) => ());
      load_source_ref := (fn (_:string) => ());
      validTarget_ref := (fn (_:string) => true);
      openCompilingEnv_ref :=  (fn () => ());
      resetCompilingEnv_ref :=  (fn () => ()))

    fun compile x y = !compile_ref x y
    fun compileAndLoad x y = !compile_and_load_ref x y
    fun loadTarget x = !load_target_ref x
    fun loadSource x = !load_source_ref x
    fun validTarget x = !validTarget_ref x
    fun openCompilingEnv () = !openCompilingEnv_ref ()
    fun resetCompilingEnv () = !resetCompilingEnv_ref ()
  end (* Compile *)
...
structure System : SYSTEM =
  ...
  abstraction CC : sig type 'a control_cont end =
	...
	val compile_ref = Compile.compile_ref
        val compile_and_load_ref = Compile.compile_and_load_ref
        val load_target_ref = Compile.load_target_ref
        val load_source_ref = Compile.load_source_ref
        val validTarget_ref = Compile.validTarget_ref
        val openCompilingEnv_ref = Compile.openCompilingEnv_ref
        val resetCompilingEnv_ref = Compile.resetCompilingEnv_ref

	exception Top_level_callcc


        val do_it = 
	    let val callcc:('a cont -> 'a) -> 'a = cast InLine.callcc
	    in callcc(fn c0 => 
                 let val (f,arg,id) = 
                         callcc(fn c => InLine.throw c0 (InLine.throw c))
	         val v = (f arg) handle e => 
                           let val ref(op :: ((k,h,_),r)) = cast compreturn
			   in InLine.:=(compreturn, cast r); h e
			   end
		 val ref(op ::((k,h,id'),r)) = cast compreturn
	      in InLine.:=(compreturn, cast r);
...
(* NOTE: this code could be optimized by examining the directory entries incrementally *)
    ...
    open PreLim
    val version = "Standard ML of New Jersey, Version 0.73, 10 September 1991"
...
structure IO : IO = IO
structure Compile : COMPILE = Compile
...
structure RealArray = struct
  ...
  in
    ...
    exception Size
    type realarray = ByteArray.bytearray
    val length : realarray -> int = fn fpa => div(InLine.slength fpa, 8)
...
structure Real =
  ...
  struct open System
	 ...
	 struct open Unsafe
		...
		val capture : ('1a control_cont -> '1a) -> '1a = InLine.capture
