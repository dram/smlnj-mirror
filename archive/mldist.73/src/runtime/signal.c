 ...
 */
void sig_setup (MLState)
     MLState_ptr MLState;
{
    register int    nextSig = MLState->SigCode;

  /* determine which signal to handle */
    do {
	if ((nextSig += 1) == NUM_ML_SIGS)
	    nextSig = 0;
    } while (MLState->SigTbl[nextSig] == 0);

  /* record the signal and count */
    MLState->SigCode	    = nextSig;
    MLState->SigCount	    = MLState->SigTbl[nextSig];
    MLState->NumPendingSigs  -= MLState->SigCount;
    MLState->SigTbl[nextSig] = 0;

    MLState->handlerPending  = 0;
    MLState->inSigHandler    = 1;
    MLState->request 	     = REQ_SIGNAL;
    ...
    struct sigcontext *scp;
{
	...
	mp_shutdown(MLState,1);
      }
    } else {

    /* record the signal */
      MLState->NumPendingSigs++;
      MLState->SigTbl[ml_sig]++;

      if (!MLState->maskSignals) {
	if (MLState->ioWaitFlag) {
	   ...
	   * so longjmp out of the operation (see io_wait() in "cfuns.c"). */
	    _longjmp (MLState->SysCallEnv, 1);
	}
	else if (MLState->inML && (! MLState->handlerPending) && 
		 (! MLState->inSigHandler)) {
#ifndef C
	    register int    pc = scp->sc_pc;
#endif

	    MLState->handlerPending	= 1;
#ifdef MIPS
 ...
 * was recorded.
 */
int gcsignal (MLState)
    MLState_ptr MLState;
{
    if (siginfo[ML_SIGGC].state == ML_SIG_ENABLED) {
	MLState->NumPendingSigs++;
	MLState->SigTbl[ML_SIGGC]++;
	return 1;
    }
    else
	return 0;
 ...
 */
void setup_signals (MLState, first_time)
     MLState_ptr MLState;
     int         first_time;
{
    int			ml_sig, sig;
 ...
 * 
 */
ML_val_t make_ml_sigh_arg (MLState)
     MLState_ptr MLState;
{
  ...
  /* allocate the ML signal handler's argument record */
    REC_ALLOC3(arg, INT_CtoML(MLState->SigCode), 
	       INT_CtoML(MLState->SigCount), resume_c);
