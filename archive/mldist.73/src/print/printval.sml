...
open System System.Unsafe BasicTypes TypesUtil ErrorMsg PrintUtil
...
fun printVal (env:Modules.env) (obj: object, ty: ty, depth: int) : unit =
      printVal'(env, obj, ty, depth, noparen, noparen)

and printVal' (_,_,_,0,_,_) = print "#"
  | printVal' (env: Modules.env,obj: object,ty: ty,depth: int,l: fixity,r: fixity) : unit =
    case ty
...
and printDcon(_,_,_,_,0,_,_) = print "#"
  | printDcon(env:Modules.env,obj:object, tyc as GENtyc{arity,path,...}, argtys,
	...
	then print dname
	  ...
	  let val fixity = ModuleUtil.lookFIX(env,Symbol.fixSymbol (Symbol.name name)) (* (??) may be inaccurate *)
	      val dom = case typ
...
and printList(env:Modules.env, obj:object, ty:ty, depth:int, length: int) : unit =
...
and printTuple(env:Modules.env, objs: object array, tys: ty list, depth:int) : unit =
...
and printRecord(env:Modules.env, objs: object array, labels: label list, tys: ty list, depth:int) =
