...
and printTycBind(env,tyc,indent) =
   let fun visibleDcons(tyc,dcons) =
			...
			fun find ((actual as DATACON{name,...}) :: rest) =
			    (let val found =
				      checkCON(ModuleUtil.lookVARCON(env,[name],fn _ => raise Env.Unbound))
			      in if TypesUtil.eqTycon(dconTyc actual,
						      dconTyc found)
   ...
   in tab indent;
      if !internals then
       ...
       (case tyc'
        ...
        | DEFtyc{path=name::_,tyfun=TYFUN{arity,...},...}=> showit(arity,name)
