...
(*       fun emit_fneg args = C.emit (SparcInstr.I_fneg args) *)
	...
	fun mkLabExp (lab, n) = LABELexp{base= C.baseLab, dst= lab, offset= (n-4096)}
        val I'm_Relative = ref(SparcInstr.Relative)  (* be crude for now *)
      in
      ...
      fun setBaseAddr () = let
	    ...
	    in
	      C.define lab;
	      C.emitSDI (
	        SparcInstr.SetBaseAddr(I'm_Relative, mkLabExp(lab, 0)))
	    end

      fun loadAddr (lab, n, dst, tmpR) = (
	    C.emitSDI (SparcInstr.LoadAddr(mkLabExp(lab, n), dst, tmpR)))
  ...
  (** Dedicated registers **)
    ...
    val miscregs = map (Direct o REG) [     (* %g1-%g3, %o0-%o1, %l0-%l7, %i4 *)
	  1, 2, 3, 8, 9, 16, 17, 18, 19, 20, 21, 22, 23, 28
	  ]
	(*
  ...
  (* move (src, dst) *)
    fun move (Immed n, Direct r) = loadImmed (n, r)
      ...
      | move (FDirect (FREG fs), FDirect (FREG fd)) = 
	let fun even x = x >= 0 andalso x div 2 = 0
	in if even fs andalso even fd 
	   then (emit_fmov(FREG fs, FREG fd);
		 emit_fmov(FREG (fs+1), FREG (fd+1)))
	   else ErrorMsg.impossible 
	       "sparc/sparc/move: Bad floating point registers"
	end
 ...
 **)
      ...
      | storeindexl _ = ErrorMsg.impossible "[SparcCM.storeindexl]"
    					...
    					(* fn(x,y,z) => y <- mem[x+4*(z-1)] *)
    fun fetchindexd(Direct x,FDirect(FREG fp),z) =
	(case z
	  of Direct z' => 
	      let val tmpR = getTmpReg()
	      in  emit_sll(z',IMrand 2,tmpR);
	   ...
	   | _ => ErrorMsg.impossible "sparc/sparc/fetchindexd: bad index")
      | fetchindexd _ = ErrorMsg.impossible "sparc/sparc/fetchindexd"

					(* mem[y+4*(z-1)] <- x *)
    fun storeindexd(FDirect(FREG fp),Direct y,z) =
	(case z 
	  of Direct z' => 
	      let val tmpR = getTmpReg()
	      in  emit_sll(z',IMrand 2,tmpR);
	   ...
	   | _ => ErrorMsg.impossible "sparc/sparc/storeindexd: bad index")
      | storeindexd _ = ErrorMsg.impossible "sparc/sparc/storeindex"
  ...
  (* rangeChk (a, b, lab):  pc <- lab if ((a < 0) or (b <= a)) *)
	...
	fun fetchReal (Direct r, FREG i) = 
	  ...
	  | fetchReal (ImmedLab lab, dst) = let val tmpR = getTmpReg()
					    in loadF (lab, 0, dst, tmpR);
						freeTmpReg tmpR
					    end
	  | fetchReal _ = ErrorMsg.impossible "[SparcCM.fetchReal]"
	fun floatOp fOp (FDirect Op1,FDirect Op2,FDirect Op3) = fOp(Op1,Op2,Op3)
	  | floatOp _ _ = 
	    ErrorMsg.impossible "Bad floating point registers to arith Op"
    in
    fun loadfloat(src,dst) =
	case dst of
	    FDirect fpr => fetchReal(src, fpr)
	  | _ => ErrorMsg.impossible "sparc: bad destination for loadfloat"
    fun storefloat(src,dst) =
	case (src,dst) of
	    (FDirect(FREG fpr),Direct(REG gpr)) => 
		let val tmpR = getTmpReg()
		in loadImmed (System.Tags.desc_real, tmpR);
	  ...
	  | _ => ErrorMsg.impossible "bad registers in sparc/sparc/storefloat"
    val addf = floatOp emit_fadd
    val subf = floatOp emit_fsub
    val mulf = floatOp emit_fmul
    val divf = floatOp emit_fdiv
    fun gbranch (cond, FDirect fp1, FDirect fp2, ImmedLab lab) = 
	(emit_fcmp(fp1,fp2);
