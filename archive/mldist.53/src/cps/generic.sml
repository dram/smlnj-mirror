...
let 
    val badReals = ref (nil : string list)
    exception Regbind
	    ...
	    of (_, STANDARD(ref NONE)) => ()
	     ...
	     | (lab, CONSTfrag(REALconst r)) =>
		    (align(); mark(); emitlong(8 * power_tags + tag_embedded);
		     ...
		     realconst r handle M.BadReal s => (badReals := s :: !badReals;
							realconst "0.0"))
  ...
  (* generate a new function header *)
    and genFun (lab, cexp, closure) = let
	  ...
	  in
	    root1 := root cexp;
	    define lab;
	    (* if (maxAllocSz = 0) then () else *)
  	    checkLimit (maxAllocSz);
...
(* the next case must be done (by all machines) with v and x in
   root registers (for offset computations in "boot") *)
	 ...
	 in alloc(x,e,any,fn x' => (case (isimmed v', isimmed w',!CG.arithopt)
	    of(SOME k,_,true) => xorb(immed(k-1), w', arithtemp)
	    | (_,SOME k,true) => xorb(v', immed(k-1), arithtemp)
	 ...
	 in alloc(x,e,any, fn x' => 
	  ...
	  (addl3(immed(~1),v',arithtemp);
       ...
       | P.rshift => (fn ([v,w],[x],[e]) =>
         let val v' = regbind v and w' = regbind w
         in alloc(x,e,v', fn x' => 
            (case (isimmed w',!CG.arithopt) of
	       (SOME k,true) => ashr(immed(Bits.rshift(k,1)), v', x')
...
in  emitlong 1; (* Bogus tag for spacing, boot_v. *)
    ...
    before case !badReals of nil => () | s => raise BadReals s
end
