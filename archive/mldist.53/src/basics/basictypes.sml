...
val arrayTycon = ref(mkABStyc([arraySym],1,OBJ,Stampset.globalStamps))
...
val refDcon = 
    DATACON
      {name = refSym,
       const = false,
       rep = REF,
       typ = ref(POLYty {sign = [{weakness=1,eq=false}], tyfun = refTyfun, abs=0}),
       sign = [REF]}
val _ = refTycon := mkDATAtyc([refSym],1,[refDcon],OBJ,Stampset.globalStamps)
...
val consDcon =
    DATACON 
      {name = consSym,
       ...
       rep = TRANSB,
       typ = ref(POLYty
		  {sign = [{weakness=infinity,eq=false}], abs=0,
		   tyfun = TYFUN
			    {arity = 1,
