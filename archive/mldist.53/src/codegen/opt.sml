...
sig
  ...
  val freevars: L.lexp -> int list
  val greduce : (L.lexp -> L.lexp option) -> L.lexp -> L.lexp
...
fun greduce oracle exp =
	  ...
	  fun makp [v] = ((case imap v of VAR w => makp[w]
	    ...
	    | makp (i::p) = i :: makp p
	  fun makcon (DATAcon(DATACON{rep=(VARIABLE(PATH p)),
	    ...
	    | makcon (c,e) = (c, mak e)
	  and mak exp =
		case oracle exp of SOME exp' => (click(); mak exp')
		| NONE => case exp
	     of FN(v,b as APP(l,VAR w)) =>
		...
		let val e' = mak e
	      ...
	      | APP(PRIM P.force, APP(PRIM P.delay, a)) => (click(); mak a)
	      | FIX([],[],b) => (click(); mak b)
		...
		let val e' = mak e
		 in mak(switch(SWITCH(e', l, d)))
	      ...
	      | e as PRIM _ => e
	 val exp' = mak exp
      in  if !clicked then greduce oracle exp' else exp'
      end

val reduce = greduce (fn _ => NONE)
