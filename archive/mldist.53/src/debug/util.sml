...
struct
  ...
  fun patvars f p =
    let fun patv (VARpat(v as VALvar{access = LVAR _,name,typ})) = [f v]
	  | patv (VARpat(VALvar{access = INLINE _,...})) = []
	  | patv (VARpat _) = impossible "non-LVAR in DebugUtil.patvars"
  ...
  local 
    ...
    fun c_function s = 
        let exception C_function_not_found of string
		    ...
		    if s=t then p
		    else if t="xxxx" then raise (C_function_not_found s)
		    else f rest
            val cfun = f Assembly.external
        in fn x => Assembly.A.callc(cfun,x)
        end

    val stor = c_function "stor"
  in 
    fun forcegc () = (stor false; ())
  end
