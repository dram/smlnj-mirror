...
open List2 PrintUtil ErrorMsg Access Stampset Basics BareAbsyn BasicTypes
     EnvAccess EnvAccess.Env TypesUtil PrintType ModUtil EqTypes
	...
	fun abstractTyc(sigtyc,strtyc) =
	    case sigtyc
	      of TYCON{kind=DATAtyc _,...} => strtyc
	       | TYCON{kind=ABStyc NULLtyc,stamp,arity,eq,path} => 
			if tycFixed stamp then strtyc
			  else TYCON{kind=ABStyc strtyc, 
				     stamp=transTycStamp stamp,
				     arity=arity,eq=eq,path=path}
		 ...
		 (let val sigTycon = tSig sub i
		   ...
		   in update(tNew,i,strTycon);
		      ...
		      then if equalTycon(sigTycon,strTycon)
			   then maptyc(s,strTycon)
			   else err COMPLAIN("bad match for fixed type spec "
				        ^ symName(name))
				 ...
				 TYCON{arity=arity',kind=DATAtyc dcons',...}) =>
				  ...
				  then (checkDataconSign(name,dcons,dcons');
				        maptyc(s,strTycon))
				  else err COMPLAIN("mismatching datatype spec: "
					       ^ symName(name))
			      | (TYCON{kind=DATAtyc _,...}, _) => 
				  err COMPLAIN("unmatched datatype spec: "^symName(name))
			      | (TYCON{arity,kind=ABStyc _,eq,...}, _) =>
				  (if arity <> tyconArity(strTycon)
				   then err COMPLAIN("mismatching tycon arities: "
					       ^ symName(name))
				   else if (!eq=YES) andalso not(isEqTycon(strTycon))
				        then err COMPLAIN("mismatched eqtype spec: "
					       ^ symName(name))
					else ();
				   maptyc(s,strTycon))
