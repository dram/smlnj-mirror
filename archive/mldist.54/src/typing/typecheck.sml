...
local datatype absp = A of int*int*int*absp| C of int*int*int*absp | N
in
    ...
    case pat
      of WILDpat => VARty(mkTyvar tvkind)
       | VARpat(VALvar{typ = refty,...}) => 
			(refty := VARty(mkTyvar tvkind); !refty)
       | INTpat _ => intTy
       | REALpat _ => realTy
	   ...
	   let val VARpat(VALvar{typ = refty,...}) = varPat
	    in refty := patType(pat,tvkind,loc);
	       !refty
	   end
    ...
    case decl
      of VALdec vbs =>
 	   ...
 	   let fun setType(RVB{var=VALvar{typ,...}, resultty=NONE, ...}) =
		 ...
		 | setType _  = impossible "typecheck.783"
		   ...
		   let val VALvar{typ = refty,...} = var
		       val ety = expType(exp,Abs(Rator occ),loc)
       ...
       | ABSTYPEdec{abstycs,withtycs,body} => 
	     let val withtycons = map (fn TB{tyc,...} => !tyc) withtycs
		 val checkeq = EqTypes.defineEqTycon (fn x => x)
		 fun makeAbstract((tr as ref(TYCON{stamp,arity,eq,path,...}))
			          :: rest) =
		      let val abstyc = TYCON{stamp=stamp,arity=arity,path=path,
					     eq=ref NO, kind=ABStyc(!tr)}
		       in tr := abstyc;
			  makeAbstract(rest)
		      end
		  | makeAbstract [] = ()
		  | makeAbstract _ = impossible "makeAbstract"
	      in decType0(body,occ,loc);
		 makeAbstract(abstycs);
		 (* recompute equality attribute for withtycs *)
		 app (fn TYCON{eq,...} => (eq := MAYBE)) withtycons;  
		 app checkeq withtycons
	     end
