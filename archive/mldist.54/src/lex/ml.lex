...
<INITIAL>"'"{idchars}*	=> (Tokens.TYVAR(Symbol.symbol yytext,yypos,
					yypos+size yytext));
<INITIAL>({sym}+|{id})	=> (Tokens.ID(Symbol.symbol yytext, yypos,
					yypos+size yytext));
<INITIAL>{real}	=> (Tokens.REAL(yytext,yypos,yypos+size yytext));
<INITIAL>[1-9][0-9]* => (Tokens.INT(makeInt yytext
		    handle Overflow => (err (yypos,yypos+size yytext)
					  COMPLAIN "integer too large"; 1),
			yypos,yypos+size yytext));
<INITIAL>{num}	=> (Tokens.INT0(makeInt yytext
		    handle Overflow => (err (yypos,yypos+size yytext)
					  COMPLAIN "integer too large"; 0),
			yypos,yypos+size yytext));
<INITIAL>~{num}	=> (Tokens.INT0(~(makeInt(substring(yytext,1,size(yytext)-1)))
		    handle Overflow => (err (yypos,yypos+size yytext)
					 COMPLAIN "integer too large"; 0),
			yypos,yypos+size yytext));
