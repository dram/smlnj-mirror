...
sig type 'a precStack
    ...
    val lookID : Symbol.symbol * ErrorMsg.complainer -> BareAbsyn.exp
    ...
    val pat_id:		symbol * ErrorMsg.complainer -> pat
    ...
    val makeEBof:	symbol * ty tsusp -> eb list susp
    val makeEBeq:	symbol * symbol list * ErrorMsg.complainer -> eb list susp
    ...
    type rawrvb
    val makeVALRECdec:	rawrvb list susp -> dec pathvstamped
    type rawclause
    ...
    val makeDB:		(symbol * int * tycon tpathstamped) list * (bool -> tb list pathvstamped) ->
				dec pathvstamped
    val makeDB':	tyvar list * symbol * (ty * tyclooker -> (symbol * bool * ty) list) uvars
				* ErrorMsg.complainer  -> tycon tpathstamped
    val makeTYPEdec:    tb list -> dec
    val makeABSTYPEdec: (symbol * int * tycon tpathstamped) list 
    ...
    val toplevelexp:	exp vstamped uvars * (BareAbsyn.linenum*BareAbsyn.linenum->ErrorMsg.complainer) * (BareAbsyn.linenum*BareAbsyn.linenum) -> dec
    val tyArg:	tyclooker
end
...
fun lookID(id,err) = varcon (lookVARCON id)
     handle Unbound => (err COMPLAIN ("unbound variable " ^ Symbol.name id);
		      VARexp(ref(mkVALvar(id, ref(VARty(mkTyvar defaultMETA))))))
...
fun pat_id(id,err) = CONpat(lookCON id) 
     ...
     val rec f =
           fn VARpat(v as VALvar{name=[name],...})=> 
		    if exists(fn n => n=name) (!vl)
		      then err COMPLAIN 
			("duplicate variable-name `"^Symbol.name name ^
				"' in pattern(s)")
		      else (if Symbol.eq(name,EQUALsym)
			    then err WARN "rebinding =" else ();
			    bindVAR(name,v); vl := name :: !vl)
 ...
 in fun sortRecord(l,err) =
	let val l' =  uniq (sort l)
	 in if length l' < length l
		then err COMPLAIN "duplicate label in record" else ();
	    l'
	end
end
...
fun makeHANDLEexp(exp,rules) =
     let fun anywild (RULE(WILDpat,_)) = true
	   ...
	   | anywild _ = false
		...
		else let val v = mkLvar() and t = ref UNDEFty
			 val r = RULE(VARpat(VALvar{name=[exnID], 
						  access=LVAR v, typ=t}),
				      RAISEexp(VARexp(ref(VALvar{name=[exnID], 
						    access=PATH[v], typ=t}))))
...
fun makeVALRECdec rvb (path,tv,st) =
    let val rvbs = rvb()
...
fun makecl(pats,[(a,INfix _,e), pat]) =
...
fun makeTB(args,name,(ty,tv),err) notwith (path,_,stamps) =
 let fun equalargs([],[]) = true
	 ...
	 | _ => ref(mkDEFtyc(name::path,
			     ...
			     stamps))
  in if notwith
       then defineEqTycon (fn x => x) (!binding)
       else (bindTYC(name,binding); ());
     [TB{tyc=binding,def=typ}]
 end
       
fun makeTYPEdec tbs =
  (app (fn TB{tyc as ref(TYCON{path=name::_,...}),...}=> bindTYC(name,tyc))
		 tbs;
   TYPEdec tbs)

fun makeDB(db,tb) (path,tv,stamps) =
  let fun predefine (id,arity,db') = 
	  let val tr = ref (mkUNDEFtyc(id,arity))
	   in bindTYC(id,tr);
	      (tr,db')
	  end
      ...
      fun redefine (tr,f) = 
		let val tyc = f(tyArg,path,stamps)
		 in tr := tyc; tr
		end
...
fun makeDB'(args,name,(constrs,cv),err) (tyclooker,path,stamps) =
  let val arity = length args
   ...
   in mkDATAtyc(name::path,arity,binddcons(sdcl,sign),MAYBE,stamps)
  end
