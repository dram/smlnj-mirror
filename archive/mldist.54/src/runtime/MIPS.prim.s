...
Closure(array_v)
1:	lw $artemp1,0($stdarg)		/* tagged length in $artemp1 */
	...
	addu $artemp3,$dataptr,$artemp1
	addu $artemp3,-4096
	addu $artemp3,0x80000000
	addu $artemp3,$limit
	bltz $artemp3,2f		/* continue if no fault */
.set noreorder	/* can't reorder because collection might occur */
	li $limit,0x7fffffff		/* MAXINT into limit */
	add $0,$dataptr,$limit		/* will cause gc */
	b 1b
...
2:
	...
	move $stdarg,$dataptr		/* put ptr in return register
	...
	j $10				/* return */
Closure(create_b_v)
	addi $11,$0,tag_bytearray	/* tag into $11 */
	b	2f
Closure(create_s_v)
	addi $11,$0,tag_string	/* tag into $11 */
2:	
1:	addi	$artemp1,$stdarg,13	/* $2n+14$ */
...
.set at
	...
	li	$artemp1,0x7fffffff
	lw	$limit,saved_limit
	sub	$limit,$artemp1,$limit
...
#ifdef MIPSEB
	add $2,1000		/* offset by 1000 on k2! */
#endif
	syscall
...
.set reorder
	...
	lw 	$artemp2,BIGPART($ptrtemp)	/* most significant part */
	...
	lw	$artemp2,BIGPART($ptrtemp)	/* most significant word */
	xor	$artemp2,$artemp3	/* change to new exponent */
	sw	$artemp2,BIGPART+4($dataptr)	/* save */
	lw 	$artemp2,LITTLEPART($ptrtemp)	/* get least significant word */
	li	$10,mak_desc(8,tag_string) /* make descriptor */
	sw	$artemp2,LITTLEPART+4($dataptr)	/* save lsw */
...
raise_real:
  ...
  */
	...
	add	$dataptr,12		/* $dataptr restored */
