...
#ifdef HPUX
#include <time.h>		/* for fionread fix */
#endif
...
#ifndef NS32
int fion(fd) int fd;
{static struct stat buf; int pos;
#ifdef HPUX
 int readfds, nfound;
 ...
 if (isatty(fd)) {		/* FIONREAD unsupported on tty's on 6.5 */
   ...
   if ((nfound = select(fd+1,&readfds,0,0,&timeout)) == -1)
     return(-1);
   else
     return((nfound << 1) + 1);
 }; /* if */
...
{int *begin = s;
 int *end = s + (s[-1]>>6) + 1;
	 /* flush i-cache from begin to end, if necessary */
#ifdef MIPS
 FlushInstructionCache(begin, ((int) end) - ((int) begin));
#endif
 return 1;
}
...
{function(fion,"fion",0)
 ...
 function(cach,"cach",14)
 function(setg,"setg",15)
 function(0,"xxxx",16)
};
...
int *cstruct[]={ 
   ...
   collectedfrom0+1,
   current0+1,
/* warning: this must be position 13 in cstruct[], else change
      construct_datalist() in run.c */
   (int*)1,
   errstrings+1,
   &(externlist0[0].func),
   gcmessages0+1,
   ...
   majorcollections0+1,
   minorcollections0+1,
#if defined(V9) || defined(HPUX) || defined(MIPSEB)
   (int*)7,
#else
#ifdef VAX
   (int*)3,
...
int exnCode(signl,code) int signl,code;
{
#ifdef HPUX
 if(signl==SIGFPE)
  {
    if(code==5)
      return (int)(div_e0+1);	/* test here before masking out bits */
...
#endif
#ifdef EXC_OV 
   case EXC_OV: /* Mips hardware integer overflow */
#endif
...
#ifdef HPUX
 else if (signl==SIGILL)
   if (code==7)
     return (int)(overflow_e0+1);
   else
     printf("exnCode: code was %d\n",code);
#endif HPUX
 else if (signl==SIGEMT)
	    return (int)(float_emt+1);
 else if (signl==SIGINT)
	    return (int)(interrupt_e0+1);
#ifdef MIPS
 else if (signl==SIGTRAP) {
    switch(code) {
      case BRK_OVERFLOW:
	    return (int)(float_ovf+1);  /* may not be right */
      case BRK_DIVZERO:
	    return (int)(div_e0+1);
      default:
	    return (int)(float_str+1);
