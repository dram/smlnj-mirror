...
#ifdef MIPSDEBUG
static char *causenames[] = { 
    "0", "CAUSE_GC", "CAUSE_RET", "CAUSE_EXN",
    "CAUSE_EXPORT", "CAUSE_BLAST", "CAUSE_FAULT", "CAUSE_STOR"
  };
#endif MIPSDEBUG
...
{int i;
 for(;;)
      {cause=0;
#ifdef MIPSDEBUG
chatting("[Entering ML with saved_dataptr == 0x%08x]\n",saved_dataptr);
#endif MIPSDEBUG
#ifdef MIPSDEBUG
/* i-cache is flushed less conservatively elsewhere */
if (cacheflush((char *) arenabase, arenasize, ICACHE)) {
    chatting("\n[");
    switch(errno) {
      case EFAULT: chatting("inaccessible address (EFAULT)"); break;
      case EINVAL: chatting("invalid args (EINVAL)"); break;
      default: chatting ("unknown error code %d", errno);
    }
    chatting("when flushing I-cache]\n");
} else chatting("[Flushed I-cache successfully]\n");
#endif MIPSDEBUG
       restoreregs();
#ifdef MIPSDEBUG
chatting("[Left ML (cause==%s) with saved_dataptr == 0x%08x]\n",
	 causenames[cause],saved_dataptr);
#endif MIPSDEBUG
...
{int p = arenabase+new_size;
 ...
 if (p<lastbreak)
    { 
#ifdef MIPSDEBUG
chatting("\n[Calling brk with argument 0x%08x]\n",p);
#endif MIPSDEBUG
     brk(p);
...
#ifdef NOCACHE
chatting("\n[Made heap uncacheable -- ");
/* try killing cacheing to see if that chases the weird bug */
if (cachectl((char *) arenabase, arenasize, UNCACHEABLE)) {
    switch(errno) {
      case EFAULT: chatting("inaccessible address (EFAULT)"); break;
      case EINVAL: chatting("invalid args (EINVAL)"); break;
      default: chatting ("unknown error code %d", errno);
...
#endif
#ifdef MIPS
  FlushInstructionCache(old_high, new_high-old_high);
...
{int need_major = 0; int was_preserving; int gamma = (ratio<7?7:ratio)>>1;
 ...
 if (need_major)
	...
	moveback(old_high,new_new_high,
	      ...
	      misc_roots);
#ifdef MIPS
	FlushInstructionCache(arenabase, new_new_high-old_high);
