  ...
  in fn lookup => 
	...
	and printStr(str as STRstr{table,env,kind,...}) =
		...
		fun visibleDcons(tyc,dcons) =
		    let fun dconTycon(CONty(ref tyc, [])) = tyc
			  | dconTycon(CONty(_,[_,CONty(ref tyc,_)])) = tyc
			  | dconTycon(POLYty{tyfun=TYFUN{body,...},...}) = dconTycon body
			  | dconTycon _ = ErrorMsg.impossible "dconTycon"
			fun find(DATACON{name,...}::rest) =
			    (let val dcon as DATACON{typ,...} =
		...
		fun printBinder(VARbind(VALvar{name=[n],typ,...})) = 
		  ...
		  | printBinder(TYCbind(ref(tyc))) = 
		      let val tyc as TYCON{path=name::_,kind,arity,eq,...} =
				   ...
				   of [] =>
					...
					printvseq (6) "con "
					  (fn DATACON{name,typ,...} => 
				        ...
				        then (nlindent(6); print "... hidden cons")
				        else ()))
			     | _ =>
				 (if (!eq=YES)
				  then print "eqtype "
