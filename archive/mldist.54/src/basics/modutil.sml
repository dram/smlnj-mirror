...
fun shiftStamps(transStrStamp,transTycStamp,newStamps,fixstamp,fixsign,fixstr,importing) =
    let fun newTyc(tyc as TYCON{stamp,arity,eq=ref(eq'),path,kind}) =
	    if tycFixed(stamp)
	    then tyc
	    else (case kind
		   of ABStyc t => 
		    ...
		    | DATAtyc _ =>
		       ...
		       TYCON{stamp=transTycStamp(stamp),
			     ...
			     kind=DEFtyc(TYFUN{arity=arity,
			     		       body=newType(body)})}
                    | RECORDtyc(labels) =>  (* only comes up in call within newType *)
			  if importing  (* have to recalculate stamp in local context *)
			  then if Tuples.isTUPLEtyc(tyc)
			       then Tuples.mkTUPLEtyc(length labels)
			       else Tuples.mkRECORDtyc labels
			  else tyc
	            | _ => tyc)
	  | newTyc tyc = tyc
	and newType(ty) =
	...
	and newStr(str as STRstr{stamp,sign,table,env,kind}) =
	    ...
	    then fixstr
		     ...
		     val newtable = if importing
				    then IntStrMap.transform (newBinding) table
				    else table
...
fun shiftSigStamps(base : stampsets, sgn as STRstr{kind=SIGkind{stamps,...},...},
	...
	and transTycStamp = join(tbase,toffset)
     in shiftStamps(transStrStamp,transTycStamp,base,~1,~1,NULLstr,importing) sgn
    end
