 ...
 *  val closeCPS : (cexp * constant? * profiling hook) ->                   *
 *                   (converted cexp * knownfunc? * escapingfunc?)          *
 *                                                                          *
 ****************************************************************************)
...
let
val (freevars,escapes,escapelabs,knownlabs) = FreeMap.freemapClose(ce,constant)
val clookup = fn (e,v) => if constant v then (Value,Direct) else lookup(e,v)
(***>
...
case !CGoptions.closureStrategy
  ...
  let val mapping = map (fn v => let val (obj,_) = clookup(env,v)
				 in  (v,obj)
				 end) free
  ...
  let val rec follow =
	fn (_,Value,_,_,_) => error "fixAccess Value in cps/closure"
	 ...
	 | (v,cl,env,SELp(i,p),h) =>
      ...
      val (obj,acc) = clookup(env,rootvar)
  in  case acc
  ...
  case ce
	...
	in  header(SELECT(i,v,w,close(c,augment((w,select(i,obj)),env))))
		...
		handle Lookup(i,e) => (print "LOOKUP FAILS in close(SELECT";
				       ilist [v,w]; print ") on "; vp i;
				       print "\nin environment:\n";
				       printEnv e;
				       SELECT(0,0,0,APP(0,nil))))
