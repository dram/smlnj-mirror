	 ...
	 ((case get f of
	     ...
	     let fun loop(TOP::r,v::vl,n) = TOP::loop(r,vl,n+1)
		   | loop(BOT::r,v::vl,n) =
			 ...
			 (flr := (f,n)::fl; 
			  (if !CG.flattenargs then COUNT(length wl) else TOP)
				::loop(r,vl,n+1))
		       | _ => raise Escapemap)
...
fun expand(cexp,bodysize) =
  ...
  fun cycle(0,cexp) = contract true cexp
	...
	let val _ = debugprint "\nHoist: "
	    val cexp = hoist cexp
	    val _ = clicked := 0
	    val cexp = if !CG.betaexpand then expand(cexp,bodysize div rounds)
			else cexp
	...
	let val _ = debugprint "\nHoist: "
	    val cexp = hoist cexp
	    val _ = clicked := 0
	    val cexp = if !CG.betaexpand then expand(cexp,(bodysize * k) div rounds)
			else cexp
	    val cl = !clicked before clicked := 0
        in if cl <= reducemore
	   then contract true cexp
