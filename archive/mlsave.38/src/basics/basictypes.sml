...
infix -->
val arrowTycon = ref(mkABStyc([arrowSym],2,NO,Stamp.global))
...
val intTycon = ref(mkABStyc([intSym],0,YES,Stamp.global))
val intTy = CONty(intTycon,nil)

val realTycon  = ref(mkABStyc([realSym],0,YES,Stamp.global))
val realTy = CONty(realTycon,nil)

val stringTycon  = ref(mkABStyc([stringSym],0,YES,Stamp.global))
val stringTy = CONty(stringTycon,nil)

val exnTycon = ref(mkABStyc([exceptionSym],0,NO,Stamp.global))
val exnTy = CONty(exnTycon,nil)

val contTycon = ref(mkABStyc([contSym],1,NO,Stamp.global))

val arrayTycon = ref(mkABStyc([arraySym],1,YES,Stamp.global))
...
val trueDcon =
    DATACON
      {name = trueSym,
       ...
       typ = boolTy,
       sign = boolsign}
val _ = boolTycon := mkDATAtyc([boolSym],0,[falseDcon,trueDcon],YES,Stamp.global)
...
val refDcon = 
    DATACON
      {name = refSym,
       ...
       sign = [REF]}
val _ = refTycon := mkDATAtyc([refSym],1,[refDcon],YES,Stamp.global)
...
val nilDcon = 
    DATACON
      {name = nilSym,
       ...
       sign = listsign}
val _ = listTycon := mkDATAtyc([listSym],1,[consDcon,nilDcon],YES,Stamp.global)
