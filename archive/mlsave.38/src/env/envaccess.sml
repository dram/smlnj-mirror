    ...
    (case look(tycKey id)
      of (TYCbind(tycref as ref(INDtyc[i])), {strenv={t,s},path=h::r}) =>
	   ...
	   then let val tyc = t sub i
		 ...
		 if depth+h = 0
		 then tycref
		 else ref(INDtyc(addzeros(depth+h,r@[i]))))
	...
	val leadStr = lookSTR0 first
     ...
     in case leadStr
	  of (STRvar{binding=INDstr i,...},{path=h::r,strenv={s,...}}) =>
	      ...
	      then ref(INDtyc(addzeros(depth+h,r@(i::get(rest, s sub i)))))
...
(* abstraction of functor body structure; relativizing bound components *)

fun abstractBody(STRstr{table,env as {s,...},...},tycTotal: int) : unit =
    let val stampmap = array(tycTotal, []:int list)
	fun mkStampMap() =
	    let val FCTstr(STRstr{env=penv,...}) = s sub 0
                fun esearch({s,t},path) =
		    let fun tsearch i =
			  ...
			  (case tyc
			     of INDtyc _ => () (* already been done *)
				  ...
				  if Stamp.fixed stamp then ()
				         ...
				         INDtyc(relative(path,stampmap sub stamp));
			   app absty args)
