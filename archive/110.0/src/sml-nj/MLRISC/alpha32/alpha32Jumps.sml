 ...
 *
 *)
functor Alpha32Jumps( Alpha32Instr : ALPHA32INSTR) : SDI_JUMPS = 
struct
  ...
  fun sdiSize(I.DEFFREG _, _, _, _) = 0
    ...
    | sdiSize(I.COPY(_, _, ref(SOME l)), _, _, _) = 4 * length l
    | sdiSize(I.FCOPY(_, _, ref(SOME l)), _, _, _) = 4 * length l
    | sdiSize(I.COPY(rds, rss, exp as ref NONE), regmap, _, _) = let
        fun mvInstr(rd, rs) = 
    ...
    | sdiSize(I.FCOPY(fds, fss, exp as ref NONE), regmap, _, _) = let
        fun mvInstr(fd, fs) = I.FOPERATE{oper=I.CPYS, fa=fs, fb=fs, fc=fd}
	val lookup = Intmap.map regmap
        val instrs = Shuffle.shuffle(mvInstr, lookup, C.fasmTmp, fds, fss)
      in
	exp := SOME instrs; 
	4 * length instrs
      end
  ...
  *)
  ...
  in
    ...
    | I.COPY(_,_,ref(SOME instrs)) => instrs
    | I.FCOPY(_,_,ref(SOME instrs)) => instrs
