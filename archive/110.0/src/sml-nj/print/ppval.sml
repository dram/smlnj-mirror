...
fun ppDcon ppstrm =
    let fun ppD(DATACON{name, rep=A.EXNFUN acc, ...}) =
	       (ppSym ppstrm (name); 
		if !internals then ppAccess ppstrm acc else ())
	  | ppD(DATACON{name, rep=A.EXNCONST acc, ...}) =
	       (ppSym ppstrm (name); 
		if !internals then ppAccess ppstrm acc else ())
...
fun ppConBinding ppstrm =
    let val {begin_block,end_block,pps,...} = en_pp ppstrm
	fun ppCon (DATACON{name, typ, rep=A.EXNFUN _, ...}, env) =
		(begin_block CONSISTENT 0;
		 pps "exception "; ppSym ppstrm name; pps " of "; 
		 ppType env ppstrm (BasicTypes.domain typ);
		 end_block())
	  | ppCon (DATACON{name, typ, rep=A.EXNCONST _, ...}, env) =
		(begin_block CONSISTENT 0;
