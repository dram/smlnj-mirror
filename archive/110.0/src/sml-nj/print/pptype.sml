...
sig
  ...
  val ppType  : StaticEnv.staticEnv -> PrettyPrint.ppstream 
                -> Types.ty -> unit
  val ppDconDomain : Types.dtmember vector -> StaticEnv.staticEnv 
		...
		{front=C PP.add_string "{",
		 ...
		 pr=ppSym} labels
          ...
          | ppTyc (RECtyc n) =
              (case membersOp
                of SOME members => 
                    let val {tycname,stamp,dcons,...} = Vector.sub(members,n)
...
and ppType1 env ppstrm (ty: ty, sign: T.polysign, 
                        membersOp: T.dtmember vector option) : unit =
...
fun ppDconDomain (members: Types.dtmember vector) (env:StaticEnv.staticEnv)
                 ppstrm (ty:ty) : unit = 
      (PP.begin_block ppstrm PP.INCONSISTENT 1;
...
fun ppDataconTypes env ppstrm (GENtyc{kind=DATATYPE{index,members,...},...}) =
    let val {begin_block, end_block, pps, add_break,...} = en_pp ppstrm
	...
	app (fn {name,domain,...} =>
		...
		case domain
		  of SOME ty => ppType1 env ppstrm (ty,[],SOME members)
		   | NONE => pps "CONST";
		add_break(1,0)))
	    dcons;
	end_block()
