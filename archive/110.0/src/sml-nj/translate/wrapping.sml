...
val ident = fn le => le
val IntOpTy = LT.ltc_arw(LT.ltc_tup[LT.ltc_int,LT.ltc_int],LT.ltc_int)

(****************************************************************************
 *                   MISC UTILITY FUNCTIONS                                 *
 ****************************************************************************)
val ltApply = LE.ltApply (* lazy apply *)
val ltAppSt = LE.ltAppSt (* strict apply *)
...
val lt_upd = 
  let val x = LT.ltc_tyc(tc_array (LT.tcc_tv 0))
...
fun mkPrim(p as PO.SUBSCRIPT, t, [tc]) = 
  ...
  | mkPrim(PO.UPDATE, t, [tc]) = 
      ...
      then if tc_eq(tc,tc_real)
           ...
           else (let val np = LE.tcUpd tc
                  in case np
                      of PO.UPDATE => (np, PARRAYOP)
                       | _ => (np, STANDARD)
                 end)
      else (LE.tcUpd tc, STANDARD)
...
(****************************************************************************
 * val transform : CO.wpEnv * LE.ltyEnv * DI.depth                          *
 *                     -> lexp -> (lexp * LT.lty)                           *
 *                                                                          *
...
fun lpve sv = 
  (case sv
    of VAR v => (SVAL sv, LE.ltLookup(venv, v, d))
     | INT i => 
     ...
     | GENOP(dict as {default=pv, table=[(_,sv)]}, 
         ...
         if tc_eq(tc, tc_real) then 
               let val nt = ltAppSt(lt,ts)
                in (SVAL(VAR sv), nt)
               end
         else (case LE.tcsWrap ts
...
and loop le = 
  (case le
    of SVAL sv => lpve sv
     ...
     | CON (x as (name, rep as (DA.CONSTANT _ | DA.EXNCONST _), lt), ts, v) => 
         let val (nv, hdr0, _) = lpsv v
             val nt = case ts of [] => lt 
                               | _ => ltAppSt(lt, ts)
          in (hdr0(CON(x, ts, nv)), nt)
         end
     | CON (x as (name, rep, lt), ts, v) => 
         let val (nv, hdr0, _) = lpsv v
             val ot = ltAppSt(lt, ts)
             val (argt, res) = ltArrow(ot)
          in (case LE.ltWrap argt
     ...
     | DECON ((_, DA.CONSTANT _, _), _, _) => (SVAL(INT 0), LT.ltc_unit)
         (* reason: unit-carrying data constructors are considered 
                    as constants; *)
     | DECON ((_, DA.EXNCONST _, _), _, _) => 
         bug "DECON on an exnconst data constructor"
     | DECON (x as (name, rep, lt), ts, sv) => 
         let val (nv, hdr0, _) = lpsv sv
             val ot = ltAppSt(lt, ts)
             val (res, argt) = ltArrow(ot)
          in (case LE.ltWrap res
     ...
     | FN(v, t, e) => 
         let val nvenv = LE.ltInsert(venv, v, t, d)
             val (ne, nt) = transform (wenv, nvenv, d) e
          in (FN(v, t, ne), LE.ltFun(t, nt))
         end
     | FIX(vs, ts, es, eb) => 
         let val nvenv = 
     ...
     | SRECORD [] => (SVAL(INT 0), LT.ltc_unit)
     | SRECORD vs => 
         let fun h([], hdr, nvs, nts) = 
     ...
     | VECTOR (vs, t) => 
         let val (wt, hdr, mhdr) = 
          ...
          in (h (vs, mhdr, []), LT.ltc_tyc(LT.tcc_app(LT.tcc_vector,[t])))
         end
     | SELECT (i, v) => 
         let val (nv, hdr, nt) = lpsv v
          in (hdr(SELECT(i, nv)), LE.ltSelect(nt, i))
         end
     | EXNF (v, t) => 
         let val (nv, hdr, _) = lpsv v
          in (hdr(EXNF(nv, t)), LT.ltc_iexn)
         end
     | EXNC v => 
         let val (nv, hdr, _) = lpsv v
          in (hdr(EXNC nv), LT.ltc_exn)
         end
     | RAISE (v, t) => 
         let val (nv, hdr, _) = lpsv v
          in (hdr(RAISE(nv, t)), t)
...
fun wrapLexp (FN(v, t, e)) = 
      let val wenv = CO.initWpEnv ()
          val venv = LE.initLtyEnv
          val d = DI.top
          val nvenv = LE.ltInsert(venv, v, t, d)
 ...
 *
 * Revision 1.5  1998/01/30 14:29:17  jhr
 *   Fix for bug 1340 from Zhong.
 *
