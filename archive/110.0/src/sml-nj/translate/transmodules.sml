...
sig 
  ...
  val strLty  : Modules.Structure * DebIndex.depth 
                    * ElabUtil.compInfo -> LambdaType.lty
              ...
              let val _ = debugmsg ">>specLtyElt"
               ...
               in case spec
                   of VALspec{spec=typ,...} => 
                    ...
                    | CONspec{spec=TP.DATACON{rep=DA.EXNCONST _, ...}, ...} => 
                        g(rest, entEnv, (LT.ltc_exn)::ltys)
                    | CONspec{spec=TP.DATACON{rep=DA.EXNFUN _, ...}, ...} => 
                        g(rest, entEnv, (LT.ltc_iexn)::ltys)
   ...
   other procedures. Thus we turn it off temporarily. (ZHONG)
        ...
        | g (sign as SIG{kind=SOME _, ...}, rlzn as {lambdaty, ...}) = 
             let val sgt = signLty(sign, depth, compInfo)
                 ...
                 val lt = LE.ltApply(sgt, map (TT.tpsTyc depth) argtycs)
              in lambdaty := SOME(lt, depth); lt
             end
*)
 ...
 *
 * Revision 1.4  1997/09/23 04:05:35  dbm
 *   Fix for representation problems (Zhong).
 *
