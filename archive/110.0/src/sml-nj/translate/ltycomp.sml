...
fun bug s = ErrorMsg.impossible ("LtyComp: " ^ s)
  ...
  (case le
    of SVAL sv => SVAL(lpsv sv)
     ...
     | CON ((_, DA.UNTAGGED, lt), ts, v) => 
         let val nt = LE.ltAppSt(lt, map (fn _ => LT.tcc_void) ts)
             val (tc, _) = LT.tcd_arw(LT.ltd_tyc nt)
             val ntc = case LE.tcWrap tc of NONE => tc
                                          | SOME z => z
     ...
     | CON ((_, DA.EXNCONST p, _), _, _) => exnLexp p
     | DECON ((_, (DA.CONSTANT _ | DA.EXNCONST _), _), _, _) => 
         bug "DECON on a constant or exnconst data constructor"

     | CON ((_, DA.EXNFUN p, nt), [], v) => 
         let val (ax, _) = LT.tcd_arw(LT.ltd_tyc nt)
             (***WARNING: the type of ax is adjusted to reflect boxing *)
             val nax = case LE.tcWrap ax of NONE => ax
                                          | SOME z => z
             val nx = LT.tcc_tup [LT.tcc_iexn, nax, LT.tcc_int]
             
          in WRAPg(nx, true, RECORDg [exnLexp p, SVAL(lpsv v), SVAL(INT 0)])
         end
     | DECON ((_, DA.EXNFUN _, nt), [], v) => 
         let val (ax, _) = LT.tcd_arw(LT.ltd_tyc nt)
             (***WARNING: the type of ax is adjusted to reflect boxing *)
             val nax = case LE.tcWrap ax of NONE => ax
                                          | SOME z => z
     ...
     | EXNF (v, t) => EXNF(lpsv v, t)         (* t is always monomorphic *)
     | EXNC v => EXNC(lpsv v)
