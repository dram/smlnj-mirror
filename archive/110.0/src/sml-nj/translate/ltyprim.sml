...
end (* signature LTYPRIM *)

structure LtyPrim : LTYPRIM = 
struct
...
val (trueDcon', falseDcon') = 
  let fun h (TP.DATACON{name,rep,typ,...}) = (name, rep, LT.ltc_bool)
...
val inteqty = LT.ltc_arw(LT.ltc_tup [intty, intty], boolty)
val intopty = LT.ltc_arw(LT.ltc_tup [intty, intty], intty)
...
fun tcLexp (kenv, tc) = 
  let fun loop x = 
	(case (tc_out x)
	  of (TC_FN(ks, tx)) => 
		let val (nenv, hdr) = tkLexpG(kenv, ks, LT.ltc_tup)
...
fun utgc (kenv, tc) = 
  (case tcTag(kenv, tc)
    of YES => (fn le => WRAPg(LT.tcc_tup [tc], true, RECORDg[le]))
     | NO => (fn le => le)
     | MAYBE ne => 
	let fun h(x as (SVAL(VAR v))) =  
		  COND(ne, WRAPg(LT.tcc_tup [tc], true, RECORDg [x]),
			   x)
	      | h x = 
		  let val w = mkv() 
		   in LET(w, x, 
		       COND(ne, WRAPg(LT.tcc_tup [tc], true, RECORD [VAR w]),
				SVAL(VAR w)))
		  end
	 in h
	end)
...
fun utgd (kenv, tc) = 
  (case tcTag(kenv, tc)
    of YES => 
	(fn le => SELECTg(0, UNWRAPg(LT.tcc_tup [tc], true, le)))
     | NO => (fn le => le)
     | MAYBE ne => 
	let fun h(x as (SVAL(VAR v))) =  
		  COND(ne, SELECTg(0, UNWRAPg(LT.tcc_tup [tc], true, x)), x)
	      | h x = 
		  let val w = mkv()
		   in LET(w, x, 
		       COND(ne, SELECTg(0, UNWRAP(LT.tcc_tup [tc],true,VAR w)),
				SVAL(VAR w)))
		  end
	 in h
	end)
...
fun tgdc (kenv, i, tc) = 
  let val nt = LT.tcc_tup [LT.tcc_int, tc]
	 ...
	 let fun h([], i, e, el, 0) = NONE
		   ...
		   let val w = mkv()                 
			 ...
			 | g(i, SOME _) = 
		       ...
		       val ntc = LT.tcc_tup(map (fn _ => LT.tcc_real) ts)
           ...
           of (YES | NO) => NONE
            | (MAYBE e) =>
                 let val w = mkv()
                     ...
                     val tc_pair = LT.tcc_tup [tc_void, tc_void]
                     val tc_fpair = LT.tcc_tup [tc_real, tc_real]
                     val tc_bfpair = LT.tcc_tup [tc_breal, tc_breal]
                     ...
                     val (argt1, body1, hh1, ih1) = 
                       if wflag then (* wrapping *)
                         (lt_pair, WRAP(tc_pair, true, VAR m),
                          fn le => WRAPcast(LT.tcc_arw(tc_pair,tc2), true, le),
                          ident)
                       else (* unwrapping *)
                         let val q = mkv()
                          in (lt_void, UNWRAP(tc_pair, true, VAR m),ident,
                              fn le => UNWRAPcast(LT.tcc_arw(tc_pair, tc2),
                                               true, le))
                         end
                     ...
                     val (argt2, body2, hh2, ih2) = 
                       if wflag then
                         (lt_bfpair, WRAPg(tc_fpair, true, 
                          ...
                          fn le => WRAPcast(LT.tcc_arw(tc_bfpair,tc2), true, le),
                          ident)
                       else
                         let val q = mkv()
                          in (lt_void, LET(q, UNWRAP(tc_fpair, true, VAR n),
                            ...
                            ident,
                            fn le => UNWRAPcast(LT.tcc_arw(tc_bfpair, tc2),
                                             true, le))
                         end
...
fun arrSub(kenv, lt, tc) = 
  let val nt = LE.ltAppSt(lt, [tc])
      val rnt = LE.ltAppSt(lt, [LT.tcc_real])
 ...
 *
 * Revision 1.5.2.1  1998/04/17 20:07:13  george
 *   bug fix for bug 1365 -- zsh
 *
