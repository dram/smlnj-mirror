      ...
      (case getSpec(elements, sym)
        of VALspec{spec,slot} =>
         ...
         | CONspec{spec=T.DATACON{name, const, typ, rep, sign}, slot} =>
             let val newrep =
                   case (rep, slot)
                    of (A.EXNCONST _, SOME i) => A.EXNCONST(A.selAcc(dacc,i))
                     | (A.EXNFUN _, SOME i) => A.EXNFUN(A.selAcc(dacc,i))
                     | _ => rep
...
fun mapDtMbers (mkStamp, f) (xs : T.dtmember vector) = 
    let fun doDcon {name, rep, domain=SOME ty} = 
	      {name=name, rep=rep, domain=SOME (f ty)}
	  | doDcon x = x
	fun doTyc {tycname, stamp, dcons, arity, eq, sign, lambdatyc} = 
	    {tycname=tycname, stamp=mkStamp(), dcons=map doDcon dcons,
	     arity=arity, eq=eq (*?*), sign=sign, lambdatyc=ref NONE}
     in Vector.map doTyc xs
    end
