 ...
 **)
  ...
  (* decompose a positive real into a list of at most maxPrec significant digits
   ...
   * ASSERT: f > 0.0
   *)
    fun decompose (f, precisionFn) =
	let fun scaleUp (x, e) =
		  if (x < 1.0) then scaleUp(10.0*x, dec e) else (x, e)
	    fun scaleDn (x, e) =
		  if (x >= 10.0) then scaleDn(0.1*x, inc e) else (x, e)
            (* mkdigits(f: real, n: int, p: int) : int list * int
	     * ASSERT 1.0 <= f < 10.0; n >= 0; p in {0,1} *)
	    fun mkdigits (f, 0, odd) = 
		  ...
		  let val d = floor f
		      val (digits, carry) =
			  mkdigits (10.0 * (f - real d), dec i, I.rem(d,2))
		      val (digit, c) =
			  (case (d, carry)
			     of (9, 1) => (0, 1)
			      | _ => (I.+(d, carry), 0)
	    ...
	    val (f, e) = if (f < 1.0) then scaleUp (f, 0)
			 else if (f >= 10.0) then scaleDn (f, 0)
			 else (f, 0)
            (* ASSERT: 1.0 <= f < 10.0 *)
    ...
    fun realFFormat (r, prec) = let
	  ...
	  in
	    ...
	    if (r < 0.0)
	      then {sign = "~", mantissa = rtoa(decompose(~r, pf))}
	    else if (r > 0.0)
	      then {sign="", mantissa = rtoa(decompose(r, pf))}
	    else if (prec = 0)
 ...
 *
 * Revision 1.2.2.1  1999/09/07 17:51:48  dbm
 * fix for basis/real regression failures
 *
 * Revision 1.2  1997/11/20 19:07:45  dbm
 *   Fixed definition of floor.
 *
