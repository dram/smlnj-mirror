...
fun debugPPTyvar tv = ED.withInternals(fn () => debugmsg (PPType.tyvarPrintname tv))
...
fun unifyErr{ty1,name1,ty2,name2,message=m,region,kind,kindname,phrase} =
    (unifyTy(ty1,ty2); true) handle Unify(mode) =>
	 ...
	 (PPType.resetPPType();
	  ...
	  in if name1="" then ()
	      else (add_newline ppstrm; add_string ppstrm (name1 ^ ": " ^ pad1);
		    ppType ppstrm ty1); 
	     if name2="" then ()
	      else (add_newline ppstrm; add_string ppstrm (name2 ^ ": " ^ pad2);
		    ppType ppstrm ty2);
	     if kindname="" then ()
...
val _ = debugPPDec(">>decType: dec = ",dec)
...
fun generalizeTy(VALvar{typ,path,...}, userbound: tyvar list,
	...
	val _ = List.app debugPPTyvar userbound
			  ...
			  then if generalize then
				  lookup tv handle NotThere =>
					...
					new
				     ...
				     else (if !Control.valueRestrictionLocalWarn
	...
	val _ = debugPPType (">>gen: before: ",!typ)
	val ty = gen(!typ)
	val _ = debugPPType (">>gen: after: ",ty)
	...
	fun elimUbound(tv as ref(UBOUND{depth,eq,...})) = 
     ...
     in if !failure andalso !Control.valueRestrictionTopWarn
	  then err region WARN
