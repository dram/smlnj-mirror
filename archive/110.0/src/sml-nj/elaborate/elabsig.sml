	    ...
	    case spec
		   ...
		   * an extremely weak notion of compatibility -- same arity. *)
		   ...
		   then TYCspec{entVar=entVar, spec=tyc,
			 ...
			 tspec)
(*
		 (case TU.unWrapDefStar tyc
		    of tyc' as T.GENtyc{kind=T.DATATYPE _,arity=arity',...} =>
			 if arity = arity'
			 then TYCspec{entVar=entVar, spec=tyc',
				      scope=SP.length spath}
			 else (error ("where type definition has wrong arity: " ^
			       ...
			       tspec)
		     | _ =>
		       (error ("nondatatype where defn applied to datatype spec: " ^
			      SP.toString spath);
		        tspec))
*)
	       | T.DEFtyc _ =>
	...
	fun applyStrDefs(spec as STRspec{entVar,sign,def,slot},defs) =
		...
		| NONE =>
		   (case defs
		      of (nil,STRdef(spath,strDef))::rest =>  (* applies directly *)
			  (case rest
			     of nil =>
		  ...
		  | _ => (* left-over defs *)
		    (app (fn (_,TYCdef(p,_,_)) =>
			   ...
			   | (_,STRdef(p,_)) =>
		         ...
		         defs;
		     rev elems))
	  | loop(elems0,nil,elems) = rev elems @ elems0 (* all defs processed *)
...
and addWhereDefs(sign,nil,nameOp,error,mkStamp) = bug "addWhereDefs"
  | addWhereDefs(sign as SIG{name,closed,fctflag,stamp,symbols,elements,boundeps,lambdaty,
	...
	closed=closed andalso closedDefs whereDefs,
        fctflag=fctflag,
	stamp=mkStamp(), (* give modified sig a new stamp -- could stack stamps *)
	symbols=symbols,
    ...
    let fun loop(AugSig(sigexp,whspecs),defs,region) =
	    let fun loop1(nil,defs) = loop(sigexp,defs,region)
		  | loop1(WhType(path,tyvars,ty)::rest,defs) =
			  ...
			  val _ = debugmsg("elabWhere:WhType: "^SP.toString spath)
			  ...
			  val tycon = T.DEFtyc{stamp = mkStamp(),
		  ...
		  | loop1(WhStruct(lhs,rhs) ::rest,defs) =
		     (let val lhspath = SP.SPATH lhs
			  ...
			  val strDef = (* remove access & inline info (bug 1201) *)
			      case strDef
    ...
    let val tyc = Lookup.lookTyc(env, SP.SPATH syms, error region)
     in case tyc
		  ...
		  val tyc' = EX.expandTycon(tyc,sigContext,entEnv)
	       in case tyc'
                    of T.GENtyc{kind=T.DATATYPE{index,members,...},...} => (* ok *)
		        let val {tycname, stamp, arity, dcons, sign, ...} =
...
fun elabDATATYPEspec0(dtycspec, env, elements, symbols, region) = 
  let val _ = debugmsg ">>elabDATATYPEspec"
      ...
      (* push a local epContext environment to be used to relativize
         the datacon types and bodies of withtype defns within this declaration *)
      val epContext = EPC.enterClosed(epContext)

      val (dtycs,wtycs,dcons,_) =
            ET.elabDATATYPEdec(dtycspec, env, elements::sctxt,
			       entEnv, IP.IPATH[], region, compInfo)
      val _ = debugmsg "--elabDATATYPEspec: elabDATATYPEdec done"

      val vizty = (fn ty => #1(MU.relativizeType epContext ty))
      fun vizmb {tycname, stamp, dcons, arity, eq, sign, lambdatyc} = 
        let fun doDcon {name, rep, domain=SOME ty} = 
	          {name=name, rep=rep, domain=SOME (vizty ty)}
   	      | doDcon x = x
         in {tycname=tycname, stamp=stamp, dcons=map doDcon dcons,
	     arity=arity, eq=eq (*?*), sign=sign, lambdatyc=ref NONE}
        end

      (* the following is taken from bindNewTycs in elaborate/elabmod.sml *)
      (* this is ok because epContext has state; a bit ugly *)
      val ndtycs =
        let fun newdt (dt as T.GENtyc{kind=T.DATATYPE{index=i,members,...},
                                      arity, eq, path, stamp}) =
                  T.GENtyc{arity=arity, eq=eq, path=path, stamp=stamp,
                           kind=T.DATATYPE{index=i,
                                           members=Vector.map vizmb members, 
                                           lambdatyc=ref NONE}}
              | newdt _ = bug "unexpected case in newdtyc"
         in map newdt dtycs
        end

      val alltycs = ndtycs@wtycs

      fun addTycs([], env, elems, syms) = (env, elems, syms)
        | addTycs(tyc::tycs, env, elems, syms) =
            let (* val ev = mkStamp() *)
		(* HACK: use the stamp of the type as its entVar.  This makes
		 * it possible to reconstruct the entPath associated with
		 * a RECty when translating the types of domains in elabDATArepl. *)
                val (tyc',ev) =
                  case tyc
                   of T.DEFtyc{stamp,tyfun=T.TYFUN{arity,body},strict,path} =>
		      (* relativizeType will work because there are no
                         forward references allowed between the withtype
			 definitions, so types will have been entered into
			 epContext before they are used. *)
                        (T.DEFtyc{stamp=stamp,strict=strict,path=path,
				  tyfun=T.TYFUN{arity=arity, body=vizty body}},
			 stamp)  (* probably doesn't matter for DEFtyc case *)
                    | T.GENtyc{stamp,...} => (tyc,stamp)
                    | _ => bug "addTyc"

		val arity = TU.tyconArity tyc'
                val tspec = TYCspec{spec=tyc', entVar=ev, scope=0}
		...
		val _ = debugmsg ("--elabDATATYPEspec - name: "^ S.name name)
                ...
                (*
                 * the sole purpose of registering tyc in epContext is
                 * because dataconstructor domain types computed by
		 * elabDATATYPEdec are not relativized.
                 * if we write a version of elabDATATYPEdec in ET 
		 * specialized for datatype spec (elabDATATYPEspec?),
		 * this would no longer be necessary.
                 *)
		val tyc_id = MU.tycId tyc'
	        val _ = debugmsg ("addTycs - tyc id: " ^ 
                                  ModuleId.idToString tyc_id)
                val _ = EPC.bindPath(epContext, tyc_id, ev)
		val _ = case EPC.lookPath(epContext,tyc_id)
			  of NONE => debugmsg "addTycs - tyc_id not found"
			   | SOME entPath =>
			      debugmsg ("addTycs - tyc_id found: " ^
					EP.entPathToString entPath)

		val _ = debugPrint("addTycs - tyc: ",
		   (fn pps => fn tc => PPType.ppTycon env pps tc), tyc')
		val _ = debugPrint("addTycs - etyc: ",
		   (fn pps => fn tc => PPType.ppTycon env pps tc), etyc)
		val _ = debugmsg ("addTycs - ev: " ^ ST.stampToShortString ev);

             in addTycs(tycs, env', elems', name::syms)
            end

      val (env', elements', symbols') = 
             addTycs(alltycs, env, elements, symbols)
      val _ = debugmsg "--elabDATATYPEspec: tycs added"

      fun addDcons([], elems, syms) = (elems, syms)
        | addDcons((d as T.DATACON{name,rep,const,sign,typ})::ds, elems, syms) = 
        ...
        let val err = error region
            fun exnspecs(nil,elems,syms,slots) =
              ...
              | exnspecs((name,tyOp)::rest,elems,syms,slots) =
                  (case tyOp
                    of SOME ty =>
                         let val (body,tv) = ET.elabType(ty,env,error,region)
                             val typ =
                               case TyvarSet.elements tv
                             ...
                             val _ = TU.compressTy typ
                             val (typ,_) = MU.relativizeType epContext typ
                             val rep = A.EXNFUN(A.nullAcc)
                             val dcon = 
                                   T.DATACON{name=name, const=false,
                     ...
                     | NONE =>
                         let val typ = BT.exnTy
                             val rep = A.EXNCONST(A.nullAcc)
                             val dcon = 
                                   T.DATACON{name=name, const=true, typ=typ, 
                                             sign=A.CNIL, rep=rep}
                             val cspec = CONspec{spec=dcon, slot=SOME slots}
                             val elems' = add(name,cspec,elems,err)
                          in exnspecs(rest, elems', name::syms, slots+1)
