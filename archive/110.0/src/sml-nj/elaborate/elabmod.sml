...
fun bindNewTycs(EU.INFCT _, epctxt, mkStamp, dtycs, wtycs, rpath, err) = 
	  ...
	  val vizty = (fn ty => #1(MU.relativizeType epctxt ty))
          ...
          val ndtycs =
            (case dtycs
              of ((T.GENtyc{kind=T.DATATYPE{index=0,members,...}, ...})::_) =>
                   let val rootev = mkStamp()
                       val rtevOp = SOME rootev
                       val rootmbs = MU.mapDtMbers (mkStamp, vizty) members

                       fun newdt (dt as T.GENtyc{kind=T.DATATYPE{index=i,...},
                                                 arity, eq, path, ...}) =
                            let val ndt =
                                  T.GENtyc{arity=arity, eq=eq, path=stripPath path, 
                                           stamp=(#stamp(Vector.sub(rootmbs,i))),
				...
				val _ =
				    ED.withInternals(fn () =>
				     debugPrint
				      ("--newdt: ",
				       (fn pps => fn tyc =>
				         PPType.ppDataconTypes SE.empty pps tyc),
				       ndt))

                                val (ev, texp) = 
                                  if i=0 then (rootev, M.FMGENtyc(ndt, NONE))
                                  else (mkStamp(), M.FMGENtyc(ndt, rtevOp))
          ...
          | B.CONbind(dcon as T.DATACON{name,const,sign,typ,rep}) =>
              let val typ' = relativize typ
                  val (rep', trans', slotOp, slotCount') =
                    case rep
                     of DA.EXNFUN _ => 
                          (DA.EXNFUN(DA.nullAcc), binding::trans, 
                           SOME slotCount, slotCount+1)

                      | DA.EXNCONST _ => 
                          (DA.EXNCONST(DA.nullAcc), binding::trans, 
                              SOME slotCount, slotCount+1)
  ...
  of StrDec strbs =>
   ...
   | DatatypeDec (x as {datatycs,withtycs}) =>
	 ...
	 of (Db{rhs=(Constrs _), ...}) :: _ =>
	      let val (datatycs,withtycs,_,env) =
		    ET.elabDATATYPEdec(x, env0, [], EE.empty,rpath,region,compInfo)
		  val (entEnv, entDec) = 
		    bindNewTycs(context, epContext, mkStamp, datatycs, withtycs,
				rpath, error region)
		  val resDec = A.DATATYPEdec{datatycs=datatycs,withtycs=withtycs}
	       in (resDec, entDec, env, entEnv)
	      end
   ...
   | AbstypeDec x =>
       (let val (decl as A.ABSTYPEdec{abstycs,withtycs,...}, env') =
              EC.elabABSTYPEdec(x, env0, context, rpath, region, compInfo)
   ...
   | dec =>
       (let val (decl,env') = EC.elabDec(dec, env0, rpath, region, compInfo)
              handle EE.Unbound => (debugmsg("$EC.elabDec"); raise EE.Unbound)
val _ = debugmsg (">>elabDecl0.dec[after EC.elabDec: top="^(Bool.toString top))
