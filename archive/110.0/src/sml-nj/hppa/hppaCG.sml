...
struct
  ...
  structure BBSched =
    BBSched2(structure Flowgraph = HppaFlowGraph
	     structure Jumps = HppaJumps(HppaInstr)
  ...
  struct
   (* spill area management *)
    ...
    fun spillR {instr, reg} = let
      val loc = getRegLoc reg
      fun spillInstr(r) = 
         [I.STORE{st=I.STW, b=C.stackptrR, d=I.IMMED(~loc), r=r, mem=stack}]
    in
      case instr
      of I.COPY([rd], [rs], _) =>{code=spillInstr(rs), instr=NONE, proh=[]}
       | _ => let
	   val newR = C.newReg()
	   val instr' = HppaRewrite.rewriteDef(instr, reg, newR)
	 in {code=spillInstr newR, instr=SOME instr', proh=[newR]}
	 end
    end

    (* reload general register *)
    fun reloadR {instr, reg} = let
      val loc = getRegLoc(reg)
      fun reloadInstr(r) = 
          I.LOADI{li=I.LDW, i=I.IMMED(~loc), r=C.stackptrR, t=r, mem=stack}
    in
      case instr 
      of I.COPY([rd], [rs], _) => {code=[reloadInstr(rd)],  proh=[]}
       | _ => let
	   val newR = C.newReg()
	   val instr' = HppaRewrite.rewriteUse(instr, reg, newR)
	 in {code=[reloadInstr(newR), instr'], proh=[newR]}
	 end
    end

    fun spillF {instr, reg} = let
    ...
    in
      case instr
      of I.FCOPY([fd], [fs], _) => {code=spillInstrs(fs), instr=NONE, proh=[]}
       | _ => let
	   val newF = C.newFreg()
	   val instr' = HppaRewrite.frewriteDef(instr, reg, newF)
	 in {code=spillInstrs(newF), instr=SOME instr', proh=[newF]}
	 end
    end

    fun reloadF {instr, reg} = let
      val disp = getFregLoc reg
      val tmpR = C.asmTmpR
    ...
    in
      case instr
      of I.FCOPY([fd], [fs], _) => {code=reloadInstrs(fd, []), proh=[]}
       | _ => let
	   val newF = C.newFreg()
	   val instr' = HppaRewrite.frewriteUse(instr, reg, newF)
    ...
    val cp        = IntRa.ra IntRa.COPY_PROPAGATION
  ...
  structure MLTreeGen = 
     MLRiscGen(structure MachineSpec=HppaSpec
		 ...
		 Hppa(structure Flowgen=FlowGraphGen
	       ...
	       structure PseudoOp=PseudoOpsBig)
