...
structure X86Assem = struct val outfile = ref(TextIO.stdOut) end

functor X86AsCode() : X86CODER = struct
...
fun die s = ErrorMsg.impossible ("x86/x86ascode.sml: " ^ s)

val offset = ref 0

local val i = ref 0 in
fun newlabel () = (i := !i + 1; "L" ^ Int.toString (!i))
end

fun itoa (i:int) = if i < 0 then "-" ^ Int.toString (~i)
		   else Int.toString i
...
fun oct i = let val m = Int.toString:int->string
...
fun emitlong (i : int) = (emit "\t.long "; emit(Int.toString i); emit "\n")

fun emitlab (offset,l2) =
	(emit "@@:\t.long "; emit l2; emit "-@b"; 
	 if offset < 0 then (emit "-"; emit (Int.toString (~offset)))
		       else (emit "+"; emit (Int.toString offset));
	 emit "\n")
...
fun jp  arg = (emit "\tjp"; emit1arg arg)
fun jnp arg = (emit "\tjnp"; emit1arg arg)
