...
fun show nil = say "."
  | show (v::vl) = (say(Int.toString(v:int)); say ","; show vl)

fun showl nil = say "]"
  | showl [x] = (say(Int.toString(x:int)); say "]")
  | showl (x::rest) = (say(Int.toString(x:int)); say "."; showl rest)
		      

fun showc (({defs=nil,path,...}:child)::vl) =
      (say "["; showl path; say "branch,"; showc vl)
  | showc ({defs=w::_,path,...}::vl) =
      (say "["; showl path; say(Int.toString w); say ","; showc vl)
  | showc nil = say "."

fun schedule(f,vl,cexp) =
let 
    val _ = if !Control.CG.printit
