...
sig
    ...
    val printExp : Basics.env -> BareAbsyn.exp * int * int -> unit
    val printRule : Basics.env -> BareAbsyn.rule * int * int -> unit
    val printVB : Basics.env -> BareAbsyn.vb * int * int -> unit
    val printRVB : Basics.env -> BareAbsyn.rvb * int * int -> unit
    val printDec : Basics.env -> BareAbsyn.dec * int * int -> unit
    val printStrexp : Basics.env -> BareAbsyn.strexp * int * int -> unit
end
...
fun printPat env =
  let
	...
	in printClosedSequence a b fields
      ...
      | printPat' (CONSTRAINTpat (p,t),d) = (printPat'(p,d-1); print " : "; printType env t)
  in printPat'
  end
  ...
  let 
    fun printDconPat'(_,_,_,0) = print "<pat>"
      ...
      | printDconPat'(CONSTRAINTpat(p,t),l,r,d) =
	    (print "("; printPat env (p,d-1); print " : "; printType env t; print ")")
	...
	let val dname = Symbol.name name
	    val fixity = EnvAccess.lookFIX env name
	    fun prdcon() =
...
fun printExp env =
  let
	...
	in printClosedSequence a b fields
	  ...
	  (print "("; printExp'(e,ind,d); print ":"; printType env t; print ")")
...
and printAppExp _ (_,_,_,_,0) = print "<exp>"
  | printAppExp env arg =
    let fun fixityprint(name,e,l,r,ind,d) =
		...
		val fixity = case name of [id] => EnvAccess.lookFIX env id
					| _ => NONfix
		fun pr() =
	  ...
	  (print "type ";
	   printvseq ind " and "
		 ...
		 (case arity
		  ...
		  printSym name; print " = "; printType env def)
	  ...
	  (print "exception ";
	   printvseq (ind+6) "and "
	     (fn (EBgen{exn=DATACON{name,...},etype}) =>
		   (printSym name;
		    case etype of NONE => ()
				| SOME ty' => (print " of "; printType env ty'))
	  ...
	  (print "functor ";
	   printvseq ind "and "
		 ...
		 (printSym fname; printAccess access; print " ("; 
		    print(formatQid pname); print ") = "; nlindent(ind+4);
		    printStrexp env (def,ind+4,d-1)))
	     fbs)
