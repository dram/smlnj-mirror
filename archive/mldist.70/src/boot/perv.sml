...
(*    val floor : real -> int = InLine.floor
    ...
    val subscriptf : string * int -> real = InLine.subscriptf
    val updatef : string * int * real -> unit = InLine.updatef
...
structure PreIO = 
  ...
  (* export the current world to the given file *)
    ...
    val debugInterface = 
	ref(InLine.cast(fn _ => let exception DebuggerNotPresent
...
(* NOTE: this code could be optimized by examining the directory entries incrementally *)
    ...
    open PreLim
    val version = "Standard ML of New Jersey, Version 0.70, 1 July 1991"
...
structure Array =
  struct
   local open InLine in
    ...
    val op sub : 'a array * int -> 'a = InLine.inlsubscript
