signature HISTORYSTORE = sig
  ...
  val hcreate: ((int * System.Unsafe.object) -> (System.Unsafe.object array))
  type storehandle
...
fun href (obj) = hcreate(1,obj)

fun harray(cnt,obj) = hcreate(cnt,obj)
	     ...
	     let fun add'(a,b,HLINK(time,wobj,nlink)) =
		   ...
		   | add'(a,b,NILhlink) =
		      ((*unsafe*)update(newtable,n,a);
		       (*unsafe*)update(newtable,n+(!size),b);
		       bucket(n+1))
...
in
  fun hcreate(cnt,value) =
    let val time = times sub 0
	fun reinit obj = 
	  let fun d (~1) = ()
	  ...
	  | dochain (NILhlink) =
        ...
        fun insert() = 
          let val addr = Bits.andb(time,!size-1)
              val (r,ch) = dochain (!table sub addr)
...
fun copyarray olda = 
  let val newa = array (Array.length olda, olda sub 0)
...
(*	    (case (strong warr) of
	       SOME a => if (not (cast (a sub 0) = unique0)) then
			   let val ai' = a sub i
...
inlined version follows: *)
	    ((*print " "; print i; print ":";*)
	     case (cast(System.Unsafe.subscript(cast warr,0)),
		   cast(System.Unsafe.subscript(cast warr,~1))) of
	       (a,22) => if (cast (a sub 0) = unique0) then
			   ((*print "U0";*)
			    douAl (next,acc))
			 else
			   let val ai' = a sub i
      ...
      in case (rc,strong wh) of
	   (ref 1,NONE) => (cls @ oldcls,ul::olduls,oldsd)
		 ...
		 fun markprune (STOREUPDATE(a,i,v,rest),acc) =
		      if not (cast (a sub i) = unique0) then
