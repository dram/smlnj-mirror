...
functor CPScomp(CM : CMACHINE) : 
		sig val compile : Lambda.lexp * ErrorMsg.complainer -> unit 
		end =
struct
...
fun compile(lexp,err) =
 let
  ...
  val cpsopt = if !System.Control.CG.cpsopt
		then time(CPSopt.reduce ctable,"cpsopt",System.Stats.cpsopt)
		else fn cps => cps
  ...
  val _ = codegen(carg,ctable,constant',err)
  val _ = debugmsg "done"
  in ()
 end
end (* functor CPScomp *)
