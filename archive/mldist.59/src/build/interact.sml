 ...
 (* functions for retrieving new bound lvars from declaration abstract syntax *)
  ...
  fun getvars (VALdec vbl) = smash (fn VB{pat,...} => patvars pat) vbl
    ...
    fun toploop(parser,inputSource as {anyErrors,lineNum,...}:inputSource,
	    ...
	    fun opt lambda =
		if !CGoptions.reduce orelse !CGoptions.hoist then
		let val timer = start_timer()
		...
		let (* val lambda = if !System.Control.reopen
		    ...
		    val code = Machm.generate(lambda,complainer)
		    val _ = if !anyErrors then raise Stop else ()
    ...
    and interact () = let
	  val inputSource = ErrorMsg.newSource("std_in",std_in,true,std_out);
	  ...
	  fun setcont () = (
		System.Unsafe.toplevelcont := callcc(fn k => (
		  ...
		  raise INTERRUPT)))
	  in
	    (setcont(); toploop(parser,inputSource,NODEBUG))
	      handle Eof => ()
		   | INTERRUPT => (print "\nInterrupt\n"; restart())
