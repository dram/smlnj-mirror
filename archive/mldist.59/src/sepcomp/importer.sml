...
struct
  ...
  val DEBUG = false
  val debug = 
      if DEBUG then fn str => outputc std_out ("<" ^ str ^ ">\n")
      else fn _ => ()
...
handle Match => (print "% Match raised in shiftFctStamps\n";  raise Match)
})
 ...
 (* New code (NICK) - I store the static information (StatModule) and
  ...
  fun timeFile filename = let
	val System.Timer.TIME{sec, ...} = SysIO.mtime(SysIO.PATH filename)
	in
	  SOME sec
	end
	  handle (Io _) => NONE
 ...
 (* uptodate should be memo'd sometime, since it's quite expensive. *)
  fun uptodate (path, myBinTime) name = let
	val {newPath, validName} = impliedPath(path, name)
	val _ = debug("uptodate(quotedName=" ^ name
			^ ", validName=" ^ validName ^ ")?")
	val timeSml = timeFile(validName ^ ".sml")
	val timeBin = timeFile(validName ^ ".bin")
	in
	  case (timeSml, timeBin)
	   of (SOME srcTime, SOME binTime) => ((
		debug("uptodate(" ^ validName ^ "): src time = "
		  ^ makestring srcTime ^ ", bin time = " ^ makestring binTime);
		if srcTime >= binTime	(* binary out of date *)
		orelse binTime >= myBinTime  (* Some other branch of the Make
						  task compiled this under me...? *)
		  then false
		  else let (* source is older; check imports *)
	    ...
	    | (NONE, NONE) =>
		fail("cannot find source or binary of required module "
		  ^ validName, "open")
      end (* uptodate *)


fun getModule(name,pervasives,TOPLEVEL_FNS{bind,lookup,getvars,opt})
	...
	let val {validName as filename, newPath as path} = impliedPath(path, name)
		...
		let val inputSource = ErrorMsg.newSource(name,source,false,std_out)
			...
			(case parser()
			  of EOF => raise Eof
			   | ABORT => fail("syntax error", "syntax")
			   | ERROR => fail("syntax or semantic error", "semantic")
			   | PARSE(BareAbsyn.IMPORTdec names) => 
			      let fun loop'([], dynMod, lvars, imports) =
					(dynMod, lvars, imports)
				    | loop'(name::rest, dynMod, lvars, imports)=
				        (* fixes provided by Nick, 4/12/90 *)
			   ...
			   | PARSE absyn => (* normal program *)
			       ...
			       then let val newLvars = getvars absyn
					val newMod = ModuleComp.addDeclaration
						       (inputSource,absyn, newLvars, dynModule)
						     handle ModuleComp.AddDeclaration =>
						     fail("error during translate",
							  "translate")
				     in loop(newMod, lvars @ newLvars, imports)
				    end
			       else fail("expecting SIGNATURE/FUNCTOR/IMPORT", "parse"))

			handle Eof => (dynModule, lvars, imports)
	 ...
	 in case (timeFile(srcName), timeFile(filename ^ ".bin"))
	     of (SOME _, NONE) =>  compSource() (* Source only: Compile! *)
		  ...
		  in
		    if (binVersion = BIN_VERSION)
		      then let
			val {statModule, dynModule, ...} = blastRead binary
			in
			  close_in binary;  done();
			  (statModule, dynModule)
			end
		      else (
			print "]\n";	(* Outstanding message... *)
			closing(fullName, indent);
			close_in binary;
			fail(fullName ^ "is the wrong format and can't open "
			  ^ srcName, "open"))
		  end

	      | (NONE, NONE) => fail("cannot open " ^ srcName, "open")
	end (* getModule' *)
