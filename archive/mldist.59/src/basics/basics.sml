...
structure Basics : BASICS = struct
  ...
  (* building variables *)
    ...
    fun mkVALvar id =
	VALvar{access = LVAR(namedLvar(id)), name = [id], 
	       typ = ref UNDEFty}
  ...
  (* building tycons, signatures, structures, and functors *)
    ...
    fun mkABStyc(path: symbol list, arity: int, eq: eqprop,
		 {tycStamps,...}: Stampset.stampsets)
	 : tycon =
	GENtyc{stamp = Stampset.newStamp(tycStamps),
	       path = path, arity = arity,
	       eq = ref eq, kind = ABStyc NULLtyc}
    
    fun mkDATAtyc(path: symbol list, arity: int, dconsref: datacon list ref, eq: eqprop,
	         {tycStamps,...}: Stampset.stampsets)
	 : tycon =
	GENtyc{stamp = Stampset.newStamp(tycStamps), path = path,
	      arity = arity, eq = ref eq, kind = DATAtyc dconsref}
