...
fun fetchindexb (Direct x, y, z) = (
      if (y = arithtemp) then raise Fetchindexb else ();
      move(Immed 0, y);
      case z
       of (Immed indx) => V.movb(Displace(x, indx), y)
	| (Direct indx) => V.movb(Index(x, 0, indx, Byte), y))
(* storeindexb(x,y,z) stores a byte: mem[y+z] <- x. *)
fun storeindexb (x, Direct y, Direct indx) = V.movb(x, Index(y, 0, indx, Byte))
