...
local
  open Access Basics BasicTypes BareAbsyn
...
fun WHILEexp (a,b) =
    let val fb as VALvar{name,access=LVAR v,typ} = mkVALvar whileSym
	val fa = VALvar{name=name,access=PATH[v],typ=typ}
        val id = fn x => x
...
fun FUNdec fbl =
    let fun fb2rvb (FB {var, clauses as (CLAUSE{pats,...}::_),tyvars}) =
	    let fun getvars (hd::tl) = mkVALvar argSym :: getvars tl
		  | getvars nil = nil;
...
fun SELECTORexp id = 
	let val v1 as VALvar{name,access=LVAR v,typ} = mkVALvar id
            val v2 = VALvar{name=name,access=PATH[v],typ=typ}
	 in FNexp(completeMatch[RULE(RECORDpat{fields=[(id,VARpat v1)], 
