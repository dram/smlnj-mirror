...
structure ReOpenTable = Dynamic(struct open Array
				       type elem = (L.lexp * Codenvl.Env)
				       type array = elem array
				end)

val reOpenTable = ReOpenTable.array(RECORD[],nil)
...
fun codegen(lab : Label, e0 as FN(v,e) : lexp, env: Env) =
 let fun genfn env =
   let fun gen(e : lexp, last : bool) : unit =
	...
	| APP(FN(w,b), INT i) =>
		genfn (augment(env,(VARentry w, CONST i))) (b,last)
	| APP(FN(w,b), e as STRING s) =>
		if length s = 1
		 then genfn (augment(env,(VARentry w, CONST(ord s)))) (b,last)
		 else let val lab = newlabel()
		      in  newfragment(lab,e,env);
			  genfn (augment(env,(VARentry w, KNOWN lab))) (b,last)
		      end
	...
	| FN(v,n) =>
		let val (_,[frag as (_,_,env)],close) = makenv(e, env, 0)
	...
	| FIX(nil,_,b) => gen(b,last)
        | FIX(vl as (v::_), el,b) => 
		let val (env0,frags,close) = makenv(e, env, ~(!offset)-1)
		    val len = length close
		in  Prof.closure len;
		    Machine.startalloc len;
		    copy(0, close);
	...
	| SWITCH (x as (APP(SELECT(i,VAR 0),RECORD[y,z]),
