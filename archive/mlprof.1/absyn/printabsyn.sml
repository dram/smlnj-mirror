...
fun printPat (_,0) = prstr "<pat>"
  ...
  | printPat (r as RECORDpat{fields,flex,...},d) =
      if isTUPLEpat r
	then printClosedSequence ("(",",",")") (fn(sym,pat)=>printPat(pat,d-1))
			         fields
        else printClosedSequence
	("{", ",", (if flex then ",...}" else "}"))
        (fn (sym,pat) => 
	     (printSym sym; prstr "="; printPat(pat,d-1)))
	fields
  ...
  | printPat (CONSTRAINTpat (p,t),d) =
      (printPat(p,d-1); prstr " : "; printType t)

and printDconPat(_,_,_,0) = prstr "<pat>"
  | printDconPat(CONpat(DATACON{name,...}),l:fixity,r:fixity,_) =
	printSym name
  ...
  | printDconPat(APPpat(DATACON{name,...},p),l,r,d) =
	...
	fun prdcon() =
		...
		| _ =>
		   (print dname; print " ";
		    printDconPat(p,NONfix,NONfix,d-1))
...
fun printExp(_,_,0) = prstr "<exp>"
  ...
  | printExp(r as RECORDexp fields,ind,d) = 
      if isTUPLEexp r
        then printClosedSequence ("(",",",")")
	     	(fn(_,exp)=>printExp(exp,ind+1,d-1))
		fields
        else printClosedSequence ("{", ",", "}")
		(fn (LABEL{name,...},exp) => 
...
and printDec(_,_,0) = prstr "<dec>"
  ...
  | printDec(TYPEdec tycons,ind,d) =
      (prstr "type ";
       printvseq ind " and "
         (fn (TYCON{name,kind=DEFtyc(TYFUN{arity,body}),...}) =>
	     (case arity
		of 0 => ()
		 | 1 => (prstr "'a ")
		 | n => (printTuple prstr (typeFormals n); prstr " ");
	      printSym name; prstr " = "; printType body)
	   | _ => impossible "printabsyn.398")
	 tycons)
  | printDec(DATATYPEdec{datatycs,withtycs},ind,d) =
      (prstr "datatype ";
       printvseq (ind+5) "and "
         (fn (TYCON{name,arity,kind=DATAtyc dcons,...}) =>
	     (case arity
		of 0 => ()
		 | 1 => (prstr "'a ")
		 | n => (printTuple prstr (typeFormals n); prstr " ");
  ...
  | printDec(EXCEPTIONdec ebs,ind,d) =
      (prstr "exception ";
       printvseq (ind+6) "and "
	     ...
	     (printSym name;
	      case ty of NONE => () | SOME ty' => (prstr " of "; printType ty');
	      case def
	        of NONE => ()
