...
struct
  ...
  open ErrorMsg Basics EnvAccess TypesUtil
  ...
  (* following is a temporary fix until type definitions are done right
  fun trueTycStamp(TYCON{stamp,kind=DEFtyc{params,def=CONty(ref tyc, args),...}) =
	let fun eqparams([],[]) = true
	      | eqparams(tv::prest,VARty(tv')::arest) =
		  if eqTyvar(tv,tv') then eqparams(prest,arest) else false
	      | eqparams _ = false
	 in if eqparams(params,args)
	    then trueTycStamp(tyc)
	    else stamp
	end
    | trueTycStamp tycon = tycStamp tycon
*)
  fun getStr([],str) = str
  ...
  fun doSharing(table,env as {s=senv,t=tenv},{s=strShare,t=typeShare}) =
	  ...
	  and sMerge(str1 as STRstr{stamp=s1,kind=k1,env={s=senv1,t=tenv1,...},...},
		     str2 as STRstr{stamp=s2,kind=k2,env={s=senv2,t=tenv2,...},
			     	    table,...}) =
	      case (k1,k2)
		of (STRkind _, STRkind _) => if s1 = s2 then () else condemn "sharing 1"
