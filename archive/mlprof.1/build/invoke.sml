...
(*  structure Reopener = Reopen(struct structure C=Machine and Machm=Codes.Machm end) *)
  ...
  val bucket0 = ref (Lambda.RECORD [])
  val bucket1 = ref (Lambda.RECORD [])
  val _ = System.prLambda := fn () => (MCprint.printLexp (!bucket0); newline())
  fun spoolLambda l = if !saveLambda then (bucket0 := !bucket1; bucket1 := l)
		      else ()
  ...
  in
    ...
    fun getvars (VALdec vbl) =
	  ...
	  smash ebl
	    (fn EB{exn=DATACON{rep=VARIABLE(LVAR v),...},def=NONE,...} =>
		  ([],[v])
	      | EB{def=SOME _,...} => ([],[])
  ...
  local 
    ...
    fun gencode lambda =
	let val (size,emit) = (Machine.generate lambda;
			       Codes.Machm.assemble())
	    val _ = if !anyErrors then raise Stop else ()
