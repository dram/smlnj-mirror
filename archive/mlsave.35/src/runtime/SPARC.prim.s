 ...
 *
 * %o0,%o1 = arith. temps
 * %g1-%g4,%i4-%i5,%l0-%l7 = misc. registers (contain only pointers or tagged ints)
 * %o5 = pointer temp.
 *
 * %o2,%o3 = used for args to ml_mul & ml_div. Also as temps.
 * %o4 = temp register.
 *
 ...
 * in array_v, create_s_v and in scalb_v.  It it important that only registers
 * saved by _saveregs be live at these points.	Also, only pointer registers can
 * contain heap pointers at these points.
 */
...
Closure(_array_v)
	...
	dec	4,%o0			/* length-- */
	clr	[%g6+%o0]		/* allocate, this can cause GC */
...
Closure(_create_b_v)
	ba	2f
	mov	tag_bytearray,%o2   /* (delay slot) */
...
2:	sra	%i0,1,%o0	    /* %o0 = length (sparc int) */
	...
	bne	3f
	nop
	set	4f,%o0		    /* this can't go in the delay slot */
	call	_die
	nop
	.align	4
...
Closure(_scalb_v)
	...
	st	%o0,[%g6+4]	    /* allocate the new real value, this can cause GC */
	st	%o1,[%g6]
	set	mak_desc(8,tag_string),%o0
	st	%o0,[%g6-4]
	mov	%g6,%i0		    /* set result. */
	inc	12,%g6		    /* store ptr += 3 */
1:	CONTINUE
...
_saveregs:
	...
	st	%g1,[%o3+64]		/* save %g1 */
	st	%g4,[%o3+68]		/* save %g4 */
	st	%o5,[%o3+72]		/* save %o5 (pointer temp) */
...
_restoreregs:
	...
	ld	[%o3+68],%g4		/* restore %g4 */
	ld	[%o3+72],%o5		/* restore %o5 (pointer temp) */
	set	_saved_nonptrs,%o3
