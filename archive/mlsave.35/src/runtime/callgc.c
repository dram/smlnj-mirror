...
struct timeval prev_t_o_d;
struct timezone zone;

initadvice()
{
    otime=zero; ogtime=zero;
    gettimeofday(&prev_t_o_d,&zone);
}

int advice(s0,sf,t0,tf,tw) int s0,sf,t0,tf,tw;
{
 printf("s0=%8d, sf=%8d, t0=%8d, tf=%8d, tw=%8d\n", s0,sf,t0,tf,tw);
 return 0;
}
...
{int s0 = arenabase+2*live_size;
 int sf = arenasize;
 int delta; struct rusage adv; struct timeval newgtime, newttime, t_o_d;
 getrusage(0,&adv);
 ...
 gettimeofday(&t_o_d,&zone);
 delta = advice(s0,sf,milli(newttime),milli(newgtime),
			    milli(subt(t_o_d,prev_t_o_d)));
 prev_t_o_d=t_o_d;
/* if (arenasize+delta < 3*live_size) return 3*live_size;
     else return arenasize+delta; */
