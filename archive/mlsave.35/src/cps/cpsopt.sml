...
fun contract last cexp =
 let val mkconst = Intmap.add ctab
	...
	(checkcount z;
	 ...
	 let val v' = ren v
	 in (case get v' of
	       {info=SELinfo iw,used=ref 1,...} =>
	            let val (x,p0) = selectop iw
		    in click "b"; (x,combinepaths(p0,p))
		    end
	     | {info=RECinfo vl,used=ref 1,...} =>
	       (case p of
	          SELp(i,p) => let val (x,p') = nth(vl,i)
			       in click "*b*"; (ren x, combinepaths(p',p))
			       end
	        | _ => (v',p))
	     | _ => (v',p))
...
fun expand(cexp,bodysize) =
      ...
      of RECORD(vl,w,ce) => RECORD(vl, w, beta(n,d+2+length vl, ce))
	   ...
	   ((case get v
		...
		let val size = !size
		  ...
		  val predicted = !call * ((size-whatsave(0,vl,args))-(1+length vl))
                  val depth = 1
		  val max = 5
  ...
  val delta = bodysize div rounds handle Div => 0
  val reducemore = !System.Control.CG.reducemore
  fun cycle(0,cexp,size) = contracter true cexp
    | cycle(k,cexp,size) = 
	let val _ = debugprint "\nHoist: "
	    val cexp = hoist cexp
	    val _ = clicked := 0
	    val cexp = expand(cexp,bodysize)
	    val cl = !clicked before clicked := 0
        in if cl <= reducemore
	   then contracter true cexp
	   else cycle(k-1, contract false cexp, size-delta)
	end
in cycle(rounds,contracter false cexp, bodysize)
   before (debugprint "\n"; debugflush())
end
end
