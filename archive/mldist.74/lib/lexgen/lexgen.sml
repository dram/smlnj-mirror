...
Copyright (c) 1989, 1990, 1991 by Andrew W. Appel,
...
val pr_err = fn x => (outputc std_out ("sml-lex: Error: "^
				       (makestring (!LineNum))^
				       ": "^x^"\n"); raise error)
val pr_synerr = fn x => (outputc std_out ("sml-lex: Syntax error: "^
				       (makestring (!LineNum))^
				       ": "^x^"\n"); raise error)
...
****)
			| x => if (isidentchr x) then
				getID(matched^x)
				else (pr_synerr "invalid char. class name")
			in ID(getID(ch))
			end
