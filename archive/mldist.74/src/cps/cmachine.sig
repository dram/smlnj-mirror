...
val beginStdFn : unit -> unit;  (* Note the beginning of a standard function;
                                 useful for machines that have to don't
                                 have PC-relative instructions and have
                                 to reset a base register *)
...
val jmpindexb : EA*EA -> unit	    (* jmpindexb(x,y)    pc <- (x+y) *)
    				    (* fetchindexl(x,y,z) fetches a word:
				     *    y <- mem[x+2*(z-1)] 
				     *)
val fetchindexl : EA * EA * EA -> unit   
				    (* storeindexl(x,y,z) stores a word:
				     *    mem[y+2*(z-1)] <- x 
                                     *)
...
val mulf : EA * EA * EA -> unit
val divf : EA * EA * EA -> unit
val addf : EA * EA * EA -> unit
val subf : EA * EA * EA -> unit
