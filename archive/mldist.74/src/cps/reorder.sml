...
val rec reorder : lexp -> {regs: int, side: bool, exp: lexp} =
  fn e as VAR _ => {regs=0, side=false, exp=e}
         ...
         let val {regs,side,exp}= reorder e0
          ...
          in case d 
              of SOME d' => let val {regs=dr,side=ds,exp=de} = reorder d'
               ...
               | NONE => {regs=lr,side=ls,exp=SWITCH(exp,sign,l',NONE)}
          end
   | CON(c,e) => let val {regs,side,exp} = reorder e
	          in {regs=regs, side=side, exp=CON(c,exp)} (* close enuf *)
		 end
   | DECON(c,e) => let val {regs,side,exp} = reorder e
	          in {regs=regs, side=side, exp=DECON(c,exp)}
		 end
   ...
   | RAISE e =>     let val {regs,side,exp} = reorder e
                     in {regs=max(1,regs),side=true,exp=RAISE exp}
                    end
