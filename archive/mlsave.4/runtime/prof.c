...
print_alloc_info()
{
	...
	if (!profvec[REFCELLS]) printf("Refs:  %d created.\n",
					profvec[REFCELLS]);
	else printf("Refs:  %d created, total size %d words.\n",
		profvec[REFCELLS],
		profvec[REFCELLS]);

	if (!profvec[REFLISTS]) printf("Ref list elements:  %d created.\n",
					profvec[REFLISTS]);
	else printf("Ref list elements:  %d created, total size %d words (no descriptor).\n",
		profvec[REFLISTS],
		2*profvec[REFLISTS]);
