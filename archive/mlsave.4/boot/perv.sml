  ...
  struct
   ...
   struct
    ...
    struct
      ...
      open InLine
      exceptionx substr
      fun substr("",0,0) : string = ""
(* without the string type constraint the typechecker dies with error
fresh in freshTy.  This may be due to the new strings, since the old
version works. *)
	| substr("",_,_) = raisex substr
        | substr(s,i,0) = (*if i<0 orelse i >= slength s
			    then raisex substr
			    else*) ""
			    (* someone is causing this to die; perhaps
			       input_line *)
    ...
    struct
       ...
       fun input(f as {filid,pos,len,buf,closed}, 0) =
	   if !closed then raisex io_failure with "Input stream is closed"
	   else ""
		...
		let val Sub = cast InLinePrim.subscript
		    fun m i =
		      if ieq(i,alen) then true
 		      else if polyequal(Sub(a,i),Sub(b,i)) then m(Pervasives.Integer.+(i,1))
		      else false
		in m 0 end
	    fun mapstr () =
		let val Sub = cast InLinePrim.subscript
		    fun m i =
		      if Pervasives.Integer.>(i,alen) then true
 		      else if ieq(Sub(a,i),Sub(b,i)) then m(Pervasives.Integer.+(i,4))
		      else false
		in m 1 end
