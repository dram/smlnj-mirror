...
void print(int dummyFP, struct string *s)
...
{int i, ans;
 ...
 ans = toymain(0 /* static link!? */);
 printf("Answer = 0x%x\n", ans);
 return ans;
}

int ord(int dummyFP, struct string *s)
{
 if (s->length==0) return -1;
 else return s->chars[0];
}

struct string *chr(int dummyFP, int i)
{
 if (i<0 || i>=256) 
...
int size(int dummyFP, struct string *s)
{ 
 return s->length;
}

struct string *substring(int dummyFP, struct string *s, int first, int n)
{
...
struct string *concat(int dummyFP, struct string *a, struct string *b)
{
