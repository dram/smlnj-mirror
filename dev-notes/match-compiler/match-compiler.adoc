...
DEFN: A pattern is "relevant" if it either discriminates (can't be matched by all values
of its type) or destructs (i.e. binds variables to components of a matching value).
Matching an irrelevant pattern has no effect, i.e. no case discrimination is made,
and no variable is bound.
...
Possible Useful Auxiliary Functions
-----------------------------------
