...
It is not clear what the corresponding notion is for OR (cor or por) patterns is.

Semantics of a pattern (2nd version)
-------------------------------------
...
7. Horizontal vs vertical overlays:
  Horizontal:
...
Note on pattern/layer subsumption
---------------------------------
...
Note on pattern/path/rule relationship
--------------------------------------
...
Note & Examples for top-level variables in OR patterns
------------------------------------------------------
...
Pattern _overlays_ or _layers_ with priorities
----------------------------------------------
...
Preliminary Implementation Ideas for OR ("layered") patterns
--------------------------------
...
Note on AndOr nodes and rules/layers
------------------------------------
...
bound multiple times in different, related, hlayers). [An hlayer is
...
Here the root of the left OR-pattern is <0> and the root of the right OR-pattern
is <1>. They have different roots so they are not "related".  Thus the
pattern is illegal because it has multiple occurrences of x in unrelated
layers. The following pattern would be legal.
...
(need "relative roots" of families of layers?)
