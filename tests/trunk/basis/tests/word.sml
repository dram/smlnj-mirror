...
val test9c = check (maxnegint = Word.toIntX (<< (i2w 1, i2w 30)));
...
val test10b = checkrange (1,65)
    (fn k => ~1 = Word.toIntX (~>> (i2w ~1, i2w k)));
...
val test12b = checkrange(0, 300) (fn k => Word.toIntX (i2w k - i2w 17) = sub(k, 17));
...
val test12f = checkrange(0, 300)
    (fn k => Word.toIntX (i2w k + i2w maxnegint) = add(k, maxnegint));
val test12g = checkrange(0, 300)
    (fn k => w2i (i2w maxnegint - i2w k - i2w 1) = sub(maxposint,k));
val test12h = checkrange(0, 300)
    (fn k => Word.toIntX (i2w k * i2w maxnegint) = mul(imod(k, 2), maxnegint));
val test12i = checkrange(0, 300)
    (fn k => Word.toIntX (i2w k * i2w maxposint + i2w k) = mul(imod(k, 2), maxnegint));
val test12j = checkrange(0, 300)
    (fn k => w2i (i2w k div i2w ~1) = 0);
val test12k = checkrange(0, 300)
    (fn k => Word.toIntX (i2w k mod i2w ~1) = k);
val test12l = check(Word.toIntX (i2w maxposint + i2w 1) = maxnegint);
