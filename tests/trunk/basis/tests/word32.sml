...
val test9b = checkrange (32,65)
    (fn k => 0 = w2i (<< (i2w 1, itow k)));
(* val test9c = check (maxnegint = Word32.toIntX (<< (i2w 1, itow 30))); *)
val test9d = checkrange (0, 1025)
    (fn i => 2 * i = w2i (<< (i2w i, itow 1)));
val test9e = checkrange (0, 1025)
    (fn i => i div 2 = w2i (>> (i2w i, itow 1)));
val test9f = checkrange (0,65)
    (fn k => rwp maxposint k = w2i (>> (i2w maxposint, itow k)));
...
val test10b = checkrange (1,65)
    (fn k => ~1 = Word32.toIntX (~>> (i2w ~1, itow k)));
val test10c = checkrange (~513, 513)
    (fn i => i div 2 = toIntX (~>> (i2w i, itow 1)));
val test10d = checkrange (0,65)
    (fn k => rwp maxnegint k = toIntX (~>> (i2w maxnegint, itow k)));
...
val test12f = checkrange(0, 300) 
    (fn k => Word32.toIntX (i2w k + i2w maxnegint) = add(k, maxnegint));

(*
val test12g = checkrange(0, 300) 
    (fn k => w2i (i2w maxnegint32 - i2w k - i2w 1) = sub(maxposint, k));

val test12h = checkrange(0, 300) 
    (fn k => Word32.toIntX (i2w k * i2w maxnegint) = mul(imod(k, 2), maxnegint));
val test12i = checkrange(0, 300) 
    (fn k => Word32.toIntX (i2w k * i2w maxposint + i2w k) = mul(imod(k, 2), maxnegint));
val test12j = checkrange(0, 300) 
    (fn k => w2i (i2w k div i2w ~1) = 0);
val test12k = checkrange(0, 300) 
    (fn k => Word32.toIntX (i2w k mod i2w ~1) = k);
val test12l = check(Word32.toIntX (i2w maxposint + i2w 1) = maxnegint);
